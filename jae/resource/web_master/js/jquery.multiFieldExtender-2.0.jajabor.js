...
(function ($) {
    $.fn.EnableMultiField = function (options) {
        ...
        return this.each(function () {
            ...
            // $(self).append(" <a href='#' class='" + options.linkClass + "'>" + options.linkText + "</a>"); // JS 
            $(self).find('.button-list').append(" <a href='#' class='" + options.linkClass + "'>" + options.linkText + "</a>");
            ...
            function handleAdd(elem) {
                ...
                if (options.maxItemsAllowedToAdd === null || totalCount < options.maxItemsAllowedToAdd) {
                    ...
                    $(elem).parent().parent().after(newElem);
                    
                    $(elem).parent().parent().find("." + options.linkClass).remove();
                    ...
                    if (options.enableRemove) {
                        if ($(newElem).find("." + options.removeLinkClass).length === 0) {
                            $(newElem).find('.button-list').append(" <a href='#' class='" + options.removeLinkClass + "'>" + options.removeLinkText + "</a>");
                        }
                    ...
                    $(newElem).find('.button-list').append(" <a href='#' class='" + options.linkClass + "'>" + options.linkText + "</a>");
            ...
            function handleRemove(elem) {
                ...
                if (cnt) {
                    var prevParent = $(elem).parent().parent().prev();
                    ...
                    if ($(elem).parent().find("." + options.linkClass).length > 0) {
                        if (options.enableRemove && $(self).attr("uniqueId") != $(prevParent).attr("uniqueId")) {
                            if ($(prevParent).find("." + options.removeLinkClass).length === 0) {
                                $(prevParent).find('.button-list').append(" <a href='#' class='" + options.removeLinkClass + "'>" + options.removeLinkText + "</a>");
                            }
                        ...
                        $(prevParent).find('.button-list').append(" <a href='#' class='" + options.linkClass + "'>" + options.linkText + "</a>");
