...
class User_service_model extends MY_Model
{
    ...
    */
    public function user_service_load($condition,$limit=NULL,$offset=NULL,$order_by=NULL)
    { 
        $rs=new stdClass();
        if(is_integer($condition))
        {
            ...
            $this->db->select('u.s_user_name,c.s_category,sc.s_sub_category,s.*');
    ...
    */
    public function add_user_service($values)
    {
        
        return $this->add_("user_service",$values);
        //$this->db->insert('', $data); 
    }
    ...
    */
    public function update_user_service($values,$where)
    {
        return $this->update_("user_service",$values,$where);
    }    
    ...
    *                       "name='Joe' AND status='boss' OR status='active'";
    */
    public function delete_user_service($where)
    {
        return $this->delete_("user_service",$where);
    }
    
    /**
    * Update specific field, i_active, 
    * 
    * @param mixed $values, 0,1 if empty then set the reverse value by default
    * @param mixed $where
    * 
    * @see, controllers/user_services.php, ajax_changeStatus()
    */
    public function update_user_service_status($values="",$where)
    {
        $values=(empty($values)?"i_active=1-i_active": "i_active=".intval($values));
        
        $qry="UPDATE user_service SET ".$values." " 
             ." WHERE ".$where;
