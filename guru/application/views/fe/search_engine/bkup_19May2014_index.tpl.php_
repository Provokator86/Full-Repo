...
<script type="text/javascript">
    jQuery(function($){
        $(document).ready(function(){
			...
			/* to remove text when click to type on any field right panel feb 2014 */
			$(".auto_remove").click(function(){			
				$(this).val('');
			}).blur(function(){
				var def_val = $(this).attr('rel');
				$(this).val(def_val);
			});
			/* to remove text when click to type on any field right panel*/
            ...
            if(search_type_value!="" && search_type_value!="Keyword")
            {
...
<script type="text/javascript">
var tot_records = '<?php echo count($search_result) ?>';
tot_records = parseInt(tot_records);
total = 0;  // Total page
function manage_page_navigation(current)
{
	$('.pagin_link').hide();
	...
	if(current == 1) // First page
	{
		for(i = 1; i <= 3; i++)
			$('.link_'+i).show();
		$(".res_cnt").html(current*10);
	}
	else if (current == total) // Last page
	{
		for(i = total; i >= (total-2); i--)
			$('.link_'+i).show();
		$(".res_cnt").html(tot_records);
	}
	else
	{
...
{
	var _this = $(t),current = _this.attr('rel');
	$('.pagination').find('.active').removeClass('active'); // Remove selected
	_this.parent('li').addClass('active'); // Pagination show current page link selected
	$('.paging').hide(); // Hide current page
	$('.paging_'+current).show(); // Show current page
	
	manage_page_navigation(current); // Generate paginf link
...
$(document).ready(function(){
	var link = '', 
	nxt = '<li><a href="javascript:;" class="nxt">Next &raquo;</a></li>',
	prev = '<li><a href="javascript:;" class="prev">&laquo; Previous</a></li>';
	
	$('.paging').each(function(i){
		// Create pagin link
		var _this = $(this), rel = $(this).attr('rel');
		if(i == 0) {
			link +='<li class="active"><a href="javascript:;" class="pagin_link link_'+rel+'" rel="'+rel+'" onclick="javascript:show_page(this)">'+rel+'</a></li>';
		} else {
			link +='<li><a href="javascript:;" class="pagin_link link_'+rel+'" rel="'+rel+'"  onclick="javascript:show_page(this)">'+rel+'</a></li>';
			_this.hide();
		}
		_this.addClass('paging_'+rel);
		total++;
	});
	
	if(total>1)
	{
		$('#pagination_link').html('<ul class="pagination">'+prev+link+nxt+'</ul>');
		manage_page_navigation(1); // Generate paginf link
	}
	else
	{		
		$(".res_cnt").html(tot_records);
	...
	$('.nxt').on('click',function(){
		var t =  $('.pagination').find('.active');
		var current = t.children('a').attr('rel');
		var n = parseInt(current)+1;
		t.next('li').addClass('active');
		manage_page_navigation(n); // Generate paginf link
		if(n <= total)
		{
			t.removeClass('active'); // Remove selected
			t.parent('li').addClass('active'); // Pagination show current page link selected
			
			$('.paging').hide(); // Hide current page
			$('.paging_'+n).show(); // Show current page
		}
		if(current > 0)
			$('.prev').show();
...
.listing a,
.listing a:hover{background-color:none !important;}
.more-link{ background-color:none !important; border:none !important; display:inline !important;}
...
<div class="col_right_sidebar">
    ...
    <div class="left_panel">
        ...
        <?php 
            ...
            {
				$i = 0; $rel = 1;
				echo '<div class="paging" rel="'.$rel.'">';
                foreach($search_result as $k=>$v)
                {
					if($i%10 == 0 && $i > 0) echo '</div><div style="display:none;" class="paging" rel="'.++$rel.'">';
                    $featured_listing="";
                ...
                <div class="listing <?=$featured_listing;?>">
                    <div class="left"> <a href="<?=site_url($v->s_service_short_url)?>"><img src="<?=site_url($v->s_profile_image?$v->s_profile_image:'resources/no_image.jpg')?>" width="120" height="120" alt="pic" /></a>
                        <a href="<?=site_url($v->s_service_short_url)?>" class="grey_button">View full Profile</a>
                    </div>
                    <div class="right">
                        <div class="border_bottom">
                            <h2 class="alignleft"><strong class="alignleft"><a href="<?=site_url($v->s_service_short_url);?>" class="black"><?=$v->s_service_name;?></a></strong> 
							<?php if(intval($v->i_user_fb_level)>0) { ?>
							<span class="alignleft short_grey_button"><?=levelToStr(intval($v->i_user_fb_level));?></span>
							<?php } ?>
							</h2>
                        </div>
                        <div class="border_bottom botmar20">
                            
							<p class="botmar20">Service Provided by <a href="<?=site_url($v->s_user_short_url);?>"><?=$v->s_provided_by;?></a> 
							
							<? /*<img src="<?= get_theme_path('guru_frontend/')?>images/pic2.png" width="14" height="13" alt="pic" /> <img src="<?= get_theme_path('guru_frontend/')?>images/pic2.png" width="14" height="13" alt="pic" /> <img src="<?= get_theme_path('guru_frontend/')?>images/pic2.png" width="14" height="13" alt="pic" /> */
                ...
                echo '<img src="'.get_theme_path('guru_frontend/').'images/pic2.png" width="14" height="13" alt="pic" />';
                        ...
                        <div class="border_bottom">
                            <?php /*?><p class="botmar20"><?= $v->s_service_desc;?> <a href="<?=site_url($v->s_service_short_url);?>" class="orange">Read more</a></p><?php */?>
							<p class="botmar20 more_txt" id="more_txt_<?=$v->service_id?>"><?= string_part(format_text($v->s_service_desc),345);?> <a href="<?=site_url($v->s_service_short_url);?>" class="orange">Read more</a></p>
                            <?php /*?><span class="short grey">Data provided by <a href="javascript:void(0);">www.webdesign.guru.in</a></span><?php */?>
                        </div>
                        <ul class="alignleft">
				...
				echo '</div>';
				/*if(!empty($search_result_pager))
				{
                	echo '<div class="listing">'.$search_result_pager.'</div>';
				}*/
				if($i>10)
					echo '<div id="pagination_link" class="listing"></div>';
            }
            else
            {
    ...
    <div id="extended_fields" class="right_sidebar">
        ...
        <div class="choose_panel"> 
            <p class="select relative"><input type="text" class="auto_remove" id="search_services" rel="Select service type" name="search_services" value="Select service type" /></p>
            <ul>
        ...
        <div id="div_search_specialization" class="choose_panel"> 
            <p class="select"><input type="text" class="auto_remove" id="search_specialization" name="search_specialization" rel="Specialization" value="Specialization" /></p>
            <ul>
                <?php /*
        ...
        <div id="div_search_qualification" class="choose_panel"> 
            <p class="select"><input type="text" class="auto_remove" id="search_qualification" name="search_qualification" rel="Highest Qualification" value="Qualification" /></p>
            <ul>
                <?php /*
        ...
        <div id="div_search_experience" class="choose_panel"> 
            <p class="select"><input type="text" class="auto_remove" id="search_experience" name="search_experience" rel="Experience" value="Experience" /></p>
            <ul></ul>
            <a class="short_grey_button refine" href="javascript:void(0);">Refine</a>
        </div>
        <?/*s_classes_ids*/?>
        <div id="div_search_classes" class="choose_panel"> 
            <p class="select"><input type="text" class="auto_remove" id="search_classes" name="search_classes" rel="Classes Teach" value="Classes" /></p>
            <ul></ul>
            <a class="short_grey_button refine" href="javascript:void(0);">Refine</a>
        </div>  
        <?/*s_medium_ids*/?>
        <div id="div_search_medium" class="choose_panel"> 
            <p class="select"><input type="text" class="auto_remove" id="search_medium" name="search_medium" rel="Language Medium" value="Classes" /></p>
            <ul></ul>
            <a class="short_grey_button refine" href="javascript:void(0);">Refine</a>
        </div>          
        <?/*d_tution_fee*/?>
        <div id="div_search_tution_fee" class="choose_panel"> 
            <p class="select"><input type="text" class="auto_remove" id="search_tution_fee" name="search_tution_fee" rel="Tution fee" value="Classes" /></p>
            <ul></ul>
            <a class="short_grey_button refine" href="javascript:void(0);">Refine</a>
        </div>          
        <?/*s_tution_mode_ids*/?>
        <div id="div_search_tution_mode" class="choose_panel"> 
            <p class="select"><input type="text" class="auto_remove" id="search_tution_mode" name="search_tution_mode" rel="Tution mode" value="Classes" /></p>
            <ul></ul>
            <a class="short_grey_button refine" href="javascript:void(0);">Refine</a>
        </div>               
        <?/*s_other_subject_ids*/?>
        <div id="div_search_subjects" class="choose_panel"> 
            <p class="select"><input type="text" class="auto_remove" id="search_subjects" name="search_subjects" rel="Subject" value="Subject" /></p>
            <ul></ul>
            <a class="short_grey_button refine" href="javascript:void(0);">Refine</a>
        </div>
        <?/*d_rate*/?>
        <div id="div_search_rate" class="choose_panel"> 
            <p class="select"><input type="text" class="auto_remove" id="search_rate" name="search_rate" rel="Hourly Rate" value="Rate" /></p>
            <ul></ul>
            <a class="short_grey_button refine" href="javascript:void(0);">Refine</a>
        </div>        
        <?/*s_employment_type_id*/?>
        <div id="div_search_employment_type" class="choose_panel"> 
            <p class="select"><input type="text" class="auto_remove" id="search_employment_type" name="search_employment_type" rel="Employment type" value="Employment type" /></p>
            <ul></ul>
            <a class="short_grey_button refine" href="javascript:void(0);">Refine</a>
        </div>             
        <?/*s_availability_ids*/?>
        <div id="div_search_availability" class="choose_panel"> 
            <p class="select"><input type="text" class="auto_remove" id="search_availability" name="search_availability" rel="Availability"  value="Availability" /></p>
            <ul></ul>
            <a class="short_grey_button refine" href="javascript:void(0);">Refine</a>
        </div>    
        <?/*s_tools_ids*/?>
        <div id="div_search_tools" class="choose_panel"> 
            <p class="select"><input type="text" class="auto_remove" id="search_tools" name="search_tools" rel="Tools" value="Tools" /></p>
            <ul></ul>
            <a class="short_grey_button refine" href="javascript:void(0);">Refine</a>
        </div>         
        <?/*s_designation_ids*/?>
        <div id="div_search_designation" class="choose_panel"> 
            <p class="select"><input type="text" class="auto_remove" id="search_designation" name="search_designation" rel="Designation"  value="Designation" /></p>
            <ul></ul>
            <a class="short_grey_button refine" href="javascript:void(0);">Refine</a>
        </div>           
        <?/*e_gender*/?>              
        <div id="div_search_gender" class="choose_panel"> 
            <p class="select"><input type="text" class="auto_remove" id="search_gender" name="search_gender" rel="Select Gender" value="Select Gender" /></p>
            <ul>
                <?php /*
        ...
        <div id="div_search_locality" class="choose_panel"> 
            <p class="select"><input type="text" class="auto_remove" id="search_locality" name="search_locality" rel="Narrow by locality" value="Narrow by locality" /></p>
            <ul>
                <?php /*
        ...
        <div id="div_search_fb_circle" class="choose_panel"> 
            <p class="select"><input type="text" class="auto_remove" id="search_fb_circle" name="search_fb_circle" rel="Narrow by fb circle" value="Narrow by fb circle" /></p>
            <ul></ul>
            <a class="short_grey_button refine" href="javascript:void(0);">Refine</a>
        </div>
    </div>
