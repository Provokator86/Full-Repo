...
(function($) {
	...
	var $tooltip = $('<div id="divToolTip"><div></div></div>');
	...
	//Runtime variable definations
		...
		var title=$(element).attr('tip_content');///added by sh
		var filename = options.folderurl + id + '.' + options.filetype;
		var dialog_id = '#divToolTip';
		$(dialog_id).addClass("acumen_tool_tip");
		...
		$(this).hover(function(e){
				...
				}else
				{
					if($('#'+options.dataAttr + '_' + id).length > 0){
						...
						//$(dialog_id).html(id);//original
						//$(dialog_id).html(size);
						//$(dialog_id).html(title);
						$("#divToolTip div").text(title);
					}
				}
