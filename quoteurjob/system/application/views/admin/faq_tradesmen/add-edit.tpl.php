...
function ajax_call_get_content(ajaxURL,language_id,category_id,s_question)
{
	jQuery.noConflict();
	jQuery(function($){
		$.ajax({
				...
				data: "language_id="+language_id+"&cat_id="+category_id+"&s_question="+s_question,
				dataType:"json",
				success: function(msg){		
				
					if(msg!=null)
					{						
						tinyMCE.get("txt_faq_answer").setContent(msg.s_answer);
					}
					else
					{					
						tinyMCE.get("txt_faq_answer").setContent('');
					}	
...
<input type="hidden" id="h_id" name="h_id" value="<?php echo $posted["h_id"];?>"> 
	...
	<div class="info_box">From here Admin can add or edit the FAQ for tradesman .</div>
	<div class="clr"></div>
    <p>&nbsp;</p>
        <div id="div_err">
            <?php
    ...
    <div>
      <table width="100%" border="0" cellspacing="0" cellpadding="0">
		...
		<tr>
          <td>Language *:</td>
          <td>
		  <select name="opt_language" id="opt_language" onchange='ajax_call_get_content("ajax_get_content",this.value,opt_cat.value,txt_faq_question.value);'>
		...
		<tr>
          <td>Category *:</td>
          <td>
		  <select id="opt_cat" name="opt_cat" onchange='ajax_call_get_content("ajax_get_content",opt_language.value,this.value,txt_faq_question.value);' >
		  	<option value="">Select</option>
			<?php echo makeOptionCategory('s_category_type="'.tradesman_faq.'" AND c.i_status=1 AND cc.i_lang_id =1',$posted["opt_cat"]); ?>
		  </select>
		  </td>
          <td>&nbsp;</td>
        </tr>
