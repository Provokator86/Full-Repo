    ...
    if($this.val()!="")
    {
        ...
        $.post(g_controller+"ajax_checkduplicate",
               {"h_id":$("#h_id").val(),
				...
				"h_french_value":$("#txt_category_name_2").val()
                  ...
                  if(data!="valid")////invalid 
                  {
				  	if(data=="Duplicate exists")
					{
					 ...
					 // alert('sam');
                      ...
                      .insertAfter("#txt_category_name_1");
					  }
					  else if(data=="Duplicate category exists")
					  {
					  	$("#txt_category_name_2").focus();
						$('<div id="err_msg" class="star_err1">Duplicate exists.</div>')
                      .insertAfter("#txt_category_name_2");
...
$("#frm_add_edit").submit(function(){
	...
	if($.trim($("#txt_category_name_1").val())=="") 
	{
		s_err +='Please Provide Category Name In  English.<br />';
		b_valid=false;
	}
	if($.trim($("#txt_category_name_2").val())=="") 
	{
		s_err +='Please Provide Category Name In French.<br />';
		b_valid=false;
	}
    ...
    <div>
      <table width="100%" border="0" cellspacing="0" cellpadding="0">
		...
		<?php /*?><tr>
			<td>Category Type *:</td>
			<td>
				<!--<select name="opd_category_type" id="opd_category_type" onchange='call_ajax_combo_change("ajax_change_parent_event",this.value,"div_parent");'>-->
					<select name="opd_category_type" id="opd_category_type">	
						<option value="">Select category type </option>
						<?php  echo makeOptionCategoryType('',$posted["opd_category_type"])?>
				</select>
			</td>
			<td>&nbsp;</td>
		</tr>
		...
		<?php if(count($language_detail)>0)
				{
				foreach($language_detail as $val) 
					{
					
		?>
		<tr>
          <td>Category Name For <?php echo $val['s_language'] ?> *:</td>
          <td><input id="txt_category_name_<?php echo $val['id'];?>" name="txt_category_name_<?php echo $val['id'];?>" value="<?php echo $posted["txt_category_name_".$val['id']];?>" type="text" size="50" /></td>
          <td>&nbsp;</td>
        </tr>
		<?php } } ?>
