...
class Auto_alert_model extends MY_Model implements InfModel
{
    private $conf;
        ...
        try
        {
          ...
          if($rs->num_rows()>0)
          {
              foreach($rs->result() as $row)
              {
				  ...
				  $ret_[$i_cnt]["s_alert_for"]	=	stripslashes(htmlspecialchars_decode($row->s_alert_for)); 
                  $ret_[$i_cnt]["s_type"]		=	stripslashes(htmlspecialchars_decode($row->s_type)); 
				  $ret_[$i_cnt]["s_subject"]	=	stripslashes(htmlspecialchars_decode($row->s_subject)); 
				  $ret_[$i_cnt]["s_placeholder"]=	stripslashes(htmlspecialchars_decode($row->s_placeholder)); 
          ...
          if($rs->num_rows()>0)
          {
              foreach($rs->result() as $row)
              {
                  ...
                  $ret_["id"]=$row->i_id;////always integer
				   $ret_["s_alert_for"]		=	stripslashes(htmlspecialchars_decode($row->s_alert_for)); 
				  $ret_["s_type"]			=	stripslashes(htmlspecialchars_decode($row->s_type)); 
				  $ret_["s_subject"]		=	stripslashes(htmlspecialchars_decode($row->s_subject)); 
				  $ret_["s_placeholder"]	=	stripslashes(htmlspecialchars_decode($row->s_placeholder)); 
        ...
        try
        {
            ...
            if(!empty($info))
            {
                ...
                $this->db->query($s_qry,array(
												  ...
												  trim(htmlspecialchars($info["s_alert_for"], ENT_QUOTES, 'utf-8')),
												  trim(htmlspecialchars($info["s_subject"], ENT_QUOTES, 'utf-8')),
												  trim(htmlspecialchars($info["s_placeholder"], ENT_QUOTES, 'utf-8')),
    ...
    */
    public function log_info($attr)
    {
	 ...
	 public function fetch_forgot_password_content($title)
    {
        try
        {
          $ret_=array();
          ...
          $s_qry="Select * "
                ."From ".$this->tbl." AS c "
                ." Where c.s_alert_for=? AND c.i_status=1";
                
          $this->db->trans_begin();///new            
          $rs=$this->db->query($s_qry,array($title));
          if(is_array($rs->result()))
          {
              foreach($rs->result() as $row)
              {
                  ...
                  $ret_["s_placeholder"]=get_unformatted_string(htmlspecialchars_decode($row->s_placeholder,ENT_QUOTES)); 
                 
				 // $ret_["dt_created_on"]=date($this->conf["site_date_format"],strtotime($row->dt_created_on)); 
                  $ret_["i_is_active"]= intval($row->i_status); 
				  $ret_["s_is_active"]=(intval($row->i_status)==1?"Active":"Inactive");
                  $ret_["i_is_deleted"]=intval($row->i_is_deleted); 
                  $ret_["s_is_deleted"]=(intval($row->i_is_deleted)==0?"":"Removed");                     
