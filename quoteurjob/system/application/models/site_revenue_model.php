    ...
    */
    public function fetch_multi($s_where=null,$i_start=null,$i_limit=null)
    {
        try
        {
          	$ret_=array();
			/*$s_qry="SELECT
						YEAR(FROM_UNIXTIME( i_payment_date , '%Y-%m-%d' )) as year,
						MONTHNAME(FROM_UNIXTIME( i_payment_date , '%Y-%m-%d' )) AS month,
						SUM(d_pay_amount ) AS d_total_amount
					FROM {$this->tbl_pay}
					WHERE FROM_UNIXTIME( i_payment_date , '%Y-%m-%d' ) 
						BETWEEN DATE(NOW()) - INTERVAL (DAY(NOW()) - 1) DAY - INTERVAL 11 MONTH
						AND NOW()
						$s_where
					GROUP BY 
							YEAR(FROM_UNIXTIME( i_payment_date , '%Y-%m-%d' ) ), 
							MONTH(FROM_UNIXTIME( i_payment_date , '%Y-%m-%d' ) )
					ORDER BY
							 YEAR(FROM_UNIXTIME( i_payment_date , '%Y-%m-%d' ) ), 
							 MONTH(FROM_UNIXTIME( i_payment_date , '%Y-%m-%d' ) )"
