...
* @link InfModel.php 
* @link MY_Model.php
* @link controllers/content_article.php
* @link views/admin/content_article/
*/


class Content_article_model extends MY_Model implements InfModel
{
    private $conf;
    ...
    public function __construct()
    {
        ...
        {
			parent::__construct();
			$this->tbl 				= 	$this->db->CMSARTICLE;   
			$this->tbl_cms_master 	= 	$this->db->CMSMASTERTYPE;        
    ...
    */
    public function fetch_multi($s_where=null,$i_start=null,$i_limit=null)
    {
        ...
        {
          	$ret_=array();
			$s_qry="SELECT c.*, m.s_name FROM ".$this->tbl." c "
				." LEFT JOIN ".$this->tbl_cms_master. " m "
				." ON c.i_cms_type_id=m.i_id "
                .($s_where!=""?$s_where:"" )
          ...
          if($rs->num_rows()>0)
          {
              foreach($rs->result() as $row)
              {
				  ...
				  $ret_[$i_cnt]["i_cms_type_id"]	=	$row->i_cms_type_id;
				  $ret_[$i_cnt]["cms_type_name"]	=	stripslashes($row->s_name);
                  $ret_[$i_cnt]["s_title"]			=	strip_tags(
				  											stripslashes(htmlspecialchars_decode($row->s_title))
																  ); 
				  $s_desc 							= 	strip_tags(
				  											stripslashes(htmlspecialchars_decode($row->s_short_description))
																  );
        ...
        try
        {
          ...
          if($rs->num_rows()>0)
          {
              foreach($rs->result() as $row)
              {
                  ...
                  $ret_["i_cms_type_id"]		=	stripslashes($row->i_cms_type_id);
				  $ret_["s_name"]				=	stripslashes($row->s_name);
				  $ret_["s_title"]				=	stripslashes(htmlspecialchars_decode($row->s_title)); 
                  $ret_["s_short_description"]	=	stripslashes(htmlspecialchars_decode($row->s_short_description)); 
				  $ret_["s_long_description"]	=	stripslashes(htmlspecialchars_decode($row->s_long_description));
    ...
    public function add_info($info)
    {
        try
        {
            ...
            if(!empty($info))
            {
				...
				$s_qry.=" i_cms_type_id=? ";
                $s_qry.=", s_title=? ";
                $s_qry.=", s_short_description=? ";
				$s_qry.=", s_long_description=? ";
                $s_qry.=", i_status=? ";
                $s_qry.=", dt_entry_date=? ";
                
                $this->db->query($s_qry,array(addslashes(
													  intval(trim(decrypt($info["i_cms_type_id"])))),
													  trim(htmlspecialchars($info["s_title"], ENT_QUOTES, 'utf-8')),
													  trim(htmlspecialchars($info["s_short_description"], ENT_QUOTES, 'utf-8')),
													  trim(htmlspecialchars($info["s_long_description"], ENT_QUOTES, 'utf-8')),
