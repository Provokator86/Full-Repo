        ...
        try
        {
          ...
          if($rs->num_rows()>0)
          {
              foreach($rs->result() as $row)
              {
				  ...
				  $s_desc = strip_tags(stripslashes(htmlspecialchars_decode($row->s_body)));
        ...
        try
        {
          ...
          if($rs->num_rows()>0)
          {
              foreach($rs->result() as $row)
              {
                  ...
                  $ret_["s_content"]		=	stripslashes(htmlspecialchars_decode($row->s_body));  
				  $ret_["s_subject_french"]	=	stripslashes(htmlspecialchars_decode($row->s_subject_french)); 
				  $ret_["s_content_french"]	=	stripslashes(htmlspecialchars_decode($row->s_body_french)); 
        ...
        try
        {
          ...
          if($rs->num_rows()>0)
          {
              foreach($rs->result() as $row)
              {
                  ...
                  $ret_["s_subject"]		=	stripslashes(htmlspecialchars_decode($row->s_subject)); 
                  $ret_["s_content"]		=	stripslashes(htmlspecialchars_decode($row->s_body)); 
				  $ret_["s_subject_french"]	=	stripslashes(htmlspecialchars_decode($row->s_subject_french,ENT_QUOTES)); 
				  $ret_["s_content_french"]	=	stripslashes(htmlspecialchars_decode($row->s_body_french,ENT_QUOTES));                   
        ...
        try
        {
            ...
            if(!empty($info))
            {
                ...
                $this->db->query($s_qry,array(
												  ...
												  trim(htmlspecialchars($info["s_content_french"], ENT_QUOTES, 'utf-8')),
        ...
        try
        {
            ...
            if(!empty($info))
            {
                ...
                $this->db->query($s_qry,array(
													  trim(htmlspecialchars($info["s_subject"], ENT_QUOTES, 'utf-8')),
                                                      trim(htmlspecialchars($info["s_content"], ENT_QUOTES, 'utf-8')),
													  trim(htmlspecialchars($info["s_subject_french"], ENT_QUOTES, 'utf-8')),
													  trim(htmlspecialchars($info["s_content_french"], ENT_QUOTES, 'utf-8')),
                                                      intval($info["dt_entry_date"]),
													  intval($info["i_status"]),
													  intval($id)
