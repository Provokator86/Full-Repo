...
class Commission_waiver_model extends MY_Model implements InfModel
{
    private $conf;
        ...
        try
        {
          ...
          if($rs->num_rows()>0)
          {
              foreach($rs->result() as $row)
              {
                  ...
                  $ret_[$i_cnt]["i_waiver_commission"]=stripslashes(htmlspecialchars_decode($row->i_waiver_commission)); 
        ...
        try
        {
          ...
          if($rs->num_rows()>0)
          {
              foreach($rs->result() as $row)
              {
                  ...
                  $ret_["i_waiver_commission"]=stripslashes(htmlspecialchars_decode($row->i_waiver_commission));
    ...
    public function add_info($info)
    {
        try
        {
            ...
            if(!empty($info))
            {
                ...
                $s_qry.=" i_waiver_commission=? ";
                $s_qry.=", i_is_active=? ";
                $s_qry.=", dt_entry_date=? ";
				
                $this->db->query($s_qry,array(
												  trim(htmlspecialchars($info["i_waiver_commission"], ENT_QUOTES, 'utf-8')),
    ...
    public function edit_info($info,$i_id)
    {
        try
        {
            ...
            if(!empty($info))
            {
				...
				//$s_qry.=", i_lang_id=? ";
                $s_qry.=", s_content=? ";
				$s_qry.=", s_person_address=? ";
                $s_qry.=", s_person_phone=? ";
				$s_qry.=", s_person_email=? ";
                $s_qry.=", s_person_image=? ";
                $s_qry.=", i_status=? ";
                $s_qry.=", dt_entry_date=? ";
                $s_qry.=" Where i_id=? ";
                
                $this->db->query($s_qry,array(
												  trim(htmlspecialchars($info["s_person_name"], ENT_QUOTES, 'utf-8')),
												  //intval(decrypt($info["i_lang_id"])),
												  trim(htmlspecialchars($info["s_content"], ENT_QUOTES, 'utf-8')),
												  trim(htmlspecialchars($info["s_person_address"], ENT_QUOTES, 'utf-8')),
												  trim(htmlspecialchars($info["s_person_phone"], ENT_QUOTES, 'utf-8')),
												  trim(htmlspecialchars($info["s_person_email"], ENT_QUOTES, 'utf-8')),
												  trim(htmlspecialchars($info["s_person_image"], ENT_QUOTES, 'utf-8')),
												  intval($info["i_status"]),
												  intval($info["dt_entry_date"]),
												  intval($i_id)
