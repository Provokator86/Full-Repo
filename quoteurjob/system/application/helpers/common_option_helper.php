...
function makeOptionUserByType($mix_where = '', $s_id = '',$i_role =1)
{
    try
	{
		...
		$res = $CI->db->query("select id, s_username from {$CI->db->USERMANAGE} {$cond} ORDER BY s_username ASC");	
		$mix_value = $res->result_array();
		$s_option = '';
		if($mix_value)
		{
...
/* date 9 sep 2011
*  for city drop down in quoteforjob
**
****/
...
function makeOptionCategory($mix_where = '', $s_id = '')
{
	...
	{		
		...
		$res = $CI->db->query("select c.id, cc.s_name AS s_category_name, c.i_status from {$CI->db->CATEGORY} c LEFT JOIN {$CI->db->CATEGORYCHILD} cc ON c.id = cc.i_cat_id {$cond} ORDER BY s_name");	
		$mix_value = $res->result_array();
