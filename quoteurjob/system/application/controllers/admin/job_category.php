    ...
    */
    public function ajax_checkduplicate()
    {
        try
        {
			...
			$posted["french_value"]= htmlspecialchars(trim($this->input->post("h_french_value")));
            ...
            if($posted["duplicate_value"]!="")
            {
				...
				$qry1 =" Where ".(intval($posted["id"])>0 ? " c.id!=".intval($posted["id"])." And " : "" )
                   ." cc.s_name='".$posted["french_value"]."' AND c.s_category_type = '".$this->type."' AND cc.i_lang_id=2 ";
				...
				$info1= $this->mod_rect->fetch_multi($qry1,$start=0,$limit=1); 
				...
				{
					echo "Duplicate category exists";
				}
                else
                {
