    ...
    public function modify_information($type=NULL,$i_id=0)
    {
        ...
        try
        {
            ...
            if($_POST)
            {
				...
				$posted["txt_subject"]			= trim($this->input->post("txt_subject"));
                $posted["txt_content"]			= trim($this->input->post("txt_content"));
				$posted["txt_subject_french"]	= trim($this->input->post("txt_subject_french"));
				$posted["txt_content_french"]	= trim($this->input->post("txt_content_french"));
				$i_active_val 					= trim($this->input->post("i_automail_is_active"));
                $posted["i_automail_is_active"]	= ($i_active_val==1)?$i_active_val:2;
                ...
                $this->form_validation->set_rules('txt_subject', 'automail subject', 'required');
                $this->form_validation->set_rules('txt_content', 'automail description in english', 'required');
				$this->form_validation->set_rules('txt_subject_french', 'automail subject in french', 'required');
				$this->form_validation->set_rules('txt_content_french', 'automail description in french', 'required');
                ...
                else///validated, now save into DB
                {
					...
					$info["s_subject"]			=	$posted["txt_subject"];
                    $info["s_content"]			=	$posted["txt_content"];
					$info["s_subject_french"]	=	$posted["txt_subject_french"];
					$info["s_content_french"]	=	$posted["txt_content_french"];
					$info["i_status"]			=	$posted["i_automail_is_active"];
                    $info["dt_entry_date"]		=	strtotime(date("Y-m-d H:i:s"));
            ...
            {	
				...
				$posted["txt_subject_french"]   = trim($info["s_subject_french"]);
				$posted["txt_content_french"]	= trim($info["s_content_french"]);
    ...
    */
    public function show_param_list($type=NULL,$start=NULL,$limit=NULL)
    {
        try
        {
            ...
            $s_where=" WHERE n.s_type='".decrypt($type)."'";
            if($s_search=="basic")
            {
