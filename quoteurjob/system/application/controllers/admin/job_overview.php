    ...
    public function show_detail($i_id = 0)
    {
        try
        {
			//echo $this->session->userdata("i_buyer_id");
				...
				unset($s_where);
				$this->data['d_high_quote'] = $this->job_model->get_quote_high_low($i_id).' '.$this->config->item("default_currency");
				$this->data['d_low_quote'] = $this->job_model->get_quote_high_low($i_id,2).' '.$this->config->item("default_currency");
