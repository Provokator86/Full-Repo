...
class Dashboard extends My_Controller implements InfController
{
    ...
    public function index()
    {
        try
        {
			...
			$this->data['i_no_buyer']=$this->mod_rect->gettotal_user_info(" Where n.i_role=1 And n.i_is_active=1 ");
			$this->data['i_no_tradesman']=$this->mod_rect->gettotal_user_info(" Where n.i_role=2 And n.i_is_active=1 ");
			...
			//$s_where = " WHERE n.i_created_date >={$today} And n.i_created_date<{$tomorrow} ";
			...
			$s_where = " WHERE n.i_is_deleted!=1 AND( n.i_status=4 || n.i_status=5 || n.i_status=6 || n.i_status=9) AND (FROM_UNIXTIME( jph.i_payment_date , '%Y-%m-%d' )= CURDATE() || FROM_UNIXTIME( wp.i_created_on , '%Y-%m-%d' )= CURDATE()) AND cat_c.i_lang_id =".$this->i_default_language; 
		    $this->data['i_accept_jobs'] = $this->job_model->gettotal_dashboard_accept_job_info($s_where);				
			
			$s_where = " WHERE n.i_status=4 AND n.i_is_deleted!=1 AND cat_c.i_lang_id =".$this->i_default_language; 
			...
			$s_where = " WHERE n.i_status=6 AND n.i_is_deleted!=1 AND FROM_UNIXTIME( n.i_completed_date, '%Y-%m-%d' )= CURDATE() AND cat_c.i_lang_id =".$this->i_default_language; 
		    $this->data['completed_jobs'] = $this->job_model->fetch_multi_completed($s_where,0,5);
			
			//print_r($this->data['completed_jobs']);
			
			$s_where = " WHERE n.i_status=0 AND n.i_is_deleted!=1 AND cat_c.i_lang_id =".$this->i_default_language; 
		    $this->data['posted_jobs'] = $this->job_model->fetch_multi_completed($s_where,0,5);
			//pr($this->data['posted_jobs']);
			$s_where = " WHERE n.i_status=1 AND n.i_is_deleted!=1 AND cat_c.i_lang_id =".$this->i_default_language; 
		    $this->data['active_jobs'] = $this->job_model->fetch_multi_completed($s_where,0,5);
			
			//$s_where = " WHERE n.i_date BETWEEN {$today} And {$tomorrow} ";
