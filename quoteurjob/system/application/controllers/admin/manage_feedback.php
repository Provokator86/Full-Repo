    ...
    public function show_list($order_name='',$order_by='asc',$start=NULL,$limit=NULL)
    {
        try
        {
           ...
           // $s_cat=($this->input->post("h_search")?$this->input->post("s_cat"):$this->session->userdata("s_cat")); 
            ...
            $s_where=" WHERE j.i_is_deleted=0 AND cat_c.i_lang_id=".$this->i_admin_language;
            if($s_search=="basic")
            {
			...
			$arr_sort = array(0=>'s_title',2=>'i_buyer_user_id',6=>'i_rating');   
			// echo $order_name.'---';
            ...
            if($_POST)
            {
                ...
                else///validated, now save into DB
                {
                    ...
                    if($i_aff)////saved successfully
                    {
						...
						$s_where = " WHERE i_receiver_user_id ={$feed_back_deatils['i_receiver_user_id']} AND n.i_status=1 AND cat_c.i_lang_id=".$this->i_admin_language; 
						$tot_accepted_feedback = $this->mod_rect->gettotal_info($s_where);
						
						$s_where = " WHERE i_receiver_user_id ={$feed_back_deatils['i_receiver_user_id']} AND n.i_status!=0 AND cat_c.i_lang_id=".$this->i_admin_language; 				
