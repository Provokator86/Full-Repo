    ...
    public function add_information()           
    {
		...
		try
        {
            ...
            if($_POST)
            {
				...
				$is_data_exist = $this->mod_rect->fetch_content_by_ajax_call(decrypt($posted['opt_language']),decrypt($posted["opt_cat"]),$posted["txt_faq_question"]);
    ...
    */
    public function ajax_checkduplicate()
    {
        try
        {
           ...
           //$posted["cat_id"] = decrypt($this->input->post("h_lang_id"));
			
			
            if($posted["duplicate_value"]!="")
            {
                $qry=" Where ".(intval($posted["id"])>0 ? " n.i_id!=".intval($posted["id"])." And " : "" )
                    ." n.s_question='".$posted["duplicate_value"]."' And n.i_user_type=2 ";
