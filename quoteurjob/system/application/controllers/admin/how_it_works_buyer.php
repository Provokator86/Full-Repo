...
class How_it_works_buyer extends My_Controller implements InfController
{
    ...
    public function __construct()
    {
        ...
        try
        {
          ...
          $this->load->model("how_it_works_buyer_model","mod_rect");
    ...
    public function show_list($start=NULL,$limit=NULL)
    {
        try
        {
            ...
            for($i=0; $i<$table_view["total_rows"]; $i++)
            {
                ...
                $table_view["tablerows"][$i][$i_col++]	=$info[$i]["s_basic_content"];
    ...
    public function add_information()           
    {
		...
		try
        {
            ...
            if($_POST)
            {
				...
				$posted["txt_post_job_content"]					=	trim($this->input->post("txt_post_job_content"));
				$posted["txt_get_quote_from_tradesman_content"]	=	trim($this->input->post("txt_get_quote_from_tradesman_content"));
				$posted["txt_hire_best_tradesman_content"]		=	trim($this->input->post("txt_hire_best_tradesman_content"));
				$posted["txt_leave_feedback_content"]			=	trim($this->input->post("txt_leave_feedback_content"));
				...
				$this->form_validation->set_rules('txt_post_job_content', 'post job', 'required');
				$this->form_validation->set_rules('txt_get_quote_from_tradesman_content', 'quote from tradesman', 'required');
				$this->form_validation->set_rules('txt_hire_best_tradesman_content', 'hire tradesman content', 'required');
				$this->form_validation->set_rules('txt_leave_feedback_content', 'leave feedback', 'required');
                ...
                else///validated, now save into DB
                {
					...
					$info["s_post_job_content"]					=	$posted["txt_post_job_content"];
					$info["s_get_quote_from_tradesman_content"]	=	$posted["txt_get_quote_from_tradesman_content"];
					$info["s_hire_best_tradesman_content"]		=	$posted["txt_hire_best_tradesman_content"];
					$info["s_leave_feeedback_content"]			=	$posted["txt_leave_feedback_content"];
            ...
            ////////////end Submitted Form///////////
            $this->render("how_it_works_buyer/add-edit");
        }
        catch(Exception $err_obj)
        {
            show_error($err_obj->getMessage());
    ...
    public function modify_information($i_id=0)
    {
        ...
        try
        {
            ...
            else
            {
				...
				$posted["txt_post_job_content"]					= trim($info["s_post_job_content"]);
				$posted["txt_get_quote_from_tradesman_content"]	= trim($info["s_get_quote_from_tradesman_content"]);
				$posted["txt_hire_best_tradesman_content"]		= trim($info["s_hire_best_tradesman_content"]);
				$posted["txt_leave_feedback_content"]			= trim($info["s_leave_feeedback_content"]);
    ...
    */
    public function show_detail($i_id=0)
    {
        try
        {
            ...
            $this->render("how_it_works_buyer/show_detail",TRUE);
            unset($i_id);
        }
        catch(Exception $err_obj)
        {
