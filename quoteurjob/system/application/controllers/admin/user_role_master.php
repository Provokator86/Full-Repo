...
class User_role_master extends My_Controller implements InfController
{
    ...
    public function __construct()
    {
        ...
        {
          parent::__construct();
          $this->data['title']="Master Role Management";////Browser Title

          ////////Define Errors Here//////
          $this->cls_msg = array();
          $this->cls_msg["no_result"]="No information found about user master role.";
          $this->cls_msg["save_err"]="Information about user master role failed to save.";
          $this->cls_msg["save_succ"]="Information about user master role saved successfully.";
          $this->cls_msg["delete_err"]="Information about user master role failed to remove.";
          $this->cls_msg["delete_succ"]="Information about user master role removed successfully.";
          ////////end Define Errors Here//////
          $this->pathtoclass = admin_base_url().$this->router->fetch_class()."/";//for redirecting from this class
		  
		  //////// loading default model here //////////////
          $this->load->model("user_role_master_model","mod_rect");
    ...
    */
    public function show_list($start=NULL,$limit=NULL)
    {
        try
        {
            $this->data['heading']="User Master Role";////Package Name[@package] Panel Heading

            ///////////generating search query///////
			
            ////////Getting Posted or session values for search///
            $s_search=(isset($_POST["h_search"])?$this->input->post("h_search"):$this->session->userdata("h_search"));
            $s_state_title=($this->input->post("h_search")?$this->input->post("txt_role_title"):$this->session->userdata("txt_role_title")); 
            ...
            if($s_search=="basic")
            {
                $s_where=" WHERE n.s_role LIKE '%".get_formatted_string($s_state_title)."%' ";
                /////Storing search values into session///
                $this->session->set_userdata("txt_role_title",$s_state_title);
                $this->session->set_userdata("h_search",$s_search);
                
                $this->data["h_search"]=$s_search;
                $this->data["txt_role_title"]=$s_state_title;
            ...
            for($i=0; $i<$table_view["total_rows"]; $i++)
            {
                ...
                $table_view["tablerows"][$i][$i_col++]	=$info[$i]["s_role"];
    ...
    */
    public function add_information()           
    {
        ...
        {
            $this->data['title']="Master Role Management";////Browser Title
            $this->data['heading']="Add Master Role";
            ...
            {
				$posted=array();
                $posted["txt_role_title"]= trim($this->input->post("txt_role_title"));
				$i_active_val = trim($this->input->post("i_role_is_active"));
                $posted["i_role_is_active"]= ($i_active_val==1)?$i_active_val:2;
                $posted["h_mode"]= $this->data['mode'];
                $posted["h_id"]= "";
				
				
               
                $this->form_validation->set_rules('txt_role_title', 'role name', 'required');
                ...
                {
                    $info=array();
                    $info["s_role"]			=	$posted["txt_role_title"];
                    $info["i_status"]		=	$posted["i_role_is_active"];
            ...
            ////////////end Submitted Form///////////
            $this->render("user_role_master/add-edit");
        }
        catch(Exception $err_obj)
        {
            show_error($err_obj->getMessage());
    ...
    */      
    public function modify_information($i_id=0)
    {
        ...
        try
        {
            $this->data['title']="Edit Master Role Details";////Browser Title
            $this->data['heading']="Edit Master Role";
            $this->data['pathtoclass']=$this->pathtoclass;
            $this->data['mode']="edit";
            ...
            if($_POST)
            {
                ...
                $this->form_validation->set_rules('txt_role_title', 'state name', 'required');
                ...
                {
                    $info	=	array();
					$info["state"]			=	$posted["txt_role_title"];
                    $info["i_status"]		=	$posted["i_role_is_active"];
                    $info["dt_entry_date"]	=	strtotime(date("Y-m-d H:i:s"));
            ...
            else
            {
                ...
                $posted=array();
				$posted["txt_role_title"]		= trim($info["s_role"]);				
				$posted["dt_created_on"]		= trim($info["dt_created_on"]);
				$posted["i_role_is_active"]		= trim($info["i_is_active"]);
				$posted["h_id"]= $i_id;
    ...
    */
    public function show_detail($i_id=0)
    {
        try
        {
            ...
            $this->render("user_role_master/show_detail",TRUE);
            unset($i_id);
        }
        catch(Exception $err_obj)
        {
        ...
        try
        {
            ...
            if($posted["duplicate_value"]!="")
            {
                $qry=" Where ".(intval($posted["id"])>0 ? " n.i_id!=".intval($posted["id"])." And " : "" )
                    ." n.s_role='".$posted["duplicate_value"]."'";
