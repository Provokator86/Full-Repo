...
class Recommend extends My_Controller
{
    ...
    public function __construct()
    {
        ...
        { 
		  ...
		  $this->cls_msg=array();
		  $this->cls_msg["save_recommend"]=t("recommend(s) send successfully.");
			 ...
			 if($i_nwid)////saved successfully
				{
					...
					if($duplicate)
					{
						...
						if(count($ar_recom)>0)
						{
							...
							$msg[] = $s_email.t(" ID(s) are already recomended by you.");
						...
						if(count($ar_ext)>0)
						{
							...
							$msg[] = $s_email.t(" ID(s) are already registered.");
							//$this->session->set_userdata(array('message'=>$msg,'message_type'=>'err'));
	...
	public function tradesman_recommend()
    {
        try
        {
		 ...
		 $this->data['i_job_invitation'] = $this->job_model->gettotal_job_invitation($s_where);
		  ...
		  /** Feed back count **/
		  $this->load->model("Manage_feedback_model","mod_feed");
		   $s_where = " WHERE n.i_receiver_user_id = ".$this->user_id."  AND cat_c.i_lang_id =".$this->i_default_language; 
