...
class Find_tradesman extends My_Controller  
{
    ...
    public function __construct()
    {
        ...
        { 
		  ...
		  $this->cls_msg["job_find_err"] = t("Tradesman not found.");
		  $this->cls_msg["tradesman_tradesman_post_err"] = t('Only Buyer can post job.');
    ...
    public function index($i_category_id='')
    {
        ...
        {
			$this->i_menu_id = 3;		
			$this->data['breadcrumb'] = array(t('Find Tradesman')=>'');	
	...
	function pagination_ajax($start=0,$param=0) {
		...
		if($total_rows > 0)
		{
			...
			{
				$s_where = " WHERE n.i_receiver_user_id = ".$tradesman_list[$i]['id']." AND n.i_status !=0 AND cat_c.i_lang_id =".$this->i_default_language; 
	...
	public function tradesman_profile($id)
	{
		try
		{
			...
			$album_list = $this->mod_trade->get_album_images($s_where);
			...
			$s_where = " WHERE n.i_receiver_user_id = ".decrypt($id)." AND n.i_status != 0 AND cat_c.i_lang_id =".$this->i_default_language; 
