    ...
    public function show_list($start = NULL, $limit = NULL)
    {
        try 
        {
            ...
            for ($i = 0; $i < $table_view["total_rows"]; $i++) {
                ...
                $action = '<a id="edit_row_' . $info[$i]["id"] . '" href="javascript:void(0);" class="btn btn-sm btn-primary" ><i class="icon-edit icon-white"></i> Edit</a>';



                $action .=' <a class="btn btn-sm btn-success" id="save_row_' . $info[$i]["id"] . '" href="javascript:void(0);" >
                <i class="icon-save icon-white"></i> Save</a>';


                $action .='&nbsp;&nbsp;<a class="btn btn-sm btn-warning"  id="delete_row_' . $info[$i]["id"] . '" href="javascript:void(0);" >
                <i class="icon-trash icon-white"></i> Delete</a>';

                $action .='&nbsp;&nbsp;<a class="btn btn-sm btn-info"  id="change_row_' . $info[$i]["id"] . '" href="javascript:void(0);" >
                <i class="icon-refresh icon-white"></i> Status</a>';
                if ($info[$i]["i_parent_id"] == 0) {
                    $action .='&nbsp;&nbsp;<a class="btn btn-sm btn-danger"  id="go_row_' . $info[$i]["id"] . '" href="' . admin_base_url() . 'menu_setting/sub_menu_list/' . encrypt($info[$i]["id"]) . '" >
    ...
    public function menu_permission($enc_menu_id = '') 
    {
        try 
        {
            ...
            if ($_POST) 
			{
                ...
                $s_actions = implode('||',array_unique($s_actions));
                //pr($data,1);
                ...
                if (!empty($data)) {
                    ...
                    $info['i_user_type'] = $this->config->item('dev_user_type');
                    foreach ($data as $val) 
                    {
			...
			{
                $s_where = " WHERE i_menu_id=". decrypt($enc_menu_id) . " AND (i_user_type=".$this->config->item('dev_user_type')." OR i_user_type=-99)";
