...
{
    protected $field_type, $required, $field_label, $reference_field, $reference_table, $form_field_type = array(), $tbl_prefix = 'mp_', $enum_drop_down, $ref_tb_list = '';
    ...
    public function index()
    {
        ...
        $tables_tmp = $this->db->list_tables();
        for($i = 0; $i< count($tables_tmp); $i++)
        {
            if(preg_match('/_copy/', $tables_tmp[$i]) == false)
                $tables[] = $tables_tmp[$i];
        }
        unset($tables_tmp);
        
        // below two loops are extra for view and old tables
        $tables_tmp = $tables;unset($tables);
        for($i = 0; $i< count($tables_tmp); $i++)
        {
            if(preg_match('/vw_/', $tables_tmp[$i]) == false)
                $tables[] = $tables_tmp[$i];
        }
        unset($tables_tmp);
        ...
        for($i = 0; $i< count($tables_tmp); $i++)
        {
            if(preg_match('/_old/', $tables_tmp[$i]) == false)
                $tables[] = $tables_tmp[$i];
        }
        unset($tables_tmp);
        // end below two loops are extra for view and old tables
    ...
    // File info
    protected function generate_file_info($file_name, $author_name = 'SWI Dev')
    {
    ...
    protected function get_field_type($name, $type)
    {
        ...
        if(preg_match('/^i_/', $name) && preg_match('/_id$/', $name) && $name != 'i_id')
        {
            ...
            $this->reference_table = $this->tbl_prefix.implode('_',$tmp);
            unset($tmp);
        }
...
##--------------------------------------------------------------------------------------##
            ...
            if($prev == 0)
            {
                $this->db->insert($this->tbl_prefix.'menu', array('s_name' => $this->mvc_title, 's_link' => $this->file_name.'/', 'i_parent_id' => 1, 'i_main_id' => 1, 's_action_permit' => 'View List||Add||Edit||Delete','en_s_name'=>$this->mvc_title)); //||View Detail
