...
class Category extends MY_Controller 
{
	...
	public function __construct(){
		parent::__construct();
		$this->data["title"] = addslashes(t('Manage Category/Sub Category'));//Browser Title 
		$this->pathtoclass = admin_base_url().$this->router->fetch_class()."/";
        //$this->tbl = 'mp_category';// Default Table
		$this->tbl = $this->db->CATEGORY;// Default Table
        $this->data['BREADCRUMB'] = array(addslashes(t('Manage Category/Sub Category')));
    ...
    */
    public function show_list($order_by = '', $sort_type = 'asc',$start = NULL, $limit = NULL)
    {
        try
        {
            $this->data['heading'] = addslashes(t("Manage Category/Sub Category")); //Package Name[@package] Panel Heading            
    ...
    */  
    ...
    /* get sub category on change of parent category 
    *  see @manage_listing->add-edit.tpl.php
    * date: july 7, 2015
    */
    function ajax_get_all_category_AJAX()
    {
        $parentID = $this->input->post('parentID');
        $parent_cat_option = $this->acs_model->get_category_selectlist($parentID);
                                   
        echo  '<option value="">Select Category</option>'.$parent_cat_option ;
    }
    
    function ajax_get_all_sub_category_AJAX()
    {
        $parentID = $this->input->post('parentID');
        $dontshowAll = $this->input->post('dontshowAll');
        $parent_cat_option = $this->acs_model->get_category_selectlist($parentID);
        if($dontshowAll)                         
            echo  $parent_cat_option ;
        else
