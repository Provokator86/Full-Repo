...
class PHPGraphLib {
	...
	function setupData() {
		...
		if ($unit_width < 1) {	
			...
			$this->bar_width = 1 * $unit_width; //JS
			$this->bar_width -=22; //JS
	...
	function generateBars() {
		...
		$dataset_offset = $this->bar_width; //JS
		foreach ($this->data_array as $data_set_num => $data_set) {
			...
			foreach ($data_set as $key => $item) {
				...
				$xStart += $this->bar_width + (2*$this->space_width); //JS
			}
		}
	}
	...
	function setDataFormat($format) {
		...
		switch ($format) {
			...
			case 'degrees': 
				...
				break;
			case 'dollar': 
				$this->data_format_array[] = 'formatDataAsDollar'; 
				$this->data_additional_length++; 
				break;
			default: 
	...
	// Added by JS
	function formatDataAsDollar($input) {
		if(intval($input) == 0) return $input;
		else return '$'.number_format($input, 0, '.', ',');
	}
	...
	function allocateColors() {
		...
		//$this->line_color_default = imagecolorallocate($this->image, 100, 100, 100); // JS
		$this->line_color_default = imagecolorallocate($this->image, 0, 0, 0);
	...
	function generateLegend() {
		...
		// JS
		$new_multi_bar_colors = array_reverse($this->multi_bar_colors);
		// END JS
		...
		for($i = 0; $i < $this->data_set_count; $i++) {
			...
			if ($this->bool_bars) {
				if ($this->bool_gradient) {
					...
					$color = $new_multi_bar_colors[$this->data_set_count - $i - 1];
				}
			}
