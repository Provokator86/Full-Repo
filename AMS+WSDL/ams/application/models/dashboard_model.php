...
class Dashboard_model extends MY_Model implements InfModel
{
	...
	private $tbl_user, $tbl_batch, $tbl_batch_status_history, $tbl_batch_files_dwn;
    ...
    public function __construct()
    {
        try
        {
		  ...
		  $this->tbl_batch 					= $this->db->BATCH_MASTER;         
		  $this->tbl_batch_status_history 	= $this->db->BATCH_STATUS_HISTORY;         
		  $this->tbl_batch_files_dwn 		= $this->db->BATCH_ASCII_FILE;         
    ...
    */
	...
	public function gettotal_batch_count($s_where=null)
    {
        try
        {
          $ret_=0;
          $s_qry="Select count(n.i_id) as i_total "
		  		  ."From ".$this->tbl_batch." n "
				  .($s_where!=""?$s_where:"" );
          $rs=$this->db->query($s_qry);
          $i_cnt=0;
          if($rs->num_rows()>0)
	...
	public function file_downloaded_count($s_where=null)
    {
        try
        {
          $ret_=0;
          $s_qry="Select count(n.i_id) as i_total "
		  		  ."From ".$this->tbl_batch_files_dwn." n "
				  .($s_where!=""?$s_where:"" );
          $rs=$this->db->query($s_qry);
          $i_cnt=0;
          if($rs->num_rows()>0)
