...
{
    private $conf;
    public $tbl_state, $tbl_city, $tbl_cat;
	...
	public function add_data($table, $info)
    {
        try
        {
            if(!empty($info))
            {
				$this->db->simple_query('SET sql-mode=""'); // set for lower version <5.5 in 1099.codeuridea.net server for advanced micro system
			    $s_qry = $this->db->insert_string($table, $info);
			    //$this->db->simple_query($s_qry);
			    //echo $s_qry;
	...
	*/	
	function performed_formatted_join($table_details  = array(), $where = '')
	{
     ...
     /* 7 July 2015
    * recursive function to create category dropbox
    */    
    function get_category_selectlist($parent_id='')
    {
        try
        {
          $ret_=array();
          ////Using Prepared Statement///
          $s_qry="SELECT i_id,s_category,i_parent_id "
                ."FROM ".$this->tbl_cat." AS n "
                ." WHERE n.e_deleted ='No' AND n.i_status = 1 AND n.i_parent_id=? ORDER BY n.s_category";
                
          $res=$this->db->query($s_qry,array(intval($parent_id))); 
    ...
    function get_payee_selectlist($payer_id='')
    {
        try
        {
          $ret_=array();
          ...
          $s_qry="SELECT i_id, s_first_payee_name_line, s_last_payee_name_line "
                ."FROM ".$this->db->PAYEE_INFO." "
                ." WHERE i_status = 1 AND i_payer_id=? ORDER BY s_first_payee_name_line";
                
          $res=$this->db->query($s_qry,array(intval($payer_id))); 
          $mix_value = $res->result_array();
          if($mix_value)
            {
                foreach ($mix_value as $val)
    ...
    */    
    function get_state_selectlist($country_id='')
    {
        try
        {
          $ret_=array();
          ////Using Prepared Statement///
          $s_qry="SELECT i_id,name,i_country_id "
                ."FROM ".$this->tbl_state." AS n "
                ." WHERE n.i_is_deleted = 0 AND n.i_status = 1 AND n.i_country_id=? ORDER BY n.name";
                
          $res=$this->db->query($s_qry,array(intval($country_id))); 
          $mix_value = $res->result_array();
          if($mix_value)
            {
                foreach ($mix_value as $val)
    ...
    */    
    function get_city_selectlist($state_id = '', $s_id = '')
    {
        try
        {
          $ret_=array();
          ////Using Prepared Statement///
          $s_qry="SELECT i_id,name,i_state_id, i_country_id "
                ."FROM ".$this->tbl_city." AS n "
                ." WHERE n.i_state_id=".$state_id." ORDER BY n.name";
                
          $res=$this->db->query($s_qry,array(intval($state_id))); 
          $mix_value = $res->result_array();
          if($mix_value)
            {
                foreach ($mix_value as $val)
    ...
    function get_city_on_country($country_id='')
    {
        try
        {
          $ret_=array();
          ...
          $s_qry="SELECT i_id,name,i_state_id, i_country_id "
                ."FROM ".$this->tbl_city." AS n "
                ." WHERE n.i_country_id=".$country_id." ORDER BY n.name";
