...
class Menu_model extends MY_Model{
    ...
    private $tbl_permit, $tbl_user_menu;
    ...
    public function fetch_menu_permission_of_user_type($s_where = null, $i_user_type_id) 
    {
        try 
        {
            ...
            $s_qry = "SELECT count(*) AS i_total_controls
                    ...
                    . ($s_where != "" ? $s_where : "" ) 
                    . " And (i_user_type={$i_user_type_id} OR s_action='Default') " 
                    . ' ORDER BY i_id ASC ' 
                    . (is_numeric($i_start) && is_numeric($i_limit) ? " Limit " . intval($i_start) . "," . intval($i_limit) : "" );
    ...
    public function fetch_access_control($i_user_type = 0) 
    {
        ...
        $s_qry = "SELECT m1.i_id first_label_id,m1.s_name first_label_menu,m2.i_id second_label_id,m2.s_name second_label_menu,m2.s_action_permit,mp2.i_id i_menu_permit_id, mp2.actions
					FROM  ( 
							SELECT *
							FROM " . $this->tbl . "
							WHERE i_parent_id = 0 ORDER BY i_display_order 
					)m1
					...
					LEFT JOIN ( 
								...
								AND mp.i_menu_id <>0
								GROUP BY (mp.i_menu_id)
                    )mp2 ON m2.i_id = mp2.i_menu_id ORDER BY m2.i_display_order";
    ...
    */
    ...
    // Fetch all set of menu corresponding to action set and menu id 
    // Added by JS on 04 June 2015
    public function get_all_menu_set($menu_id, $action_list)
    {
        $rs = $this->db->select('GROUP_CONCAT(s_link) AS menu_list')
                       ->where('i_menu_id = '.$menu_id.' AND s_action IN('.$action_list.') AND i_user_type = '.$this->config->item('dev_user_type'))
                       ->get($this->tbl_permit)
                       ->result_array();
        unset($menu_id, $action_list);
        return $rs[0]['menu_list'];
    }
    
    // Fetch customize access control
    // Added by JS on 04 June 2015
    public function fetch_customize_access_control($user_id, $user_type_id)
    {
        $sql = "SELECT REPLACE(TRIM(BOTH \"'\" FROM um.s_set_of_action), \"','\", \"##\") AS actions, 
                m.i_id AS second_label_id, m.s_name as second_label_menu, m.s_action_permit, fm.s_name AS first_label_menu,
                fm.i_id AS first_label_id 
                FROM {$this->tbl_user_menu} AS um
                LEFT JOIN {$this->tbl} AS m ON `m`.`i_id` = `um`.`i_menu_id` 
                LEFT JOIN {$this->tbl} AS fm ON `fm`.`i_id` = `m`.`i_parent_id` WHERE `um`.`i_user_id` = {$user_id}";
        return $this->db->query($sql)->result_array();     
    }
    
    # Custome menu section 
    // Fetch custome menu for an user
    // Added by JS on 08 June 2015
    public function fetch_custom_menus_navigation($user_id, $user_type_id = 1)
    {
        $ret_ = $tmp = array();
        if($user_id > 0)
        {
            $sql = "SELECT n.*, 
                        (
                            SELECT count(i_id) 
                                FROM {$this->tbl_permit} 
                                WHERE i_menu_id = n.i_id AND (i_user_type = {$user_type_id} OR s_action = 'Default') 
                        ) AS i_total_controls
                    FROM {$this->tbl} n  
                    WHERE n.i_parent_id = 0  AND n.i_id IN(
                        SELECT DISTINCT i_parent_id 
                        FROM {$this->tbl_user_menu} 
                        WHERE i_user_id = {$user_id} AND s_set_of_action != ''
                    )
                    ORDER BY n.i_display_order ASC";
            ...
            for($i = 0; $i < count($tmp); $i++)
            {
                $ret_[$i] = array(
                    ...
                    's_name' => $tmp[$i]['s_name'],
                    's_link' => $tmp[$i]['s_link'],
                    'i_total_controls' => $tmp[$i]['i_total_controls'],
                    's_icon_class' => $tmp[$i]['s_icon_class'],
                    't_name' => $tmp[$i][db_field_wrtcl('s_name')],
                ); 
            }
        }
        unset($tmp, $user_id, $user_type_id);
        return $ret_;
    }
    
    public function fetch_custome_child_menus_navigation($conf = array())
    {
        $ret_ = $tmp = array();
        if($conf['user_id'] > 0)
        {
            $sql = "SELECT um.i_id, um.i_menu_id,um.i_parent_id, um.i_user_id, mm.s_name, mm.s_link, mm.s_icon_class, 
                    REPLACE(TRIM(BOTH \"'\" FROM um.s_set_of_action), \"','\", \"##\") AS s_set_of_action 
                    FROM {$this->tbl_user_menu} AS um
                    LEFT JOIN {$this->tbl} AS mm ON mm.i_id = um.i_menu_id
                    WHERE um.i_parent_id = {$conf['parent_id']} AND i_user_id={$conf['user_id']} AND um.s_set_of_action != ''
                    ORDER BY mm.i_display_order ASC ";
            $tmp = $this->db->query($sql)->result_array();
            for($i = 0; $i < count($tmp); $i++)
            {
                $ret_[$i] = array(
                    ...
                    's_name' => $tmp[$i]['s_name'],
                    's_link' => $tmp[$i]['s_link'],
                    's_set_of_action' => $tmp[$i]['s_set_of_action'],
                    'i_total_controls' => $tmp[$i]['i_total_controls'],
                    's_icon_class' => $tmp[$i]['s_icon_class'],
                    't_name' => $tmp[$i][db_field_wrtcl('s_name')],
                ); 
    ...
    // Fetch all the action list
    public function fetch_all_the_action_permission()
    {
        $this->db->simple_query('SET SESSION group_concat_max_len=150000');
        
        $sql = "SELECT GROUP_CONCAT(s_action_permit SEPARATOR '||') AS permition FROM {$this->tbl} WHERE i_parent_id != 8 AND i_parent_id != 0";
        $rs = $this->db->query($sql)->result_array();
        
        if($rs[0]['permition'] != '')
        {
            $action = array_unique(explode('||', $rs[0]['permition']));
