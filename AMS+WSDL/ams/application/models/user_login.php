...
class User_login extends MY_Model
{
	...
	private $tbl_usr, $tbl_role, $tbl_rg, $tbl_fr;
    ...
    public function fetch_display_fe_user($s_where = null, $i_start = null,$i_limit = null, $order_by=null, $sort_type=null)
    {
        try
        {            
                     
            $sql = "SELECT n.*, ur.i_region_id , rg.s_name, fr.s_franchise_name      
                 FROM {$this->tbl_usr}  as n  "
                 . " INNER JOIN  {$this->tbl_role} as ur ON n.i_id = ur.i_user_id " 
                 ." LEFT JOIN {$this->tbl_rg} as rg ON ur.i_region_id = rg.i_id  "
                 ." LEFT JOIN {$this->tbl_fr} as fr ON ur.i_franchise_id = fr.i_id  "
                .($s_where!=""? $s_where:"" )
                ." GROUP BY n.i_id "
                .($order_by!=""? " ORDER BY ".$order_by." ".$sort_type:"" )
    ...
    */
    public function count_display_fe_user($s_where = null)
    {
        try
        {
            $sql = "SELECT n.i_id  FROM {$this->tbl_usr}  as n  "
                 . " INNER JOIN  {$this->tbl_role} as ur ON n.i_id = ur.i_user_id " 
                 ." LEFT JOIN {$this->tbl_rg} as rg ON ur.i_region_id = rg.i_id  "
                 ." LEFT JOIN {$this->tbl_fr} as fr ON ur.i_franchise_id = fr.i_id  "
                .($s_where!=""?$s_where:"" )
    ...
    */
    public function fetch_multi($s_where = null, $i_start = null,$i_limit = null)
    {
        try
        {
		    $s_qry = "SELECT n.* FROM {$this->tbl_usr} AS n "
					  .$s_where
					 .(is_numeric($i_start) && is_numeric($i_limit)?" LIMIT ".intval($i_start).",".intval($i_limit):"");
    ...
    */
    public function gettotal_info($s_where = null)
    {
        try
        {
            $tmp = $this->db->query("SELECT count(1) AS total FROM {$this->tbl_usr} AS n {$s_where}")->result_array();
            return $tmp[0]['total'];
        }
        catch(Exception $err_obj)
        {
    ...
    public function delete_info($id)
    {
        try
        {
            ...
            if(intval($id)>0)
            {
				...
				$s_qry="UPDATE ".$this->tbl_usr." SET e_deleted = 'Yes' ";
                $s_qry.="WHERE i_id=? ";
    ...
    */
	...
	public function backend_user_login($login_data)
    {
        try
        {
	  	    ...
	  	    $s_qry = "SELECT i_id, s_first_name, s_last_name, s_user_name, s_email, i_status, i_user_type, e_access_type 
                      FROM ".$this->tbl_usr." WHERE BINARY s_user_name=?";
	        //$s_qry.=" And s_password=? And i_status = 1 AND e_deleted = 'No'";
	        $s_qry.=" And s_password=? And i_status = 1 AND i_user_type IN(1,2,3,4) AND e_deleted = 'No'"; // date 22 aug 2017 as per see@ https://shieldwatch.teamwork.com/#messages/709228 (only admin users can login to the site)
	...
	public function fe_user_login($login_data)
    {
		...
		{
			$ret_ = array();
			$s_qry = "SELECT i_id, s_first_name, s_last_name, s_email 
                        FROM ".$this->db->BUYER." 
						WHERE s_email = ? And s_password = ? AND e_deleted = 'No' AND i_is_verified = 1";
			
			$stmt_val["username"] = $login_data["username"];
            
			//Added the salt value with the password
			$stmt_val["password"]= md5($login_data["password"].$this->conf["security_salt"]);
				...
				foreach($rs->result() as $row)
				{
					...
					$this->session->set_userdata(
						array(
								"fe_loggedin"=> 
									array(
											"user_id"		=> encrypt(intval($ret_["i_id"])),
											"user_name"		=> $ret_["s_user_name"],
											"user_fullname"	=> $ret_["s_full_name"],
	...
	public function change_display_status($info,$i_id)
    {
        try
        {
            $i_ret_=0;//Returns false
            ...
            {
                $s_qry    =    "Update ".$this->tbl_usr." Set ";
                $s_qry.=" i_front_display= ? ";
                $s_qry.=" Where i_id=? ";
                //echo $i_id.$info['i_is_active'];
                $i_ret_=$this->db->query($s_qry,array(    intval($info['i_front_display']),
                intval($i_id)
                ));
                ...
                {
                    $logi["msg"]="Updating ".$this->tbl_usr." ";
                    $logi["sql"]= serialize(array($s_qry,array(    intval($info['i_front_display']),
                    intval($i_id)
                    )) ) ;                                 
                    $this->log_info($logi); 
                    unset($logi,$logindata);
    ...
    public function backend_chk_user($login_data)
    {
        try
        {
            $ret_ = array();
            $s_qry = "SELECT i_id, s_first_name, s_last_name, s_user_name, s_email, i_status, i_user_type, e_access_type 
                  FROM ".$this->tbl_usr." WHERE BINARY s_email=?";
