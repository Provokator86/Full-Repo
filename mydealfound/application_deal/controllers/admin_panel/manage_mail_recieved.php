...
class Manage_mail_recieved extends My_Controller implements InfController
{
    ...
    public function __construct()
    {
        try
        {
          ...
          ////////Define Errors Here//////
			...
			$this->cls_msg["delete_err"]			= "Mail Information failed to remove.";
			$this->cls_msg["delete_succ"]			= "Mail Information removed successfully.";
			...
			$this->thumb_big_wd						= 60;
		 	...
		 	$this->load->model("mail_recieved_model");
    ...
    public function index()
    {
        try
        {
            ...
            $this->data['title']		=	"Manage Mail";////Browser Title
            $this->data['heading']		=	"Manage Mail";
    ...
    */      
    public function remove_information($i_id=0)
    {
        ...
        { 
			...
			{
				case "all":
							$i_ret_=$this->mail_recieved_model->delete_info(-1);
							break;
				default: 		///Deleting selected,page ///
							...
							while($tot>=0)
							{
							$i_ret_=$this->mail_recieved_model->delete_info(decrypt($id[$tot]));
							$tot--;
							}
							}
							elseif($id>0)///Deleting single Records
							{
							$i_ret_=$this->mail_recieved_model->delete_info(decrypt($id));
							}                
							break;
			}
    ...
    */
	...
	public function show_list($order_name='',$order_by='desc',$start=NULL,$limit=NULL)
	{
		try
        {
			$this->data['heading']="Manage Mail";////Package Name[@package] Panel Heading
            ...
            $srch_s_url=($this->input->post("h_search")?$this->input->post("srch_s_url"):$this->session->userdata("srch_s_url")); 
			 if($s_search=="basic")
            {
                $s_where =" WHERE (s_from LIKE '%".my_receive_like($srch_s_url)."%' )";
			...
			$info	= $this->mail_recieved_model->fetch_multi_sorted_list($s_where,$s_order_name,$order_by,intval($start),$limit);///////////test
			//pr ($info,1);
            ...
            $table_view["caption"]     			= "Manage Mail";
            $table_view["total_rows"]			= count($info);
			$table_view["total_db_records"]		= $this->mail_recieved_model->gettotal_info($s_where);
			...
			$table_view["headers"][2]["val"]	= "Message";
			$table_view["headers"][2]["align"]  = "center";
			$table_view["headers"][3]["val"]	= "Date(dd-mm-yyyy)";
            ...
            for($i=0; $i<$table_view["total_rows"]; $i++)
            {
                ...
                $table_view["tablerows"][$i][$i_col++]	= $info[$i]["s_from"];
				$table_view["tablerows"][$i][$i_col++]	= $info[$i]["s_subject"];
				$table_view["tablerows"][$i][$i_col++]	= $info[$i]["s_message"];
				$table_view["tablerows"][$i][$i_col++]	= date('d-m-Y',strtotime($info[$i]["dt_date"]));
