...
class Manage_comment extends My_Controller implements InfController
{
    ...
    public function add_information()           
    {
			...
			if($_POST)
        {
        ...
        else
           {
					...
					$inserted_user_id	= $this->comment_model->add_info($info);					
        ...
        { //echo "12"."$i_td";
            ...
            {	
                ...
                else///validated, now save into DB
                {
                    ...
                    $i_aff=$this->comment_model->edit_info($info, decrypt($posted["h_id"]));
    ...
    */      
    public function remove_information($i_id=0)
    {
        ...
        { 
			...
			{
				case "all":
							$i_ret_=$this->comment_model->delete_info(-1);
							break;
				default: 		///Deleting selected,page ///
							...
							while($tot>=0)
							{
							$i_ret_=$this->comment_model->delete_info(decrypt($id[$tot]));
							$tot--;
							}
							}
							elseif($id>0)///Deleting single Records
							{
							$i_ret_=$this->comment_model->delete_info(decrypt($id));
							}                
							break;
			}
    ...
    */
	...
	public function show_list($order_name='',$order_by='desc',$start=NULL,$limit=NULL)
	{
		try
        {
			...
			$info	= $this->comment_model->fetch_multi($s_where,$order_by,intval($start),$limit);///////////test
				//print_r ($info);
			...
			$table_view["total_db_records"]	=	$this->comment_model->gettotal_info($s_where);
...
public function show_vote($id)
    {
        ...
        {	$this->data['title']		="Vote Details";////Browser Title
			...
			//echo $id;exit;
			$info=$this->comment_model->get_vote_details($id);
			//pr($info,1);
			//-----------------------------------------------------------------------------------------
	...
	public function show_comments($id=0)
    {
        try
        {	
			$this->data['title']		="Comment Details";////Browser Title
            $this->data['heading']		="Commnet Details";
			//echo $id;exit;
			$info=$this->comment_model->get_comments_details($id);
			//pr($info,1);
			//-----------------------------------------------------------------------------------------
            ...
            //////Table Headers, with width,alignment///////
            $table_view["caption"]     		=	"Comments Details";
            $table_view["total_rows"]		=	count($info);
			$table_view["total_db_records"]	=	$this->comment_model->gettotal_info($s_where);
			$table_view["order_name"]		=	$order_name;
			$table_view["order_by"]  		=	$order_by;
			...
			$table_view["headers"][1]["align"]	="center";
			$table_view["headers"][1]["val"]	="IP";
			$table_view["headers"][2]["val"]	="Date";
			$table_view["headers"][3]["val"]	="Comments"; 
			//pr($table_view,1);
            ...
            for($i=0; $i<$table_view["total_rows"]; $i++)
            {
				...
				$info[$i]["s_is_active"] = ($info[$i]["i_status"]==1)?'Active':'Inactive';
				$name_coupon=$this->coupon_model->fetch_this($info[$i]['i_coupon_id']);
				//pr($name_coupon);
				$name=$name_coupon[0]['s_title'];
				$i_col=0;
                $table_view["tablerows"][$i][$i_col++]	= encrypt($info[$i]["i_id"]);////Index 0 must be the encrypted PK 
