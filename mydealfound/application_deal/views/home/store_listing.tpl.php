...
<div class="wrapper_inner_banner">
	<div class="left_box">
        ...
        <div class="l_des">
		<a href="javascript:"><img src="<?=base_url()?>images/cashback.png" alt="" /></a> <strong><?php echo $store_details[0]['s_cash_back']?></strong>
        	<p>
        ...
        <div class="rate_it">
						...
						<div class="clear"></div>
							<a href="javascript:void(0);" onclick="showCashBackDetails();" style="color:#003366; font-weight:bold;" class="view_det">Cashback Details <span>&raquo;</span></a>
							<?php if(!empty($loggedin_details)) { 
								$s_uid = $loggedin_details[0]['s_uid'];
								//$chk_url = $store_details[0]['s_store_url']."&UID=$s_uid";
								$chk_url = $store_details[0]['s_store_url'];
								$chk_url = str_replace('MYDEALFOUNDXXX',$loggedin_details[0]['s_uid'],$chk_url);
							?>
							<input class="in_rw_submit" name="Submit" type="submit" onclick="chekForShop('<?php echo $chk_url;?>')" value="Shop Now" />
							
							<?php } else { ?>
							<?php /*?><input class="in_rw_submit" name="Submit" type="submit" onclick="chekForShop('<?php echo $store_details[0]['s_store_url']?>')" value="Shop Now" /><?php */?>
							<input class="in_rw_submit shop_now_store" name="Submit" type="submit" rel="<?php echo $store_details[0]['s_store_url']?>" value="Shop Now" />
							<?php } ?>
            ...
            <div class="resp-tabs-container">
                <div>
					...
					?>
                           <div class="tab_row">
                                ...
                                <div class="col_2"><span><?php echo ($popular_val['deal_count']>0)?$popular_val['deal_count'].$number_txt:'';?></span> <span><?php echo ($popular_val['s_cash_back']!='')?$popular_val['s_cash_back']:'';?></span></div>
				...
				?>
                           <div class="tab_row">
                                ...
                                <div class="col_2"><span><?php echo ($top_val['deal_count']>0)?$top_val['deal_count'].$number_txt:'';?></span><span><?php echo ($top_val['s_cash_back']!='')?$top_val['s_cash_back']:'';?></span></div>
				...
				?>
                            <div class="tab_row">
                                ...
                                <div class="col_2"><span><?php echo ($max_val['deal_count']>0)?$max_val['deal_count'].$number_txt:'';?></span>
								<span><?php echo ($max_val['s_cash_back']!='')?$max_val['s_cash_back']:'';?></span></div>
...
<?php /*?> changed on 13 may as per requirements  <?php */?>
<div class="clear"></div>
<?php //if($store_ads_list[0]["s_html"]!='') { ?>
<?php if($store_ads_list) { ?>
<div class="deal_list_wrapper">
	<div class="prodct1">
		<div class="prodct_heading"><?php echo $searchTitle; ?> </div>
		<div class="store_ads_list">
		<?php
			foreach($store_ads_list as $val)
			{
				echo $val["s_html"];
			}
		 ?>
		<?php //echo $store_ads_list[0]["s_html"] ?>
		</div>
	</div>
</div>
<?php } ?>
...
<script type="text/javascript">
$(document).ready(function(){
	$(".store_ads_list a").each(function(){
		$(this).click(function(evn){
			//console.log($(this).attr('href'));
			evn.preventDefault();
			var org_href = $(this).attr('href');
			window.open('<?php echo base_url().'thankyou/shopads/'.$store_details[0]['i_id']; ?>/?ref_url='+encodeURIComponent(org_href),'_blank');
			   
      	 	
		});
	});
	
	$(".shop_now_store").click(function(){
		var storeUrl = $(this).attr('rel');
		var redir_url = '<?php echo base_url().'thankyou/shopnowstore/'?>/?ref_url='+encodeURIComponent(storeUrl);
