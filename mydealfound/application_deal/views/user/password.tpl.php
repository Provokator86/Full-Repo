				...
				<div class="tab_container22">   
					 <div id="tab4" class="tab_content22"> <?php */?>
						<div class="account_box box_acnt" >
							<form action="<?=  base_url()?>user/update_password" method="post" class="pwd_form" onsubmit="return false;">
									...
									<div class="in_clm">
											...
											<div class="in_rw2">
												<input class="in_rw_submit5" name="Submit" type="submit" onclick="validate_password_form()" value="Submit" />
											</div>
									</div>
									<div class="clear"></div>
						   </form>
...
<script>
  ...
  function validate_password_form(){
        if(validate_form($('.pwd_form'),
        {
         ...
         ajax_pwd_form_submit($('.pwd_form'));
        }
    }

    function ajax_pwd_form_submit(targetForm){
        $.post($(targetForm).attr('action'), $(targetForm).serialize(), function(respData){
            //console.log($(targetForm).attr('action'));
            if(respData.status =='success'){
                window.location = '<?=  base_url()?>user/password';
            } else {
            }
        }, 'json');
