...
class MY_Controller  extends CI_Controller {
    ...
    */
    ...
    protected function render($passedData =array(),$view_file= '',$returnData=FALSE,$header_tpl='common/header.tpl.php',$footer_tpl='common/footer.tpl.php')
    {
        try
        {
			...
			$cms = $this->cms_model->get_list(array('i_id'=>5),'en_s_description as s_description,en_s_title as s_title',1); 
			...
			// previous commented four lines are old logic, new logic are below 26Mar 2014 by ACS
			
			$s_where = "WHERE i_parent_id=0 AND e_show_in_frontend='1' ";
			$passedData['categoryData'] = $this->category_model->get_category_list($s_where,0,10);
			//pr($passedData['categoryData'],1);
			if(!empty($passedData['categoryData']))
			{
				foreach ($passedData['categoryData'] as $key => $val) {
				
					//$this->output->cache(2);
					
					$condn = "WHERE i_parent_id = '".$val["i_id"]."' AND e_show_in_frontend='1' ";
					$passedData['categoryData'][$key]['sub_cat'] = $this->product_model->get_sub_category($condn,0,10);
					
					$all_category_str = ltrim(select_product_under_category($val["i_id"]),',');
					$popular_where  = " WHERE p.i_is_active = 1 AND p.i_cat_id IN({$all_category_str}) ";
					
					$distinct_store_ids = $this->product_model->get_distinct_store_from_product($popular_where);
					if($distinct_store_ids!="")
					{
						$s_con = "WHERE i_is_active =1 AND i_id IN ({$distinct_store_ids}) ";
						$passedData['categoryData'][$key]['popular_store'] = $this->product_model->get_all_store($s_con,0,10);
					}
					
					$brand_where = " WHERE p.i_is_active = 1 AND p.i_cat_id IN({$all_category_str}) AND p.s_brand_name!='' ";
					$distinct_brands = $this->product_model->get_distinct_brand_from_product($brand_where,0,10);
					if(!empty($distinct_brands))
					{
						$passedData['categoryData'][$key]['popular_brand'] = $distinct_brands;
					}
					
					$condn = "WHERE p.i_cat_id = '".$val["i_id"]."' AND p.i_is_hot = 1 ";
					$passedData['categoryData'][$key]['top_deals'] = $this->manage_deals_model->get_deals_offers_list($condn,0,5);
					
				}
			}
			
			// END previous are old logic, new logic are below 26Mar 2014 by ACS
		   ...
		   $latest_deals = $this->deal_model->get_active_deal_list(" i_is_hot=1 AND (CONCAT(DATE(dt_exp_date),' 23:59:59')>='".now()."' OR dt_exp_date = 0) ",'i_id,s_title,s_url,s_image_url,s_meta_description,s_seo_url,s_summary');
			...
			// 7 apr 2014
			$passedData['fb_app_id'] = "788272717851332"; // FB App ID/API Key,
			$passedData['fb_app_secret'] = "2d170f2cb1e86b9d1330a76284d74b89"; // FB App Secret 
			...
			//$returnViewData .= $this->load->view($facebook_js,$passedData,$returnData);
   ...
   protected function process_deal_list($dealListCondition = '',$dealListTitle = '',$paging = 0,$limit = 3,$base_url= null,$uri_segment = 3,$likeCondition = null,$ajax_pagination=false) {
	 ...
	 */
	 protected function process_new_deal_list($dealListCondition = '',$dealListTitle = '',$paging = 0,$limit = 3,$base_url= null,$uri_segment = 3,$likeCondition = null,$ajax_pagination=false) {

        $start = $paging;
        $totalData = $this->manage_deals_model->count_joined_active_deal_total($dealListCondition,$likeCondition);
        $config['base_url'] = $base_url;
        $config['total_rows'] = $totalData;
        $config['per_page'] = $limit;        
        $config['uri_segment'] = $uri_segment;
        ...
        $dealListData['title'] = $dealListTitle;  
        //$config['use_page_numbers'] = TRUE;
        $dealListData['dealList'] = $this->manage_deals_model->get_joined_active_deal_list($dealListCondition,'cd_deals.i_id,cd_deals.i_cat_id,cd_deals.i_coupon_code,s_title,s_store_title,s_store_logo,s_store_logo,s_seo_url,i_cashback,dt_exp_date,cd_deals.s_url,cd_deals.s_url,s_store_url,s_image_url,cd_store.s_meta_description as store_meta,cd_deals.s_meta_description as coupon_meta,d_list_price,d_selling_price,d_discount,s_discount_txt',$limit,$start,$likeCondition);
		
		//echo $this->db->last_query();
		
        //return $this->load->view('elements/deal_list.tpl.php',array('dealListData'=>$dealListData,'config'=>$config),true);
		return $this->load->view('elements/offers_list.tpl.php',array('dealListData'=>$dealListData,'config'=>$config),true);
