    ...
    */
    ...
    class vote_model extends MY_Model 
    {
        ...
        */
        ...
        public function check_whether_voted($ip,$store_id)
        {
            try
            {
                $ret_=0;
                $s_qry="SELECT COUNT(*) as i_total FROM ".$this->tbl." WHERE i_ip='".$ip."' AND i_store_id=".$store_id;
                $rs=$this->db->query($s_qry);
                $i_cnt=0;
                if($rs->num_rows()>0)
                {
        ...
        * @author Mainak Das
        * @date 16-11-13
        * fetch avg vote for a spcific store
        * 
        */
        public function fetch_store_avg_rating($store_id="")
        {
            if(empty($store_id))
                return ;
            else{
                $sql= "SELECT  v.i_store_id, CEIL(AVG(v.i_vote) ) AS AVG_VOTE 
                        FROM {$this->tbl} AS v 
                        WHERE v.i_store_id=$store_id 
                        ORDER BY v.i_id 
                        desc 
                        LIMIT 0, 10
                        ";
                ...
                if($rs->result_array()){
                    ...
                    foreach($rs->result_array() as $k=>$v){
                        $ret_['store_id']      = $v['i_store_id'];  
                        $ret_['AVG_VOTE']    = $v['AVG_VOTE'];
                    }
                }
