...
class Food_dining_offer extends My_Controller implements InfController
{
    ...
    */
    public function add_information()   
    {
		try
        {
          	$this->data['title']				= "Food & Dining Offer Management";////Browser Title
            $this->data['heading']				= "Add Food & Dining Offer ";
        	...
        	{				
				...
				{		
					$offer_url	= getSeoUrl($this->db->FOOD_OFFER,$this->input->post("s_offer"));				
					$info	= array(	
					...
					$inserted_user_id	= $this->food_dining_model->add_offer_info($info);
            ...
            ////////////end Submitted Form///////////
            $this->render("food_dining_offer/add-edit");
    ...
    */      
    ...
    { 
        ...
        { 
            ...
            $this->data['title']		="Edit Food & Dining Offer";////Browser Title
            $this->data['heading']		="Edit Food & Dining Offer";
            $this->data['pathtoclass']	=$this->pathtoclass;
            $this->data['mode']			="edit";
            ////////////Submitted Form///////////
            if($_POST)
            {	
                ...
                else///validated, now save into DB
                {
					...
					$offer_url					= getSeoUrl($this->db->FOOD_OFFER,$posted["s_offer"],$posted["h_id"]);
					$info						= array();
                    ...
                    $i_aff=$this->food_dining_model->edit_offer_info($info, decrypt($posted["h_id"]));
            ...
            { 
                $info=$this->food_dining_model->fetch_this_offer(decrypt($i_id));	
    ...
    public function remove_information($i_id=0)
    {
        ...
        { 
            ...
            switch($s_del_these)
			{
				...
				case "all":
							$i_ret_=$this->food_dining_model->delete_offer_info(-1);
							break;
				default: 		///Deleting selected,page ///
							...
							if(is_array($id) && !empty($id))///Deleting Multi Records
							{
								...
								while($tot>=0)	
								{	
									$if_exist= $this->food_dining_model->check_offer_in_food_table(decrypt($id[$tot]));
									if(!$if_exist)
									{
										$i_ret_=$this->food_dining_model->delete_offer_info(decrypt($id[$tot]));
									}
									$tot--;	
								}
							...
							{
								$i_ret_=$this->food_dining_model->delete_offer_info(decrypt($id));
							}                
    ...
    */
    ...
    public function show_detail($i_id=0)
    {
        try
        {
            ...
            $this->render("food_dining_offer/show_detail",TRUE);
	...
	public function show_list($order_name='',$order_by='desc',$start=NULL,$limit=NULL)
	{
		try
        {
			...
			$this->data['heading']="Manage Food & Dining offer";////Package Name[@package] Panel Heading
			...
			$info	= $this->food_dining_model->fetch_multi_offer_list($s_where,$s_order_name,$order_by,intval($start),$limit);
			//print_r ($info);
			...
			$table_view["total_db_records"]	=	$this->food_dining_model->gettotal_offer_info($s_where);
    ...
    */
	...
	public function ajax_change_status()
    {
        try
        {
				...
				$if_exist= $this->food_dining_model->check_in_food_table($posted["id"]);
				if(!$if_exist)
				{
					...
					$i_rect=$this->food_dining_model->change_offer_status($info,$posted["id"]); /*don't change*/	
