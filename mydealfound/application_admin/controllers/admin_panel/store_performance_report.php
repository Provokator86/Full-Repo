...
* 
* Purpose:
* Controller For Manage Store Performance Report
* 
* @package Report
* @subpackage Store Performance Report
* 
...
class Store_performance_report extends My_Controller implements InfController
{
	...
	public function __construct()
    {
      try
      {
			...
			parent::__construct();
			$this->data['title']	= "Store Performance Report Management"; // Browser Title
    ...
    */
	...
	public function show_list($order_name = '', $order_by = 'desc', $start = NULL, $limit = NULL)
    {
        ...
        {           
			...
			$this->data['heading']="Store Performance Report";////Package Name[@package] Panel Heading
           ...
           $arr_session_data    =    $this->session->userdata("arr_session");
			...
			$search_variable["s_merchant_name"] = ($this->input->post("h_search")?$this->input->post("s_merchant_name"):$arr_session_data["s_merchant_name"]);
			...
			$search_variable["i_output"] = ($this->input->post("h_search")?$this->input->post("i_output"):$arr_session_data["i_output"]);
            ...
            if($s_search=="basic")
            {
				...
				if(trim($search_variable["s_merchant_name"])!="")
				{
					$s_where.=" AND e.s_merchant_name = '".my_receive_text($search_variable["s_merchant_name"])."' ";
				}
                ...
                $arr_session["s_merchant_name"]	= $search_variable["s_merchant_name"] ;
				...
				$arr_session["i_output"] 	    = $search_variable["i_output"];
				
                $this->session->set_userdata("arr_session",$arr_session);
                $this->session->set_userdata("h_search",$s_search);
				
                $this->data["h_search"]	   		= $s_search;
                $this->data["s_merchant_name"]	= $search_variable["s_merchant_name"];            
				...
				$this->data["i_output"]    		= $search_variable["i_output"]; 
           ...
           unset($s_search,$arr_session,$search_variable);
			...
			$arr_sort = array(0=>'s_merchant_name',1=>'dt_of_payment',2=>'i_status');
			...
			$info	= $this->mod_pay->fetch_store_performance_list($s_where,$s_order_name,$order_by,intval($start),$limit);
			//pr($info);
            ...
            $table_view["caption"]     		=	"Store Performance Report";
            $table_view["total_rows"]		=	count($info);
			$table_view["total_db_records"]	=	$this->mod_pay->gettotal_store_performance($s_where);
			...
			$table_view["headers"][0]["val"]	="Store";
			$table_view["headers"][1]["val"]	="Sell Amount";
			$table_view["headers"][2]["val"]	="Commission Earned";
			$table_view["headers"][3]["val"]	="Cashback";
			if(trim($this->input->post("i_output"))=="sum")
				$table_view["headers"][4]["val"]	="Sum";
			else
				$table_view["headers"][4]["val"]	="Count";
			/*$table_view["headers"][5]["width"]	="12%";
			$table_view["headers"][5]["val"]	="Date";*/	
            ...
            for($i=0; $i<$table_view["total_rows"]; $i++)
            {
				...
				$table_view["tablerows"][$i][$i_col++]	= 'Rs '.number_format($info[$i]["total_amount"],2);
				$table_view["tablerows"][$i][$i_col++]	= $info[$i]["total_commission_amount"]?'Rs '.number_format($info[$i]["total_commission_amount"],2):"";
				$table_view["tablerows"][$i][$i_col++]	= $info[$i]["total_cashback_amount"]?'Rs '.number_format($info[$i]["total_cashback_amount"],2):"";
				
				if(trim($this->input->post("i_output"))=="sum")
					$table_view["tablerows"][$i][$i_col++]	= 'Rs '.number_format($info[$i]["total_cashback_amount"],2);
				else
					$table_view["tablerows"][$i][$i_col++]	= $info[$i]["i_total"];
