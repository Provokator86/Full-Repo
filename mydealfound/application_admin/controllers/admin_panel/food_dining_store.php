...
class Food_dining_store extends My_Controller implements InfController
{
    ...
    */
    ...
    public function add_information()   
    {
		try
        {
            ...
            $this->data['heading']				= "Add Food & Dining Store ";
			...
			if($_POST)
        	{
				...
				{		
					...
					{					
						...
						redirect(admin_base_url().'food_dining_store/add_information');						
					}				
					else				
					{
					...
					$store_url	= getSeoUrl($this->db->FOOD_STORE,$this->input->post("s_store_title"));
					...
					$inserted_user_id	= $this->food_dining_model->add_store_info($info);	
            ...
            $this->render("food_dining_store/add-edit");
        }
        catch(Exception $err_obj)
        {
            show_error($err_obj->getMessage());
        ...
        try
        {
            ...
            {	
                ...
                {	
					...
					{
						$this->session->set_userdata(array('message'=>$this->cls_msg["img_upload_err"],'message_type'=>'err'));
						redirect(base_url().'food_dining_store/modify_information');	
					}
					else
					{
					...
					$store_url						= getSeoUrl($this->db->FOOD_STORE,$posted["s_store_title"],$posted["h_id"]);
                    ...
                    $i_aff=$this->food_dining_model->edit_store_info($info, decrypt($posted["h_id"]));
            ...
            { 
                $info=$this->food_dining_model->fetch_this_store(decrypt($i_id));	
				//pr($info,1);	
                $posted=array();
            ...
            switch($s_del_these)			{
				...
				default: 		///Deleting selected,page ///
							...
							if(is_array($id) && !empty($id))///Deleting Multi Records
							{
								...
								while($tot>=0)							
								{
									$if_exist= $this->food_dining_model->check_in_food_table(decrypt($id[$tot]));
									if(!$if_exist)
									{
										$i_ret_=$this->food_dining_model->delete_store_info(decrypt($id[$tot]));
									}
									$tot--;
								}
							}
							elseif($id>0)///Deleting single Records
							{
								...
								if(!$if_exist)
								{
									$i_ret_=$this->food_dining_model->delete_store_info(decrypt($id));
								}
							}                
    ...
    */
    public function show_detail($i_id=0)
    {
        try
        {
            ...
            $this->render("food_dining_store/show_detail",TRUE);
            unset($i_id);
        }
        catch(Exception $err_obj)
        {
	...
	public function show_list($order_name='s_store_title',$order_by='asc',$start=NULL,$limit=NULL)
	{
        ...
        {          
			...
			$this->data['heading']="Manage Food & Dining Store";////Package Name[@package] Panel Heading
			...
			$info	= $this->food_dining_model->fetch_multi_store_list($s_where,$s_order_name,$order_by,intval($start),$limit);
			//print_r ($info);
			...
			$table_view["total_db_records"]		=	$this->food_dining_model->gettotal_store_info($s_where);
    ...
    */
	...
	public function ajax_change_status()
    {
        try
        {
				...
				$if_exist= $this->food_dining_model->check_offer_in_food_table($posted["id"]);
				if(!$if_exist)
				{
					...
					$i_rect=$this->food_dining_model->change_store_status($info,$posted["id"]); /*don't change*/	
