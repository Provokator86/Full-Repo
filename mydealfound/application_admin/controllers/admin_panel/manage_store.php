...
class Manage_store extends My_Controller implements InfController
{
	...
	public function show_list($order_name='s_store_title',$order_by='asc',$start=NULL,$limit=NULL)
	{
        ...
        {          
			...
			$table_view["headers"][3]["val"]	="Status"; 
			 ...
			 $table_view["headers"][6]["val"]	="Top Store";
			 
			$table_view["headers"][7]["val"]	="Inactivate Comments";
            ...
            {	
				...
				$sql= "SELECT COUNT(*) as inactive FROM cd_store_comment WHERE i_is_active = 0 && i_store_id = ".$info[$i]["i_id"];
				$inactvcmnt 	= $this->db->query($sql);
				$inactvcmnt_no	= $inactvcmnt->result_array();
				
				$sql= "SELECT COUNT(*) as total FROM cd_store_comment WHERE i_store_id = ".$info[$i]["i_id"];
				$cmnt 	= $this->db->query($sql);
				$cmnt_no	= $cmnt->result_array();
				...
				// $table_view["tablerows"][$i][$i_col++]	=($info[$i]["i_is_active"] == 1)?$action."  "."Active":$action." "."Inactive";
				 ...
				 $is_top_checked = $info[$i]["i_is_top"];						
						...
						if($is_top_checked==1)
							...
							$is_top = '<input type = "checkbox" '.$chk.' id="i_is_top_'.($info[$i]["i_id"]).'" name="i_is_top" onclick=" return top_status_check('.($info[$i]["i_id"]).' );" />';

				 $table_view["tablerows"][$i][$i_col++]	= $is_top;
				}
				
				$table_view["tablerows"][$i][$i_col++]	= '<a  href="'.base_url().'admin_panel/manage_store_comments/filter_stores/'.$info[$i]["i_id"].'">'.$inactvcmnt_no[0]['inactive'].'/'.$cmnt_no[0]['total'].'</a>';		
    ...
    */
	...
	public	function get_coupon_top_status()
	{
		...
		if($store_details[0]['i_is_active']==1)
			...
			echo $this->store_model->get_coupon_top_status($current_store_id);
...

	{
...

		{
			...
			echo $this->store_model->update_top_coupon_status($current_coupon_id, $current_coupon_status);
