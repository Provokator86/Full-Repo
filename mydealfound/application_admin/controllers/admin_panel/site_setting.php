    ...
    public function modify_information($i_id=0)
    {
        try
        {
            ...
            if($_POST)
            {
				...
				$posted["d_min_balance"]			= trim($this->input->post("d_min_balance"));
				$posted["d_cashback"]				= trim($this->input->post("d_cashback"));				

                $this->form_validation->set_rules('s_site_title', 'admin email', 'required');
                $this->form_validation->set_rules('d_min_balance', 'minimum balance', 'required|trim');
                $this->form_validation->set_rules('d_cashback', 'cashback return', 'required|trim');	
                ...
                else///validated, now save into DB
                {
					...
					$info["d_min_balance"]			= $posted["d_min_balance"];				
					$info["d_cashback"]				= $posted["d_cashback"];							
            ...
            else
            {
				...
				$info=$this->mod_rect->fetch_this(NULL);  // This method id modifiedon 24 June 2011
				//pr($info);
                $posted=array();
				...
				$posted["d_min_balance"]			= trim($info["d_min_balance"]);
				$posted["d_cashback"]				= trim($info["d_cashback"]);
