    ...
    public function show_list($start=NULL,$limit=NULL)
    {
        try
        {
            ...
            if($s_search=="basic")
            {
				...
				if($s_milestones_title)
				{
					//$s_where =." AND n.en_s_subject LIKE '%".my_receive($s_milestones_title)."%' ";
				}
...

    {
...

        {
...

            {
				...
				$posted["txt_content_french"]= trim($this->input->post("txt_content_french"));
				...
				$this->form_validation->set_rules('txt_content_french', 'automail description in french', 'required');
...

                {
					...
					$info["s_content_french"]	=	$posted["txt_content_french"];
...

        {
...

            {
...

                {
					...
					$info[$this->lang_prefix."_s_subject"]			=	$posted["txt_subject"];

                    $info[$this->lang_prefix."_s_body"]			=	$posted["txt_content"];
    ...
    */
...

    {
...

        {
			...
			$this->edit_info_link       =   'modify_information/'.$type.'/';

            $this->data['heading']= ucfirst($this->db->AUTOMAIL_KEY[decrypt($type)])." Automail";////Package Name[@package] Panel Heading
...

            {
                ...
                $s_where .=" AND n.{$this->lang_prefix}_s_subject LIKE '%".my_receive($s_milestones_title)."%' ";////////-------
            ...
            $this->data["table_view"]=$this->admin_showin_param_table($table_view);
...
/**
    ...
    * This ajax function is used to fetch contains of automail in selected language.
