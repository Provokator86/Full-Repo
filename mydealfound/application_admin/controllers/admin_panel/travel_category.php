...
class Travel_category extends My_Controller implements InfController
{
    ...
    public function __construct()
    {
        try
        {
			...
			$this->cls_msg["save_err"]			= "Category failed to save.";
		...
		try
        {
			...
			if($_POST)
			{
				...
				else
				{
						...
						{
							$this->session->set_userdata(array('message'=>$this->cls_msg["img_upload_err"]
										,'message_type'=>'err'));
							redirect(base_url().'travel_category/add_information');	
						}
						else
						{
						...
						else
						{
		 				...
		 				$category_url	= getSeoUrl($this->db->TRAVEL_CAT,$this->input->post("s_category"));	
						 $info	= array(				
							...
							"s_thumb"			=> $arr_upload_res_2[2],
							"s_url"				=> $category_url

		 					);
						 //pr($info,1);
		  				$inserted_user_id	= $this->travel_category_model->add_info($info);	
            ...
            ////////////end Submitted Form///////////
            $this->render("travel_category/add-edit");
        }
        catch(Exception $err_obj)
        {
            show_error($err_obj->getMessage());
    ...
    */      
    ...
    { 
        ...
        { 
            ...
            $this->data['title']		= "Edit Travel Category";////Browser Title
            $this->data['heading']		= "Edit Travel Category";
            $this->data['pathtoclass']	= $this->pathtoclass;
            $this->data['mode']			= "edit";
           ...
           $this->data['category']		= $this->list_category(decrypt($i_id));		
            ...
            {	
                ...
                else///validated, now save into DB
                {
					...
					$category_url			= getSeoUrl($this->db->TRAVEL_CAT,$posted["s_category"],$posted["h_id"]);	
					...
					$i_aff=$this->travel_category_model->edit_info($info, decrypt($posted["h_id"]));
            ...
            { 
                $info	= $this->travel_category_model->fetch_this(decrypt($i_id));	
				//pr($info,1);		
                $posted=array();
    ...
    */      
    ...
    public function remove_information($i_id=0)
    {
        ...
        { 
			...
			{
				case "all":
							$i_ret_=$this->travel_category_model->delete_info(-1);
							break;
				default: 		///Deleting selected,page ///
							...
							///////////Deleting Information///////
								...
								while($tot>=0)
								{
								$i_ret_=$this->travel_category_model->delete_info(decrypt($id[$tot]));
								$tot--;
								}
							}
							elseif($id>0)///Deleting single Records
							{
								$i_ret_=$this->travel_category_model->delete_info(decrypt($id));
							}                
    ...
    */
    public function show_detail($i_id=0)
    {
        try
        {
            ...
            $this->render("travel_category/show_detail",TRUE);
            unset($i_id);
        }
        catch(Exception $err_obj)
        {
	...
	public function show_list($order_name='',$order_by='desc',$start=NULL,$limit=NULL)
	{
		try
        {
            $this->data['heading']	= "Manage Travel category";////Package Name[@package] Panel Heading
            ...
            $info = $this->travel_category_model->fetch_multi_sorted_list($s_where,$s_order_name,$order_by,intval($start),$limit);
            ...
            $table_view["total_db_records"]		= $this->travel_category_model->gettotal_info($s_where);
            ...
            {
                $img = $info[$i]["s_image"] != ''? '<img src="'.base_url().'uploaded/category/thumb/thumb_'.$info[$i]['s_thumb'].'" />' : '<img src="'.base_url().'uploaded/img/no_image.jpg"/>';
    ...
    */
	...
	public function ajax_change_status()
    {
        try
        {
				...
				$if_exist= $this->travel_category_model->check_in_cpn_table($posted["id"]);
				if(!$if_exist)
				{
					$posted["i_status"]     = trim($this->input->post("i_status"));
					$info = array();
					$info['i_status']    = $posted["i_status"]  ;
					$i_rect=$this->travel_category_model->change_status($info,$posted["id"]); /*don't change*/	
    ...
    private function list_category($skipId = NULL) {
        ...
        $query = $this->db->get($this->db->TRAVEL_CAT);
