...
class Manage_coupon extends My_Controller implements InfController
{
    ...
    public function __construct()
    {
        try
        {
		 ...
		 //$this->session->unset_userdata("s_store");		  
			...
			$this->load->model("admin_deal_model");
			...
			//$make_exp_cpn_notop=$this->admin_deal_model->remove_top_status();
    ...
    public function index()
    {
        try
        {
            ...
            $this->data['heading']		=	"Manage Deals";		
    ...
    */
    ...
    public function add_information()   
    {
		try
        {
          	$this->data['title']				= "Deals Management";////Browser Title
            $this->data['heading']				= "Add Deals ";
			...
			if($_POST)
        	{
				...
				*/
			    ...
			    {		
					...
					$time_now = date('H:i:s');
					$dt_exp_date = $this->input->post("dt_exp_date")!=''?$this->input->post("dt_exp_date").' '.$time_now:"";	
					$dt_of_live_coupons =  $this->input->post("dt_of_live_coupons")!=''?$this->input->post("dt_of_live_coupons").' '.$time_now:"";
	
					$coupon_url	= getSeoUrl_for_coupon($this->db->DEALS,$this->input->post("s_title"));
					...
					$info	= array(										
									...
									"i_bank_offer"					=> $this->input->post("i_bank_offer")
	
									);
	
						//pr($info,1);
	
						$inserted_user_id	= $this->admin_deal_model->add_info($info);	
						//$inserted_user_id = true;						
	
						if($inserted_user_id)////saved successfully	
						{	
							/*update cashback comparing with cashback matrix table as on 27 june 2014*/
							if(intval($info["i_cat_id"])>0)
							{
								//cashback_model
								$s_where = " WHERE n.i_cat_id ='".$info["i_cat_id"]."' ";
								$row_matrix = $this->cashback_model->fetch_cashback_matrix($s_where);
								//pr($row_matrix,1);
								$cashback = array();
								$cashback["i_cat_id"] 	= $info["i_cat_id"];
								$cashback["0-499"] 		= $row_matrix[0]['0-499'];
								$cashback["500-999"] 	= $row_matrix[0]['500-999'];
								$cashback["1000-1499"] 	= $row_matrix[0]['1000-1499'];
								$cashback["1500-1999"] 	= $row_matrix[0]['1500-1999'];
								$cashback["2000+"] 		= $row_matrix[0]['2000+'];
								$i_update = $this->cashback_model->update_product_cashback_txt($info["i_cat_id"],$cashback);
							}
							/*update cashback comparing with cashback matrix table as on 27 june 2014*/
    ...
    { 
        ...
        { 
            ...
            {	
				...
				$posted["i_bank_offer"]			= trim($this->input->post("i_bank_offer"));
                ...
                {	
					...
					$coupon_url	= getSeoUrl_for_coupon($this->db->DEALS,$posted["s_title"],$posted["h_id"]);					
					...
					$info["i_bank_offer"]			= $posted["i_bank_offer"];
                    ...
                    $i_aff=$this->admin_deal_model->edit_info($info, decrypt($posted["h_id"]));
            ...
            { 
                ...
                $info=$this->admin_deal_model->fetch_this(decrypt($i_id));		
					//pr($info,1);	
				...
				$posted["i_bank_offer"]					= $info[0]["i_bank_offer"];
    ...
    */      
    public function remove_information($i_id=0)
    {
        ...
        { 
			...
			{
				case "all":
							$i_ret_=$this->admin_deal_model->delete_info(-1);
							break;
				default: 		///Deleting selected,page ///
							...
							if(is_array($id) && !empty($id))///Deleting Multi Records
							{
								...
								while($tot>=0)
								{
									$i_ret_=$this->admin_deal_model->delete_info(decrypt($id[$tot]));
									$del_cpn_brnd=$this->admin_deal_model->del_cpn_brand(decrypt($id[$tot]));
									$tot--;
								}
							}
							elseif($id>0)///Deleting single Records
							{
								$i_ret_=$this->admin_deal_model->delete_info(decrypt($id));
								$del_cpn_brnd=$this->admin_deal_model->del_cpn_brand(decrypt($id[$tot]));
							} 
							break;
    ...
    */
	...
	public function show_list($order_name='',$order_by='desc',$start=NULL,$limit=NULL)
	{
        ...
        {           
			...
			$this->data['heading']="Manage Deals";////Package Name[@package] Panel Heading
            ...
            if($s_search=="basic")
            {
				...
				{	
					...
					$cat_id=$this->admin_deal_model->get_cat_id(my_receive_text($s_category));
					//pr($cat_id);
			...
			$info	= $this->admin_deal_model->fetch_multi_sorted_list($s_where,$s_order_name,$order_by,intval($start),$limit);
			//print_r ($info);
			...
			$table_view["total_db_records"]	=	$this->admin_deal_model->gettotal_info($s_where);
           ...
           // $table_view["detail_view"]		=   FALSE;         
			...
			$table_view["headers"][7]["val"]	="Popular";
			$table_view["headers"][7]["align"]	="center";			
			$table_view["headers"][8]["val"]	="Deal code";
			$table_view["headers"][8]["align"]	="center";
			$table_view["headers"][9]["val"]	="Expiry date";
			$table_view["headers"][9]["align"]	="center"; 
					
			/*$table_view["headers"][7]["val"]	="Deal code";
			$table_view["headers"][7]["align"]	="center";
            ...
            {					
				...
				$brand=$this->admin_deal_model->fetch_coupon_brand($info[$i]["i_id"]);
				//pr($brand);
					$brand_name = "";
					if(!empty($brand))
					{
				...
				$is_popular_checked = $info[$i]["i_is_popular"];	
				if($is_popular_checked==1)
					$chk_pop	= 'checked="checked"';
				else
					$chk_pop	= "";

				$is_popular = '<input type="checkbox" '.$chk_pop.' id="i_is_popular_'.($info[$i]["i_id"]).'" name="i_is_popular" onclick=" return popular_status_check('.($info[$i]["i_id"]).' );" />';
    ...
    */
...

    {
...

        {
                ...
                $i_rect=$this->admin_deal_model->change_status($info,$posted["id"]); /*don't change*/				
   ...
   public function upload_csv()
	...
	public	function get_coupon_hot_status()
	{
		...
		$coupon_details		= $this->admin_deal_model->fetch_this($current_coupon_id);	

		if($coupon_details[0]['i_is_active']==1)
			echo $this->admin_deal_model->get_coupon_hot_status($current_coupon_id);
		else
			echo 0;
		//echo $this->admin_deal_model->get_coupon_hot_status($current_coupon_id);
	...
	public function change_hot_status()
	{
		try
		{
			...
			echo $this->admin_deal_model->update_hot_coupon_status($current_coupon_id, $current_coupon_status);
		}
		catch(Exception $err_obj)
		{
			show_error($err_obj->getMessage());
	...
	public	function get_coupon_popular_status()
	{
		...
		if($coupon_details[0]['i_is_active']==1)
			echo $this->admin_deal_model->get_coupon_popular_status($current_coupon_id);
		else
			echo 0;
		//echo $this->admin_deal_model->get_coupon_hot_status($current_coupon_id);
	}	
	
	public function change_popular_status()
	{
		try
		{
			$current_coupon_id	= $this->input->post('current_coupon_id');
			$current_coupon_status	= $this->input->post('current_coupon_status');
			echo $this->admin_deal_model->update_popular_coupon_status($current_coupon_id, $current_coupon_status);
		}
		catch(Exception $err_obj)
		{
			show_error($err_obj->getMessage());
...

	{
...

        {
           ...
           // $s_where=" WHERE dt_exp_date<now()";/////////////////////////1====
			...
			$s_where=" WHERE CONCAT(DATE(dt_exp_date),' 23:59:59')<now() AND i_coupon_type = 2 AND (dt_exp_date!='0000-00-00 00:00:00' AND dt_exp_date!='') ";
...

            {
				...
				{	
					...
					$cat_id=$this->admin_deal_model->get_cat_id($s_category);
			...
			/*	 if(trim($s_store)!="")
				...
				{	
					...
					$cat_id=$this->admin_deal_model->get_store_id($s_store);
			...
			$info	= $this->admin_deal_model->fetch_multi_sorted_list($s_where,$s_order_name,$order_by,intval($start),$limit);///////////test
