    ...
    public function __construct()
    {
        try{
           ...
           //$this->load->helper('my_language_helper');
			...
			$this->data['offer_header']   			= $this->offer_model->fetch_multi($where);
			//-----------------------------------------------------------------------------------------------
			...
			/*********************** CATEGORY STARTS ***************************/
				...
				$this->data['category_header']   			= $this->category_model->fetch_multi($where,0,12);
				$this->data['category']   			= $this->category_model->fetch_multi($where,0,12);
				$this->data['category_image_path']	= base_url()."uploaded/category/";
			...
			//-----------------------------Festive Offer-------------------------------------------
			$this->load->model('offer_model');
			$where	= " WHERE i_is_active	= 1 AND i_is_top=1";
			$this->data['festive_offer_header']   			= $this->offer_model->fetch_multi($where);
			//pr($this->data['festive_offer_header']);
			//------------------------------Festive Offer-------------------------------------------
			
			
			//-----------------------------JOIN US-------------------------------------------
			$this->load->model('site_setting_model');
			$info=$this->site_setting_model->fetch_this(NULL);
			$this->data['join_us']   			= $info;
			//pr($info);
			//------------------------------JOIN US-----------------------------------------
    ...
    protected function render($s_template='',$b_popup=FALSE)
    {
        try
        {
           ...
           $s_router_directory=$this->router->fetch_directory();
            ...
            //pr(APPPATH . 'views/'.$s_router_directory.$s_template.'.tpl.php');
    ...
    */
    protected function parse_lang($s_raw_content)
    {
        try
        {
            ...
            if(SITE_FOR_LIVE)
            {
		$dir =  ($this->i_default_language==1)?'english':'english';
...
//----------For the category list in the header menu-----------------------------
public function category_select()
	{	
	
		$this->add_css('css/fe/dd.css');				
			$this->add_css('css/fe/nivo-slider.css');
			$this->add_js('js/fe/jquery.nivo.slider.js');
			$this->add_js('js/fe/jquery.dd.js');
		$where	= " WHERE i_status	= 1";
		$this->data['category']   			= $this->category_model->fetch_multi($where);
		//pr($this->data['category'],1);
		$this->render('fe/common/header');
	}
//----------For the category list in the header menu-----------------------------
