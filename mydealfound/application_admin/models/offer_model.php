...
class offer_model extends MY_Model implements InfModel
{
    private $conf;
	...
	public function change_status($info,$i_id)
    {
        try
        {
            ...
            if(!empty($info))
            {
				...
				{
					$sql="UPDATE ".$this->tbl." Set i_is_top = 0 WHERE i_id=".$i_id;
...
public function fetch_industry_numerically_sorted($s_where=null,$i_start=null,$i_limit=null)
    {
        try
        {   //echo $s_where;exit;
			//echo $this->tbl;
...
GROUP BY pi.i_id ORDER BY total" 
				   /*.($s_where!=""?$s_where:"" ).(is_numeric($i_start) && is_numeric($i_limit)?" Limit ".intval($i_start).",".intval($i_limit):"" )*/;
...
public function fetch_industry_alphabatically_sorted($s_where=null,$i_start=null,$i_limit=null)
    {
        try
        {   //echo $s_where;exit;
			//echo $this->tbl;
...
GROUP BY pi.i_id ORDER BY pi.s_title" 
				   /*.($s_where!=""?$s_where:"" ).(is_numeric($i_start) && is_numeric($i_limit)?" Limit ".intval($i_start).",".intval($i_limit):"" )*/;
...
public function fetch_industry_by_letter($letter)
    {
        try
        {   //echo $s_where;exit;
			//echo $this->tbl;
          	$s_qry="
SELECT pi.i_id, pi.s_title, COUNT( psp.i_id ) AS total
FROM pl_industry AS pi
LEFT JOIN pl_startup_personal AS psp ON pi.i_id
IN (
psp.s_industry1, psp.s_industry2, psp.s_industry3
)
WHERE pi.s_title LIKE  '".$letter."%'
GROUP BY pi.i_id" 
				   /*.($s_where!=""?$s_where:"" ).(is_numeric($i_start) && is_numeric($i_limit)?" Limit ".intval($i_start).",".intval($i_limit):"" )*/;
...
public function fetch_this_with_url($url)
	...
	public function get_offer_top_status($offer_id)
	{
		try
		{
			$s_query="SELECT count(*) as total FROM ".$this->tbl." WHERE i_is_top = 1";
			$sqr="SELECT i_is_top FROM ".$this->tbl." WHERE i_id = " .$offer_id ;
			$rs = $this->db->query($s_query);
	...
	public function update_top_offer_status($current_offer_id, $status = 0)
	{
		try
		{		
				if ($status==0)
				{
				$s_query = "UPDATE ".$this->tbl ." SET `i_is_top` = 1 WHERE i_id = ".intval($current_offer_id);
				$this->db->query($s_query);
				}
				if($status==1)
				{
				$s_query = "UPDATE ".$this->tbl ." SET `i_is_top` = 0 WHERE i_id = ".intval($current_offer_id);
				$this->db->query($s_query);
				}
