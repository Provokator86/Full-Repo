...
class Advertisement_model extends MY_Model implements InfModel
{
    private $conf;
    ...
    public function __construct()
    {
        try
        {
		  ...
		  $this->banner_mstr = $this->db->AD_PAGE; 
    ...
    */
	...
	public function get_page_name($i_id)
	{
		$s_query	= " SELECT page_reference FROM ".$this->banner_mstr." AS bnm
							LEFT JOIN ".$this->tbl." AS adv ON adv.i_page_id= bnm.i_id
							 WHERE bnm.i_id = '".my_receive($i_id)."'";
		$rs=$this->db->query($s_query); 
		return ($rs->result_array());
	}
	
	
	public function count_no_of_add_sense($i_id)
	{
		$s_query	= " SELECT count(*) AS total_add FROM  ".$this->tbl." WHERE i_page_id='".my_receive($i_id)."'";
		$rs=$this->db->query($s_query); 
		return ($rs->result_array());
	}
