        ...
        try
        {
          ...
          if($rs->num_rows()>0)
          {
              foreach($rs->result() as $row)
              {
				  ...
				  $ret_["i_store_id"]=$row->i_store_id;
				  $ret_["i_send_to"]=$row->i_send_to;
				  ...
				  $ret_["dt_send_on"]=date($this->conf["site_date_format"],intval($row->i_send_date )); 
				   $ret_["send_on"]=date("d-m-Y",intval($row->i_send_date )); 
        ...
        try
        {
            ...
            if(!empty($info))
            {
                ...
                $this->db->query($s_qry,array(
												  ...
												  intval($info["i_store_id"]),
												  intval($info["i_send_to"])
												 ));
        ...
        try
        {
            ...
            if(!empty($info))
            {
                ...
                $this->db->query($s_qry,array(
												  trim(htmlspecialchars($info["s_subject"], ENT_QUOTES, 'utf-8')),
												  trim(htmlspecialchars($info["s_content"], ENT_QUOTES, 'utf-8')),
												  trim(htmlspecialchars($info["i_user_type"], ENT_QUOTES, 'utf-8')),												  												  intval($info["dt_entry_date"]),
												  intval($info["i_send_date"]),
												  intval($info["i_store_id"]),
												  intval($info["i_send_to"]),
												  intval($id)
