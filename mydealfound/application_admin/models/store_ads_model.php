...
class Store_ads_model extends MY_Model
{

    private $conf;
    private $tbl_store_ads, $tbl_store;///used for this class
	...
	{	
        try
        {
			...
			$s_qry= "SELECT n.*,s.s_store_title 
					...
					." LEFT JOIN ".$this->tbl_store." AS s ON s.i_id = n.i_store_id "		
					.($s_where!=""?$s_where:"" )
					." ORDER BY n.{$order_name} {$order_by} "
					.(is_numeric($i_start) && is_numeric($i_limit)?" Limit ".intval($i_start).",".intval($i_limit):"" );
    ...
    */
    ...
    public function gettotal_info($s_where=null)
    {
        ...
        {
			$ret_=0;
			$s_qry= "SELECT n.i_id AS i_total
					FROM ".$this->tbl_store_ads." AS n "
					." LEFT JOIN ".$this->tbl_store." AS s ON s.i_id = n.i_store_id "	
					.($s_where!=""?$s_where:"" );
    ...
    */
    public function add_info($info)
    {
        try
        {
            $sql = $this->db->insert_string($this->tbl_store_ads,$info);
    ...
    public function edit_info($info,$i_id)
    {
        try
        {
            ...
            { 
				...
				$sql = $this->db->update_string($this->tbl_store_ads,$info,$where);
				if($this->db->simple_query($sql))
				{
    ...
    */
    public function delete_info($i_id)
    {
        try
        {
            ...
            if(intval($i_id)>0)
            {
				$s_qry="DELETE FROM ".$this->tbl_store_ads." ";
                $s_qry.=" Where i_id=? ";
                ...
                if($i_ret_)
                {
                    $logi["msg"]="Deleting ".$this->tbl_store_ads." ";
            ...
            elseif(intval($i_id)==-1)////Deleting All
            {
                ...
                if($i_ret_)
                {
                    ...
                    $logi["msg"]="Deleting all information from ".$this->tbl_store_ads." ";
