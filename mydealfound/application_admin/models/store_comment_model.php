...
class Store_comment_model extends MY_Model implements InfModel
{
    private $conf;
    ...
    public function __construct()
    {
        ...
        {
          parent::__construct();
          $this->tbl 				=	$this->db->STORE_COMMENT;
		  $this->tbl_cmnt 			=	$this->db->COMMENT; 
		  $this->tbl_cpn_commnet	=	$this->db->STORE_COMMENT;		         
    ...
    */
	...
	public function fetch_multi_sorted_list($s_where=null,$order_name,$order_by,$i_start=null,$i_limit=null)
	{
        ...
        {
            $ret_=array();
            $s_qry="SELECT cm.*,c.s_store_title  FROM ".$this->tbl.
                    " as cm LEFT JOIN cd_store AS c ON c.i_id = cm.i_store_id ".
    ...
    public function fetch_this($i_id)
    {
        try
        {
          ...
          ////Using Prepared Statement///
           $s_query="SELECT cm.*,c.s_store_title as store_name 
                                            FROM cd_store_comment as cm".
                                              "  LEFT JOIN cd_store AS c ON c.i_id = cm.i_store_id 
...
public function get_comments_details($id)
    {
        ...
        {   //echo $s_where;exit;
			//echo $this->tbl;
          	$s_qry="SELECT * FROM cd_store_comment WHERE i_store_id=".$id;
...
public function fetch_comments_for_store($cpn_id)
{
	try
	{
		$s_qry	=	"SELECT c.*,cm.*, cm.i_id as i_id FROM  cd_store_comment as cm 
					LEFT JOIN cd_store AS c 
					ON c.i_id = cm.i_store_id
					WHERE cm.i_store_id = ".$cpn_id;
		$rs		=	$this->db->query($s_qry);
		return($rs->result_array());
...
public function gettotal_comment_for_store($id=0)
    {
        try
        {
          $ret_=0;
          $s_qry="SELECT count(cm.i_id) AS i_total FROM  cd_store_comment as cm 
					LEFT JOIN cd_store AS c 
					ON c.i_id = cm.i_store_id
					WHERE cm.i_store_id = ".$id;
          $rs=$this->db->query($s_qry);
          $i_cnt=0;
          if($rs->num_rows()>0)
          {
...
public function get_cpn_name($cpn_id)
{
 ...
 public function add_store_comment($info)
    {
        try
        {  $sql = $this->db->insert_string($this->tbl,$info);
...
public function get_store_id($cpn_name)
{
	try
	{
		 $s_qry	="SELECT i_id 
...
public function fetch_comment_for_this_store($id)
{
	  $s_qry	="SELECT * 
			  FROM ".$this->tbl." 
			  WHERE i_store_id = '".($id)."'
