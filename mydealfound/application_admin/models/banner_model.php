...
class Banner_model extends MY_Model implements InfModel
{
    ...
    */
	...
	{
		$s_query	= " SELECT page_reference FROM ".$this->banner_mstr." WHERE i_id = '".my_receive($i_id)."'";
		$rs=$this->db->query($s_query); 
		return ($rs->result_array());
	}
 

	public function count_no_of_add_banner($i_id)
	{
		$s_query	= " SELECT count(*) AS total_banner FROM  ".$this->tbl." WHERE i_page_id='".my_receive($i_id)."'";
		$rs=$this->db->query($s_query); 
		return ($rs->result_array());
	}
