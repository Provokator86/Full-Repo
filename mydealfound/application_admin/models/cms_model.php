...
class Cms_model extends MY_Model implements InfModel
{
    ...
    public function fetch_multi($s_where=null,$i_start=null,$i_limit=null)
    {
        try
        {
			...
			$s_qry="SELECT * FROM ".$this->tbl." c "
                .($s_where!=""?$s_where:"" ).(is_numeric($i_start) && is_numeric($i_limit)?" ORDER BY en_s_title ASC Limit ".intval($i_start).",".intval($i_limit):"" );
          ...
          if($rs->num_rows()>0)
          {
              foreach($rs->result() as $row)
              {
                  ...
                  $ret_[$i_cnt]["s_title"]          =   get_unformatted_string($row->{$this->s_lang_prefix.'_s_title'});
				  if($ret_[$i_cnt]["s_title"]=='')
				  {
				  	$ret_[$i_cnt]["s_title"]          =   get_unformatted_string($row->{$this->site_default_lang_prefix.'_s_title'}); 
				  } 				 
				  $s_desc                           =   strip_tags($row->{$this->s_lang_prefix.'_s_description'});	
				  if($s_desc=='')
				  {
				  	//$s_desc                           =   $row->{$this->site_default_lang_prefix.'_s_description'};	
				  }			 
        ...
        try
        {
          ...
          if($rs->num_rows()>0)
          {
              foreach($rs->result() as $row)
              {
                  ...
                  $ret_["s_description"]	=	$row->{$this->s_lang_prefix.'_s_description'};
				  $ret_["s_title"]          =   $row->{$this->s_lang_prefix.'_s_title'}; 
				  
				  $ret_["i_cms_type_id"]          =   $row->i_cms_type_id; 
    ...
    */
	...
	/*******
    * Fetches One record from db for the cms_type_id value.
    * 
    * @param int $i_id
    * @returns array
    */
    public function fetch_content_using_cms_type_id($i_id,$s_prefix='')
    {
        try
        {
          $ret_=array();
