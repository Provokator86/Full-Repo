        ...
        try
        {
          ...
          if($rs->num_rows()>0)
          {
              foreach($rs->result() as $row)
              {
                    ...
                    $ret_[$i_cnt]["s_user_type"]		=	$this->db->USERROLE[$row->i_user_type]; 
        ...
        try
        {
          ...
          if($rs->num_rows()>0)
          {
              foreach($rs->result() as $row)
              {
                        ...
                        $ret_[$i_cnt]["i_user_type"]		=	intval($row->i_user_type); 
                        $ret_[$i_cnt]["s_user_type"]		=	$this->db->USERROLE[$row->i_user_type]; 
						$ret_[$i_cnt]["i_store_id"]			=	intval($row->i_store_id);
                        $ret_[$i_cnt]["i_is_active"]		=	intval($row->i_subscribe_status); 
                        $ret_[$i_cnt]["s_is_active"]		=	(intval($row->i_subscribe_status)==1?"Subscribed":"Unsubscribed");
        ...
        try
        {
          ...
          if($rs->num_rows()>0)
          {
              foreach($rs->result() as $row)
              {
				  ...
				  $ret_["s_user_type"]		=	intval($row->i_user_type); 
...
{
	$s_qry	= "DELETE FROM ".$this->tbl." WHERE s_email= '".$email."'";
	$this->db->query($s_qry); 
}
    ...
    */
	...
	public function update_subscription($info = array(), $where = array())
	{
		try
		{
			if(!empty($info) && !empty($where))
			{
				$s_qry = $this->db->update_string($this->tbl, $info, $where);
				return ($this->db->simple_query($s_qry))? TRUE : FALSE ;
			}
			return FALSE;
		}
		catch(Exception $err_obj)
...
public function add_newsletter_subscription($info)
	{
...
public function check_email_exist($email)
{
	$s_query="SELECT i_id 
						 FROM ".$this->tbl."
						 WHERE s_email='".my_receive($email)."'";
	$rs=$this->db->query($s_query);
	return ($rs->result_array());
}

public function get_mail_id_for_store($id)
{
	$s_qry="SELECT s_email FROM ".$this->tbl." WHERE i_store_id=".$id;
	$rs=$this->db->query($s_qry);
	return ($rs->result_array());
}


public function get_all_mail_id()
{
	$s_qry="SELECT s_email FROM ".$this->tbl;
	$rs=$this->db->query($s_qry);
	return ($rs->result_array());
}


public function gettotal_info_subscribed()
{
	$s_qry="SELECT COUNT(*) As total FROM ".$this->tbl." WHERE i_subscribe_status=1";
	$rs=$this->db->query($s_qry);
	
	return ($rs->result_array());
}

public function gettotal_info_subscribed_stored()
{
	$s_qry="SELECT COUNT(*) As total_store FROM ".$this->tbl." WHERE i_subscribe_status=1 AND i_store_id!=0 AND i_del_status=1";
	$rs=$this->db->query($s_qry);
...
public function subscribe_for_good_deals($info)
    {
        try
        { //print_r ($info);exit;
             $sql = $this->db->insert_string($this->tbl,$info);
...
//-------------------------------For store----------------------------  
public function check_email_exist_for_store($email,$store_id)
{
	$s_query="SELECT i_id 
						 FROM ".$this->tbl."
						 WHERE s_email='".my_receive($email)."' 
