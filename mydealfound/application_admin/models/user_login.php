...
class User_login extends MY_Model implements InfModel
{
    ...
    public function __construct()
    {
        try
        {
		  ...
		  $this->tbl_college_user=$this->db->USER;  
    ...
    */
	...
	public function college_user_login($login_data)
    {
        ...
        try
        {
          $ret_=array();
          ////Using Prepared Statement///
          $s_qry="SELECT i_id,s_username,s_email,i_college_id FROM ".$this->tbl_college_user." 
		  		WHERE s_username=?";
          $s_qry.=" And s_password=? ";
          
          $stmt_val["s_username"]= trim($login_data["s_username"]);
          /////Added the salt value with the password///
          ...
          if($rs->num_rows()>0)
          {
              ...
              {
                  $ret_["id"]			=	$row->i_id;////always integer
                  $ret_["s_username"]	=	stripslashes($row->s_username); 
                  $ret_["s_email"]		=	trim($row->s_email);                    
                  $ret_["i_college_id"]	=	intval($row->i_college_id); 
                  
                  ////////saving logged in user data into session////
                  $this->session->set_userdata(array(
                                                    "college_loggedin"=> array(
                                                    "user_id"=> encrypt(intval($ret_["id"])),
													"user_college_id"=> intval($ret_["i_college_id"]),
                                                    "user_name"=> $ret_["s_username"],
                                                    "user_email"=> $ret_["s_email"]    
                                                )));
                  ////////end saving logged in user data into session////
                  //////////log report///
                    $logi["msg"]="Logged in as ".$ret_["user_name"]." ";
                    //$logi["sql"]= serialize(array($s_qry) ) ;
                    //$logi["user_id"]=$ret_["id"];///Loggedin User Id                                 
                    $this->log_info($logi); 
                    unset($logi);  
