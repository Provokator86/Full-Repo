                        	...
                        	<div class="det_offer">
                                ...
                                <div class="offer">
                                  ...
                                  <?php /*?>  <span class="image_like"><iframe src="//www.facebook.com/plugins/like.php?href=<?php echo rawurlencode(base_url().'product_land/index/'.$val["s_url"].'/'.$val['s_seo_url']);?>&amp;send=false&amp;layout=button_count&amp;width=450&amp;show_faces=false&amp;font&amp;colorscheme=light&amp;action=like&amp;height=21" scrolling="no" frameborder="0" style="border:none; overflow:hidden; width:72px; height:21px;" allowTransparency="true"></iframe></span><?php */?><div class="clear"></div>
                                    ...
                                    <div class="get_code">
                                   <?php /*?> <a id="copy" target="_blank" href="<?php echo make_valid_url($val['cpn_url'])?>"  onclick="open_coupn_details_page('<?php echo $val['id']?>','<?php echo $val['s_url']?>','<?php echo $val['s_seo_url']?>');"><?php if($val['i_coupon_type']==2) { echo 'Click here to view code';}else 
									{echo 'Click to activate Deal';}?></a><?php */?>
                                    
                                    <span class="coupon_code" id="coupon_code<?php echo $val['id']?>" onclick="open_coupn_details_page('<?php echo $val['id']?>','<?php echo base64_encode($val['s_url'])?>','<?php echo base64_encode($val['s_seo_url'])?>','<?php echo base64_encode($val['cpn_url'])?>');">
										<?php if($val['i_coupon_type']==1){echo "Activate Deal";}else {echo "View Code";} ?>
									</span>
                                    </div>
                                    <a class="offer_code" href="#" id="code_paste<?php echo $val['id'] ?>"></a>
                                    <input type="hidden" value="<?php echo $val['i_coupon_code']; ?>" id="copycode_coupon_code<?php echo $val['id']?>" />
                                    
                                    <?php      if($val['i_coupon_type']==2)
                                    { ?>
                                    <div id="tooltip<?php echo $val['id'] ?>" class="tooltip-2" style="float:right;display:none">
                                            <div class="tooltip1-2">&nbsp;</div>
                                            <div class="tooltip2-2" id="copied_coupon_code<?php echo $val['id']?>">
