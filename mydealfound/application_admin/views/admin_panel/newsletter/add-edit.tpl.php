...
$(document).ready(function(){
	var j=$('#i_store_id').val();
	//alert(j);
	if(j=='k')
		{
			$('#i_send_to').css({display:''});
		}
		else
		{
			$('#i_send_to').css({display:'none'});
...
$('input[id^="btn_send"]').each(function(i){
   $(this).click(function(){
       $.blockUI({ message: 'Just a moment please...' });
	   
	   var formUrl = '<?php echo base_url() ?>admin_panel/newsletter/send';
	   $("#frm_add_edit").attr('action',formUrl)
       $("#frm_add_edit").submit();
...
<input type="hidden" id="h_id" name="h_id" value="<?php echo $posted["h_id"];?>"> 
    ...
    <div class="info_box">From here Admin can create and save the newsletters of the site.</div>
    <p>&nbsp;</p>
        <div id="div_err">
            <?php
              show_msg("error");  
    ...
    <div>
      <table width="100%" border="0" cellspacing="0" cellpadding="0">
        ...
        <tr>
          ...
          <td width="51%"><select name="i_store_id" id="i_store_id"><option value="k">Select</option>
          <?php echo makeOptionNoEncrypt($store,$posted['i_store_id']);?>
          </select></td>
          <td width="14%">&nbsp;</td>
        </tr>
        
         <tr id="i_send_to" style="display:none">
          <td width="35%">Send To:</td>
          <td width="51%">All<input type="radio" name="i_send_to" id="all" value="1" <?php if($posted['i_send_to']==1) echo 'checked="checked"'?>/>
          				 Mailing List<input type="radio" name="i_send_to" id="mail_list" value="2" <?php if($posted['i_send_to']==2) echo 'checked="checked"'?>/></td>
          <td width="14%">&nbsp;</td>
        </tr>
        
        
        <tr id="s_file" style="display:none">
          <td width="35%">Upload File:</td>
          <td width="51%"><input type="file" name="s_file" /></td>
          <td width="14%">&nbsp;</td>
        </tr>
		...
		<tr>
			<td></td>
			<td><input id="i_all" name="i_all" value="3" <?php if($posted["i_all"]==3) echo 'checked="checked"'; else echo '';?>  type="hidden" /></td>
			<td>&nbsp;</td>
		</tr>
		...
		<tr>
			<td></td>
			<td><input name="i_user" id="child_box2" value="1" <?php if($posted["i_user"]==1) echo 'checked="checked"'; else echo '';?> type="hidden" class="child_box" onclick="check()"/></td>
			<td>&nbsp;</td>
		</tr>
		
		<tr>
			<td> </td>
			<td><input name="i_general" id="child_box3" value="2" <?php if($posted["i_general"]==2) echo 'checked="checked"'; else echo '';?>  type="hidden"  class="child_box" onclick="check()"/></td>
			<td>&nbsp;</td>
		</tr>
		<tr>
    ...
    <div class="left">
      <input id="btn_send" name="btn_send" type="button" value="Send Now" title="Click here to save information." /> 
