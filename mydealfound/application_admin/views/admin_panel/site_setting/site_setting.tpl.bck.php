...
/*	
	...
	if($.trim($("#i_num_category").val())=="") 
    {
        s_err +='Please provide categories.<br />';
        b_valid=false;
    }
	else if(isNaN($.trim($("#i_num_category").val())))
	{
		s_err +='Categories will be numeric value.<br />';
        b_valid=false;
	}
	
	if($.trim($("#i_discount_rate1").val())=="") 
    {
        s_err +='Please provide discount of categories.<br />';
        b_valid=false;
    }
	else if(isNaN($.trim($("#i_discount_rate1").val())))
	{
		s_err +='Discount of categories will be numeric value.<br />';
        b_valid=false;
	}
	
	if($.trim($("#i_num_story").val())=="") 
    {
        s_err +='Please provide stories.<br />';
        b_valid=false;
    }
	else if(isNaN($.trim($("#i_num_story").val())))
	{
		s_err +='Stories will be numeric value.<br />';
        b_valid=false;
	}
	
	if($.trim($("#i_discount_rate2").val())=="") 
    {
        s_err +='Please provide discount of stories.<br />';
        b_valid=false;
    }
	else if(isNaN($.trim($("#i_discount_rate2").val())))
	{
		s_err +='Discount of stories will be numeric value.<br />';
        b_valid=false;
	}
	...
	if($.trim($("#dd_default_currency").val())=="") 
    {
        s_err +='Please select default currency.<br />';
        b_valid=false;
    }*/ 
		...
		<form id="frm_add_edit" name="frm_add_edit" method="post" action="" enctype="multipart/form-data">
			...
			<div id="myaccount">
				<table width="100%" border="0" cellspacing="0" cellpadding="0" >
				  ...
				  <tr>
					<td>&nbsp;</td>
					<td colspan="2" class="caption_txt">From here Admin can set the number of records to be displayed per page in admin interface.</td>
				  </tr>
					 <?php /*?><tr>
				  ...
				  <tr>
						<td valign="top">Default Currency  *: </td>
						<td align="left">

						<select name="dd_default_currency" id="dd_default_currency">
						<option value="">Select Currency</option>
						<?php echo makeOptionCurrency(' ',$posted["i_default_currency"]);?>
						</select>
						</td>
						<td>&nbsp;</td>
					</tr>     
					<tr>
						<td>&nbsp;</td>
						<td colspan="2" class="caption_txt">From here Admin can set the default currency. </td>
				   </tr> <?php */?>
				</table>
			</div>
			<? /*****end Modify Section*******/?>      
