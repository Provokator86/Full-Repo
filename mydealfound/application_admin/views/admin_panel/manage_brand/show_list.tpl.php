...
function hot_status_check(coupon_id)
{
	jQuery.ajax({
					type: "POST",
					url: "<?php echo base_url().'admin_panel/manage_brand/get_coupon_hot_status/'?>",
					data: "current_coupon_id="+escape(coupon_id),
					success: function(status)
					{
						if(status==0)
						{
							alert('Inactive store cannot be check as Favourite Brand');
							location.reload();
						}
						else
						{
							...
							if(status[0]<12)
							{
								...
								if(status[1] ==1)
								{
									...
									alert ('The brand is no more a Favourite Brand.');
									
								}
								else
								{
									change_status(coupon_id, status[1]);
									alert('You have make this brand as Favourite BRAND');
									
								}
							}
							else
							{	
								if(status[1] ==1 && status[0]==12)
								{
									change_status(coupon_id, status[1]);
									alert ('The brand is no more a Favourite Brand.');
								}
								else{
								alert ('Maximum 12 brands can be set as Favourite Brand.');
								location.reload();
								}
...
function change_status(coupon_id, status)
{
	jQuery.ajax({
		type: "POST",
		url: "<?php echo base_url().'admin_panel/manage_brand/change_hot_status/'?>",
...
<div id="right_panel">
	...
	<div class="info_box">From here Admin can manage the brands.</div>
	<div class="clr"></div>
      ...
      <form id="frm_search_2" name="frm_search_2" method="post" action="" >
        ...
        <table width="100%" border="0" cellspacing="0" cellpadding="0">
          <tr>
            ...
            <td width="23%">
            <input id="s_brand" name="s_brand" value="<?php echo $s_brand;?>" type="text" size="28" />
            </td>
            <td colspan="4">
            </td>
          </tr>
