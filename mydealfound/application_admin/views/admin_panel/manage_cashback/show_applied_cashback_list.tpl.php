...
<script type="text/javascript" language="javascript" >
    function saveAmount(paramObj){
        $cashBackOldValue = parseFloat($(paramObj).val());
        $cashBackNewValue = parseFloat(prompt('Please Confirm The Amount,Press "Ok" To Save',$cashBackOldValue));
        if($cashBackNewValue == 'NaN'){
            alert('Your Entry Is Invalid');
        }
        if($cashBackNewValue!=$cashBackOldValue){
            $.post('<? base_url() ?>admin_panel/manage_cashback/apply_cashback',
            {'user_deals_ID':$(paramObj).attr('name'),'d_cashback_amount':$cashBackNewValue},function(respData){
            ...
            <div id="div2" <?php //echo ($h_search=="advanced"?"":'style="display: none;"')  ?> >
                <form id="frm_search_2" name="frm_search_2" method="post" action="" >
                    ...
                    <table width="100%" border="0" cellspacing="0" cellpadding="0">
                        <tr>
                            <td width="15%">User Name/Email/UID: </td>
                            <td width="10%">
                                <input id="s_uid" name="s_uid" value="<?php echo $s_uid; ?>" type="text" size="28" />
                            </td>
                            <td width="15%">Status: </td>
                            <td width="10%">
                                <select name="i_active">
                                    <option value="">select</option>
                                    <option value="1" <?php if ($i_active == '1') echo 'selected=selected' ?>>Active</option>
                                    <option value="0" <?php if ($i_active . 'a' == '0a') echo 'selected=selected' ?>>Inactive</option>
                                </select>
                            </td>
                            <td colspan="4">
                            </td>
