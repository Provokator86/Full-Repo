...
class City extends My_Controller implements InfController
{
    ...
    */
    public function show_list($order_name='',$order_by='asc',$start=NULL,$limit=NULL)
    {
        try
        {
            ...
            $s_city	  	=($this->input->post("h_search")?$this->input->post("txt_city"):$this->session->userdata("array_list_search","txt_city"));  
			$s_state  	=($this->input->post("h_search")?$this->input->post("opt_state"):$this->session->userdata("array_list_search","opt_state"));
			$s_country  =($this->input->post("h_search")?$this->input->post("opt_country"):$this->session->userdata("array_list_search","opt_country"));    
			$s_top_destination  =($this->input->post("h_search")?$this->input->post("opt_destination"):$this->session->userdata("array_list_search","opt_destination"));        
            ...
            if($s_search=="advanced")
            {
				if($s_city)
				{
                	$s_where.=" AND c.s_city LIKE '%".get_formatted_string($s_city)."%' ";
				}
				if(decrypt($s_country))
				{
                	$s_where.=" AND c.i_country_id =".decrypt($s_country)." ";
				}
				if(decrypt($s_state))
				{
                	$s_where.=" AND c.i_state_id =".decrypt($s_state)." ";
				}
				if($s_top_destination)
				{
                	$s_where.=" AND c.i_top_destination =".$s_top_destination." ";
				}

                /////Storing search values into session///
				$array_list_search = array('txt_city'          => $s_city,
										   'opt_country' 	   => $s_country,
										   'opt_state'   	   => $s_state,
										   'opt_destination'   => $s_top_destination	
										  );
                $this->session->set_userdata("array_list_search");
                $this->session->set_userdata("h_search",$s_search);
                
                $this->data["h_search"]			= $s_search;
                $this->data["txt_city"]			= $s_city;  
				$this->data["opt_country"]		= $s_country; 
				$this->data["opt_state"]		= $s_state;
				$this->data["opt_destination"]	= $s_top_destination;          
            ...
            else////List all records, **not done
            {
				...
				$array_list_search = array('txt_city'    => '',
										   'opt_country' => '',
										   'opt_state'   => ''	,
										   'opt_destination'   => ''
										  );
            ...
            for($i=0; $i<$table_view["total_rows"]; $i++)
            {
				...
				if($info[$i]["i_top_destination"] == 1)
				{
					$action .='<a  href="javascript:void(0);" id="destination_img_id_'.encrypt($info[$i]["id"]).'_not"><img width="12" height="12" title="Remove top destination" alt="Remove top destination" src="images/admin/remove_mark.png"></a>';
				}
				else
				{
				   
					 $action .='<a  href="javascript:void(0);" id="destination_img_id_'.encrypt($info[$i]["id"]).'_yes"><img width="12" height="12" alt="Mark top destination" title="Mark top destination" src="images/admin/add_mark.png"></a>';
				}
				/* end mark as top destination */
    ...
    */
	...
	/***
    * Change status of the city as marking/removing as top destination
    * @author Mrinmoy 
    */
    public function ajax_change_destination_status()
    {
        try
        {
            $posted				= array();
			...
			//pr($posted,1);
			$info 				= array();
			$info['i_top_destination']   = $posted["i_status"];
			$arr_where          = array('i_id'=>$posted["id"]);
			$i_rect				= $this->mod_common->common_edit_info($this->db->CITY,$info,$arr_where); /*don't change*/                
			if($i_rect)////saved successfully
			{
