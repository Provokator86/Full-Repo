    ...
    */
    public function show_list($order_name='',$order_by='desc',$start=NULL,$limit=NULL)
    {
        try
        {
            ...
            $search_variable["s_question"]	= ($this->input->post("h_search")?$this->input->post("txt_question"):$arr_session_data["txt_question"]);
			$search_variable["d_date_on"]	= ($this->input->post("h_search")?$this->input->post("txt_created_on"):$arr_session_data["txt_created_on"]);
            ...
            {
                if(trim($search_variable["s_question"]))
                {
                    $s_where.=" AND n.s_question LIKE '%".get_formatted_string($search_variable["s_question"])."%' ";
                }    
                if(trim($search_variable["d_date_on"])!="")
                {
                    $dt_start=date("Y-m-d",strtotime(trim($search_variable["d_date_on"]." "))) ; 
                ...
                $arr_session["txt_question"]    =    $search_variable["s_question"];
                $arr_session["txt_created_on"]	=    $search_variable["d_date_on"];
                ...
                $this->data["txt_question"]    	=    get_unformatted_string($search_variable["s_question"]);              
                $this->data["txt_created_on"]	=    $search_variable["d_date_on"];             
            ...
            else////List all records, **not done
            {
                ...
                $this->data["h_search"]		= $s_search;
                $this->data["txt_question"]	=    "";
                $this->data["txt_date_to"]	=    "";                   
                /////end Storing search values into session///                 
                
            }
            unset($s_search,$arr_session);
            ...
            $arr_sort = array(2=>'dt_created_on',3=>'i_status'); 
    ...
    public function add_information($type='')           
    {
		...
		try
        {
            ...
            if($_POST)
            {
                ...
                $posted["ta_answer"]	= trim($this->input->post("ta_answer"));
                $posted["h_mode"]		= $this->data['mode'];
                $posted["h_id"]			= "";    

                $this->form_validation->set_rules('txt_question', 'question', 'required');
                $this->form_validation->set_rules('ta_answer', 'answer', 'required');
                ...
                else///validated, now save into DB
                {
                    ...
                    $info["s_answer"]           =    $posted["ta_answer"];
    ...
    public function modify_information($i_id=0)
    {
        ...
        try
        {
            ...
            else
            {
				...
				$posted["ta_answer"]		= trim($info["s_answer"]);				
				$posted["h_id"]				= $i_id; 
                $this->data["posted"]		= $posted;  
