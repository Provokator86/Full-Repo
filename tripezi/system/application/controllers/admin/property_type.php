...
class Property_type extends My_Controller implements InfController
{
    ...
    public function __construct()
    {
        ...
        {
          parent::__construct();
          $this->data['title']="Property Type Management";////Browser Title

          ////////Define Errors Here//////
          $this->cls_msg = array();
          $this->cls_msg["no_result"]	=	"No information found about property type.";
          $this->cls_msg["save_err"]	=	"Information about property type failed to save.";
          $this->cls_msg["save_succ"]	=	"Information about property type saved successfully.";
          $this->cls_msg["delete_err"]	=	"Information about property type failed to remove.";
          $this->cls_msg["delete_succ"]	=	"Information about property type removed successfully.";
    ...
    */
    public function show_list($order_name='',$order_by='asc',$start=NULL,$limit=NULL)
    {
        try
        {
            $this->data['heading']="Property Type";////Package Name[@package] Panel Heading
            ...
            $s_where    =" WHERE p.i_status!=2 ";
           
            if($s_search=="advanced")
            {
                $s_where.=" AND p.s_name LIKE '%".get_formatted_string($s_title)."%' ";
			...
			$arr_sort = array(0=>'p.s_name'); 
			$s_order_name = !empty($order_name)?in_array(decrypt($order_name),$arr_sort)?decrypt($order_name):$arr_sort[0]:$arr_sort[0];
          
            $limit	= $this->i_admin_page_limit;
			$info	= $this->mod_rect->fetch_multi_sorted_property_type_list($s_where,$s_order_name,$order_by,intval($start),$limit);
			...
			$table_view["total_db_records"]		= $this->mod_rect->gettotal_property_type_info($s_where);
            ...
            $this->render('assets/property-type-show-list');          
    ...
    */
    public function add_information()           
    {
		...
		try
        {
            ...
            {
				$posted=array();
                $posted["txt_property"]		= trim($this->input->post("txt_property"));			
                $posted["h_mode"]			= $this->data['mode'];
                $posted["h_id"]= "";
				
				
                $this->form_validation->set_rules('txt_property', 'property type name', 'required');
                ...
                {
                    $info=array();
                    $info["s_name"]		= $posted["txt_property"];
					//pr($info,1);
                    $i_newid = $this->mod_rect->add_new_property_type($info);
            ...
            ////////////end Submitted Form///////////
            $this->render("assets/property-type-add-edit");
        }
        catch(Exception $err_obj)
        {
            show_error($err_obj->getMessage());
        ...
        try
        {
            ...
            if($_POST)
            {
                ...
                else///validated, now save into DB
                {
					...
					$s_tablename= $this->db->PROPERTYTYPE;
            ...
            {
                $info=$this->mod_rect->fetch_this_property_type(decrypt($i_id));				
                $posted=array();				
				
				$posted["txt_property"] = trim($info["s_name"]);
				$posted["h_id"]         = $i_id;
    ...
    */
    public function ajax_checkduplicate()
    {
        try
        {
            ...
            {
                $qry=" WHERE ".(intval($posted["id"])>0 ? " p.i_id!=".intval($posted["id"])." And " : "" )
                    ." p.s_name='".$posted["duplicate_value"]."'" ;
					
				
				$info=$this->mod_rect->gettotal_property_type_info($qry); /*don't change*/
    ...
    */
    public function ajax_change_status()
    {
        try
        {
			...
			$i_rect				= $this->mod_common->common_edit_info($this->db->PROPERTYTYPE,$info,$arr_where); /*don't change*/                
