...
class Bed_type extends My_Controller implements InfController
{
    ...
    public function __construct()
    {
        ...
        {
          parent::__construct();
          $this->data['title']="Bed Type Management";////Browser Title

          ////////Define Errors Here//////
          $this->cls_msg = array();
          $this->cls_msg["no_result"]	=	"No information found about bed type.";
          $this->cls_msg["save_err"]	=	"Information about bed type failed to save.";
          $this->cls_msg["save_succ"]	=	"Information about bed type saved successfully.";
          $this->cls_msg["delete_err"]	=	"Information about bed type failed to remove.";
          $this->cls_msg["delete_succ"]	=	"Information about bed type removed successfully.";
    ...
    */
    public function show_list($order_name='',$order_by='asc',$start=NULL,$limit=NULL)
    {
        try
        {
            $this->data['heading']="Bed Type";////Package Name[@package] Panel Heading
            ...
            $s_where    =" WHERE b.i_status!=2 ";
           
            if($s_search=="advanced")
            {
                $s_where.=" AND b.s_name LIKE '%".get_formatted_string($s_title)."%' ";
			...
			$arr_sort = array(0=>'b.s_name'); 
			$s_order_name = !empty($order_name)?in_array(decrypt($order_name),$arr_sort)?decrypt($order_name):$arr_sort[0]:$arr_sort[0];
          
            $limit	= $this->i_admin_page_limit;
			$info	= $this->mod_rect->fetch_multi_sorted_bed_type_list($s_where,$s_order_name,$order_by,intval($start),$limit);
			...
			$table_view["total_db_records"]		= $this->mod_rect->gettotal_bed_type_info($s_where);
            ...
            $this->render('assets/bed-type-show-list');          
    ...
    */
    public function add_information()           
    {
        ...
        {
            $this->data['title']="Bed Type Management";////Browser Title
            $this->data['heading']="Add bed type";
            ...
            {
				$posted=array();
                $posted["txt_bed"]		= trim($this->input->post("txt_bed"));			
                $posted["h_mode"]		= $this->data['mode'];
                $posted["h_id"]= "";
				
				
                $this->form_validation->set_rules('txt_bed', 'bed type name', 'required');
                ...
                {
                    $info=array();
                    $info["s_name"]		= $posted["txt_bed"];
					
					$s_tablename	=	$this->db->PROPERTYBEDTYPE;
            ...
            ////////////end Submitted Form///////////
            $this->render("assets/bed-type-add-edit");
        }
        catch(Exception $err_obj)
        {
            show_error($err_obj->getMessage());
    ...
    */      
    public function modify_information($i_id=0)
    {
        ...
        try
        {
            $this->data['title']="Edit Bed Type Details";////Browser Title
            $this->data['heading']="Edit Bed Type";
            $this->data['pathtoclass']=$this->pathtoclass;
            $this->data['mode']="edit";
            ...
            {
                $info=$this->mod_rect->fetch_this_bed_type(decrypt($i_id));				
                $posted=array();				
				
				$posted["txt_bed"] 		= trim($info["s_name"]);
				$posted["h_id"]         = $i_id;
    ...
    */
    public function ajax_checkduplicate()
    {
        try
        {
            ...
            {
                $qry=" WHERE ".(intval($posted["id"])>0 ? " b.i_id!=".intval($posted["id"])." And " : "" )
                    ." b.s_name='".$posted["duplicate_value"]."'" ;
					
				
				$info=$this->mod_rect->gettotal_bed_type_info($qry); /*don't change*/
    ...
    */
    public function ajax_change_status()
    {
        try
        {
			...
			$i_rect				= $this->mod_common->common_edit_info($this->db->PROPERTYBEDTYPE,$info,$arr_where); /*don't change*/                
