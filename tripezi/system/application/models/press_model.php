...
* @link InfModel.php 
* @link MY_Model.php
* @link controllers/press.php
* @link views/admin/press/
*/

class Press_model extends MY_Model implements InfModel
{
    private $conf;
    ...
    */
    public function fetch_multi($s_where=null,$i_start=null,$i_limit=null)
    {
        ...
        {
          $ret_=array();
          $s_qry="SELECT p.* FROM ".$this->tbl." p "
                .($s_where!=""?$s_where:"" )." ORDER BY p.dt_created_on DESC "
        ...
        try
        {
          ...
          if($rs->num_rows()>0)
          {
              foreach($rs->result() as $row)
              {
                  $ret_[$i_cnt]["id"]               = $row->i_id;////always integer
                  $ret_[$i_cnt]["s_title"]          = get_unformatted_string($row->s_title);                   
                  $s_desc                           = nl2br(stripslashes($row->s_description));
        ...
        try
        {
          ...
          if($rs->num_rows()>0)
          {
              foreach($rs->result() as $row)
              {
                  ...
                  $s_desc                   = stripslashes($row->s_description);
                  $ret_["s_desc_full"]      = $s_desc;  
                  if(strlen($s_desc)>50)
                      $s_desc = substr_replace($s_desc,'...',50);
                  $ret_["s_description"]    =     $s_desc ;                   
