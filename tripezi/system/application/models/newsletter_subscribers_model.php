        ...
        try
        {
          ...
          if($rs->num_rows()>0)
          {
              foreach($rs->result() as $row)
              {
                  ...
                  $ret_[$i_cnt]["dt_created_on"]	=	($row->dt_created_on)?date($this->conf["site_date_format"],intval($row->dt_created_on)):''; 
				
                  $ret_[$i_cnt]["i_failure_count "]	=	intval($row->i_failure_count ); 
            ...
            if(!empty($info))
            {
                ...
                $s_qry.=" Where i_id=? ";
                 ...
                 $i_ret_= $this->db->query($s_qry,array(
												  get_unformatted_string($info["s_name"]),
												  get_unformatted_string($info["s_email"]),
												  intval($i_id)
