...
class Property_model extends MY_Model implements InfModel
{
    ...
    */
		 ...
		 //echo '<br>'.$this->db->last_query();
          ...
          if($rs->num_rows()>0)
          {
              foreach($rs->result() as $row)
              {
				  ...
				  // gettotal_booking_of_a_property
				  $s_where = " WHERE i_property_id=".$ret_[$i_cnt]["id"]." AND e_status='Amount paid' ";
				  $ret_[$i_cnt]["i_total_booking"]	= $this->gettotal_booking_of_a_property($s_where);
        ...
        try
        {
          ...
          if($rs->num_rows()>0)
          {
              foreach($rs->result() as $row)
              {
				  ...
				  $ret_[$i_cnt]["i_show_homepage"]          = intval($row->i_show_homepage);
	...
	public function fetch_properties_on_search($s_where=null,$s_order='',$i_start=null,$i_limit=null,$login_id='')
    {
        ...
        {   
		   ...
		   $s_qry="SELECT p.*,pt.s_name AS s_property_type,u.s_first_name,u.s_last_name,u.s_email,u.s_image,u.i_phone_verified,
			co.s_country,s.s_state,ci.s_city,new.d_currency_rate,new.o_price,
            ...
            ." LEFT JOIN ".$this->tbl_property_amenity." pa ON pa.i_property_id=p.i_id "
			." LEFT JOIN ".$this->tbl_property_type." pt ON pt.i_id = p.i_property_type_id "
			." LEFT JOIN (SELECT pp.i_id,ROUND(pp.d_standard_price/c.d_currency_rate,2) o_price,pp.d_standard_price,c.d_currency_rate FROM propertyspace_property pp 
			LEFT JOIN  propertyspace_currency  c ON pp.i_currency_id=c.i_id ) new ON new.i_id = p.i_id "
		 ...
		 //echo '<br>'.$this->db->last_query();
          ...
          if($rs->num_rows()>0)
          {
              foreach($rs->result() as $row)
              {
				  ...
				  // fetch total positive review to be verified host
				  $s_where = " WHERE r.i_property_owner_id=".$ret_[$i_cnt]["i_owner_id"]." AND r.i_rating = 5 ";
				  $ret_[$i_cnt]["i_total_positive_review"]	= $this->gettotal_review_count($s_where);
	...
	public function fetch_random_property_and_images($s_where=null,$i_start=null,$i_limit=null)
    {
        ...
        {   
              ...
              $ret_=array();
            $s_qry="SELECT p.i_id,p.s_property_name,p.e_accommodation_type,p.d_standard_price,p.i_currency_id  FROM ".$this->tbl." p "        
    ...
    */
	...
	/* this function count total book done for a property 
	*  author @ mrinmoy
	*/
	public function gettotal_booking_of_a_property($s_where=null)
    {
        try
        {
          $ret_=0;
           $s_qry="Select count(*) as i_total " ."From ".$this->tbl_booking." "
                .($s_where!=""?$s_where:"" );
    ...
    public function fetch_this($i_id)
    {
        try
        {
          ...
          ////Using Prepared Statement///
            $s_qry="SELECT p.*,u.s_first_name,u.s_last_name,u.s_email,u.s_phone_number,u.s_paypal_details,co.s_country,s.s_state,ci.s_city,
	...
	*/
	public function delete_property($i_id)
    {
        try
        {
		  $s_qry = "SELECT * FROM  ".$this->tbl_booking." WHERE i_property_id = ".$i_id." " ;        
    ...
    */
    public function fetch_booking_list($s_where=null,$i_start=null,$i_limit=null,$user_id='')
    {
        ...
        {   
            ...
            p.i_total_guests property_total_guests,p.i_currency_id,p.d_standard_price,
            p.d_weekly_price,p.d_monthly_price,p.i_owner_user_id,p.i_cancellation_policy_id,
            u.s_first_name,u.s_last_name,u.s_email,
            cu.d_currency_rate,cu.s_currency_symbol,us.s_first_name owner_first_name,us.s_last_name owner_last_name,us.s_email owner_email   
         ...
         //echo '</br>'.$this->db->last_query();
          ...
          if($rs->num_rows()>0)
          {
              ...
              foreach($rs->result() as $row)
              {
                  ...
                  $ret_[$i_cnt]["s_currency_symbol"]        = get_unformatted_string($row->s_currency_symbol);
                  $ret_[$i_cnt]["i_currency_id"]            = intval($row->i_currency_id);
				  $ret_[$i_cnt]["i_cancellation_policy_id"] = intval($row->i_cancellation_policy_id);
    ...
    public function fetch_property_info($property_id)
    {
        try
        {
            $ret_=array();
          ////Using Prepared Statement///
            $s_qry="SELECT p.*,u.s_first_name,u.s_last_name,u.s_email,po.d_cancellation_charge_percentage 
             FROM ".$this->tbl." p "
            ." LEFT JOIN ".$this->tbl_user." u ON p.i_owner_user_id=u.i_id "
            ." LEFT JOIN ".$this->tbl_cancellation_policy." po ON p.i_cancellation_policy_id=po.i_id "
            ." Where p.i_id=?";
          ...
          if($rs->num_rows()>0)
          {
              foreach($rs->result() as $row)
              {
                  ...
                  $ret_["i_maximum_night_stay"]     = intval($row->i_maximum_night_stay); 
                  $ret_["i_cancellation_policy_id"] = intval($row->i_cancellation_policy_id);
                  $ret_["d_cancellation_charge_percentage"] = $row->d_cancellation_charge_percentage;
	...
	*/
	...
	/* this function count total reviews with condition */
	public function gettotal_review_count($s_where=null)
    {
        try
        {
          $ret_=0;
          $s_qry="Select count(*) as i_total "
                ."From ".$this->tbl_review_rating." r "
                .($s_where!=""?$s_where:"" );
    ...
    *
    */
    public function fetch_booking_order_list($s_where=null,$order_name=null,$order_by=null,$i_start=null,$i_limit=null)
    {
        try
        {   
            ...
            cu.s_currency_symbol,cu.s_currency_code     
             FROM ".$this->tbl_booking." b "
            ." LEFT JOIN ".$this->tbl." p   ON b.i_property_id=p.i_id "
            ." LEFT JOIN ".$this->tbl_user." u  ON b.i_traveler_user_id=u.i_id "
            ." LEFT JOIN ".$this->tbl_user." us  ON p.i_owner_user_id=us.i_id "
         ...
         //echo '<br>'.$this->db->last_query();
          ...
          if($rs->num_rows()>0)
          {
              ...
              foreach($rs->result() as $row)
              {
                  ...
                  $ret_[$i_cnt]["t_booked_from"]            = intval($row->dt_booked_from);
                  $ret_[$i_cnt]["t_booked_to"]              = intval($row->dt_booked_to);
                  $ret_[$i_cnt]["dt_paid_on"]               = date($this->conf["site_date_format"],intval($row->dt_paid_on));
                  
                  $ret_[$i_cnt]["dt_cancellation_approve_date"]= date($this->conf["site_date_format"],intval($row->dt_cancellation_approve_date)); 
                  ...
                  $ret_[$i_cnt]["d_amount_paid"]            = $row->d_amount_paid;
                  $ret_[$i_cnt]["d_service_charge_amount"]  = $row->d_service_charge_amount;
                  $ret_[$i_cnt]["d_site_commission_amount"] = $row->d_site_commission_amount; 
                  $ret_[$i_cnt]["d_host_amount"]            = $row->d_host_amount;
                  $ret_[$i_cnt]["i_cancellation_id"]         = intval($row->i_cancellation_id);
                  $ret_[$i_cnt]["d_cancellation_percentage"] = $row->d_cancellation_percentage;
                  
                  $ret_[$i_cnt]['currency_charges'][1]      = $row->d_currency_rate_usd; 
                  $ret_[$i_cnt]['currency_charges'][2]      = $row->d_currency_rate_euro; 
                  $ret_[$i_cnt]['currency_charges'][3]      = $row->d_currency_rate_gbp; 
                  $ret_[$i_cnt]['i_currency_id']            = intval($row->i_currency_id);
                  $ret_[$i_cnt]["s_transaction_details"]    = get_unformatted_string($row->s_transaction_details);  
                  
                  $ret_[$i_cnt]["i_paid_to_host"]           = $row->i_paid_to_host;
                  $ret_[$i_cnt]["i_tax_withdrawn"]          = $row->i_tax_withdrawn;
                  $ret_[$i_cnt]["i_site_commision_withdrawn"]= $row->i_site_commision_withdrawn; 
                  ...
                  $ret_[$i_cnt]["i_total_guests"]           = intval($row->property_total_guests); // from property table
                  $ret_[$i_cnt]["d_standard_price"]         = $row->d_standard_price;
                  $ret_[$i_cnt]["d_weekly_price"]           = $row->d_weekly_price;
                  $ret_[$i_cnt]["d_monthly_price"]          = $row->d_monthly_price;
                  ...
                  $ret_[$i_cnt]["s_currency_symbol"]        = get_unformatted_string($row->s_currency_symbol); 
                  $ret_[$i_cnt]["s_currency_code"]          = get_unformatted_string($row->s_currency_code); 
        ...
        {   
          ...
          if($rs->num_rows()>0)
          {
              ...
              foreach($rs->result() as $row)
              {
                  ...
                  $ret_["d_amount_paid"]            = $row->d_amount_paid;
                  $ret_["d_service_charge_amount"]  = $row->d_service_charge_amount;
                  $ret_["d_site_commission_amount"] = $row->d_site_commission_amount;
                  $ret_["d_host_amount"]            = $row->d_host_amount;
	...
	*/
	public function fetch_favourite_property_list($s_where=null,$i_start=null,$i_limit=null)
    {
        ...
        {   
            ...
            $ret_=array();
            $s_qry="SELECT fp.i_user_id,fp.i_property_id,fp.dt_created_on,p.*,pt.s_name s_property_type,us.s_first_name AS owner_first_name,us.s_last_name AS owner_last_name,us.s_image,u.s_first_name,u.s_last_name,u.s_email,u.i_phone_verified,
    ...
    *
    */
    public function fetch_cancellation_order_list($s_where=null,$order_name=null,$order_by=null,$i_start=null,$i_limit=null)
    {
        try
        {   
            ...
            u.s_email,us.s_first_name owner_first_name,
            us.s_last_name owner_last_name,us.s_email owner_email,p.s_property_id s_property_id,
            cu.s_currency_symbol,cu.s_currency_code,cp.s_name cancellation_policy_name     
             FROM ".$this->tbl_booking." b "
            ." LEFT JOIN ".$this->tbl." p   ON b.i_property_id=p.i_id "
            ." LEFT JOIN ".$this->tbl_user." u  ON b.i_traveler_user_id=u.i_id "
            ." LEFT JOIN ".$this->tbl_user." us  ON p.i_owner_user_id=us.i_id "
            ." LEFT JOIN ".$this->tbl_currency." cu  ON b.i_currency_id=cu.i_id "
            ." LEFT JOIN ".$this->tbl_cancellation_policy." cp  ON b.i_cancellation_id=cp.i_id "
         ...
         //echo '<br>'.$this->db->last_query();
          ...
          if($rs->num_rows()>0)
          {
              ...
              foreach($rs->result() as $row)
              {
                  ...
                  $ret_[$i_cnt]["d_amount_paid"]            = $row->d_amount_paid;
                  $ret_[$i_cnt]["d_service_charge_amount"]  = $row->d_service_charge_amount;
                  $ret_[$i_cnt]["d_site_commission_amount"] = number_format(($row->d_site_commission_amount * $row->d_cancellation_percentage)/100,2); 
                  $ret_[$i_cnt]["d_host_amount"]            = number_format(($row->d_host_amount * $row->d_cancellation_percentage)/100,2);
                  
                  $ret_[$i_cnt]["d_refund_amount"]          = $row->d_refund_amount;
                  $ret_[$i_cnt]["d_cancellation_amount"]    = $row->d_cancellation_amount;
                  ...
                  $ret_[$i_cnt]["i_tax_withdrawn"]          = $row->i_tax_withdrawn;
                  $ret_[$i_cnt]["i_site_commision_withdrawn"]= $row->i_site_commision_withdrawn; 
                  $ret_[$i_cnt]["i_paid_to_traveler"]       = $row->i_paid_to_traveler;
                  ...
                  $ret_[$i_cnt]["cancellation_policy_name"] = get_unformatted_string($row->cancellation_policy_name); 
    ...
    /**
    * This function used in account controller booking_details function 
    * to fetch details   of booking by joining booking table with property and user table
    * 
    ...
    * @return int
    */
    public function fetch_booking_front_end($s_where)
    {
        try
        {   
         ...
         //echo '</br>'.$this->db->last_query();
          ...
          if($rs->num_rows()>0)
          {
              ...
              foreach($rs->result() as $row)
              {
                  ...
                  $ret_[$i_cnt]["d_currency_rate"]          = $row->d_currency_rate; 
                  $ret_[$i_cnt]["i_total_guests"]           = intval($row->property_total_guests);
                  $ret_[$i_cnt]["d_standard_price"]         = $row->d_standard_price/$ret_[$i_cnt]["d_currency_rate"];
                  $ret_[$i_cnt]["d_weekly_price"]           = $row->d_weekly_price/$ret_[$i_cnt]["d_currency_rate"];
                  $ret_[$i_cnt]["d_monthly_price"]          = $row->d_monthly_price/$ret_[$i_cnt]["d_currency_rate"];
                  $ret_[$i_cnt]["s_currency_symbol"]        = get_unformatted_string($row->s_currency_symbol);
                  $ret_[$i_cnt]["i_currency_id"]            = intval($row->i_currency_id);
                  $ret_[$i_cnt]["i_cancellation_policy_id"] = intval($row->i_cancellation_policy_id);
