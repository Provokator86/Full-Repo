        ...
        try
        {
          ...
          if($rs->num_rows()>0)
          {
              foreach($rs->result() as $row)
              {
                  ...
                  $ret_[$i_cnt]["s_answer"]		= stripslashes($row->s_answer); 
        ...
        try
        {
          ...
          if($rs->num_rows()>0)
          {
              foreach($rs->result() as $row)
              {
                  $ret_[$i_cnt]["id"]			= intval($row->i_id);////always integer
                  $ret_[$i_cnt]["s_question"]   = get_unformatted_string($row->s_question);                 
                  $s_desc                       = stripslashes($row->s_answer); 
        ...
        try
        {
          ...
          if($rs->num_rows()>0)
          {
              foreach($rs->result() as $row)
              {
                  ...
                  $ret_["s_question"]   = get_unformatted_string($row->s_question);
                  $ret_["s_answer"]		= strip_tags(get_unformatted_string($row->s_answer));   
