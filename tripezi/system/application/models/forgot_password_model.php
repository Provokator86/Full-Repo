...
class Forgot_password_model extends MY_Model implements InfModel
{
    ...
    */
	...
	public function fetch_name($s_where=null)
	{
		 try
        {
          	$s_qry = "SELECT s_first_name,s_last_name,s_user_name FROM ".$this->db->ADMIN." ".$s_where;	          
			$rs=$this->db->query($s_qry);
			if(is_array($rs->result()))
			{
			  foreach($rs->result() as $row)
			  {				 
				  $ret_["s_username"]=get_unformatted_string($row->s_user_name); 
			  }    
			  $rs->free_result();          
			}
    ...
    public function change_password($s_where=null,$org_changed_pass)
    {
        try
        {
		  ...
		  ////Using Prepared Statement///			
			$s_qry = "UPDATE ".$this->db->ADMIN." SET s_password='".$changed_pass."'  ".$s_where;				
    ...
    */
    public function change_password_front_user($s_where=null,$org_changed_pass)
    {
        try
        {
		  ...
		  $changed_pass	= 	md5($org_changed_pass.$this->conf["security_salt"]);
		  ////Using Prepared Statement///			
			$s_qry = "UPDATE ".$this->db->MST_USER." SET s_password='".$changed_pass."'  ".$s_where;				
                $this->db->trans_begin();///new  
                $this->db->query($s_qry);
                $i_ret_=$this->db->affected_rows();        
                if($i_ret_)
	...
	public function fetch_name_front_user($s_where=null)
	{
		 try
        {
          	$s_qry = "SELECT s_name,s_username FROM ".$this->db->MST_USER." ".$s_where;	          
			$rs=$this->db->query($s_qry);
			if(is_array($rs->result()))
			{
			  foreach($rs->result() as $row)
	...
	public function fetch_front_user_detail($s_where=null)
	{
		 try
        {
          	$s_qry = "SELECT s_name,s_username,i_signup_lang FROM ".$this->db->MST_USER." ".$s_where;	          
			$rs=$this->db->query($s_qry);
			if(is_array($rs->result()))
			{
			  foreach($rs->result() as $row)
