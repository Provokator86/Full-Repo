...
* @link InfModel.php 
* @link MY_Model.php
* @link controllers/currency.php
* @link views/admin/currency/
*/


class Currency_model extends MY_Model implements InfModel
{
    private $conf;
          ...
          if($rs->num_rows()>0)
          {
              foreach($rs->result() as $row)
              {
                 $ret_[$i_cnt]["id"]               =    $row->i_id;////always integer
                  ...
                  $ret_[$i_cnt]["i_default"]        =    intval($row->i_is_default); 
                  $ret_[$i_cnt]["s_default"]        =    (intval($row->i_is_default)==1?"Yes":"No");
                  $ret_[$i_cnt]["d_currency_rate"]    =    doubleval($row->d_currency_rate); 
    ...
    */
    public function fetch_this($i_id)
    {
        try
        {
          ...
          $s_qry="SELECT * FROM ".$this->tbl." AS n  "
                ." WHERE n.i_id=?";
          ...
          if($rs->num_rows()>0)
          {
              foreach($rs->result() as $row)
              {
				  ...
				  $ret_["d_currency_rate"]	=	doubleval($row->d_currency_rate); 				  
				  $ret_["i_default"]		=	intval($row->i_is_default); 
				  $ret_["s_default"]		=	(intval($row->i_is_default)==1?"Yes":"No");
    ...
    public function add_info($info)
    {
        try
        {
            ...
            if(!empty($info))
            {
                ...
                $s_qry.=" s_currency_code=? ";
				$s_qry.=" ,s_currency_symbol=? ";
				$s_qry.=" ,d_currency_rate=? ";
                
                $this->db->query($s_qry,array(
												  get_formatted_string($info["s_currency_code"]),
												  get_formatted_string($info["s_currency_symbol"]),
												  doubleval($info['d_currency_rate'])
												 ));
