...
jQuery(function($){
     $(document).ready(function() {   
           $(".strong li").css("font-weight","bold"); // if not read
     });
})  ;
</script>  
<!--tab 1-->
<?php
    if(!empty($message_list))
    {
        ...
        {
            if($val['i_sender_read']==0)
            {
                $strong   =   'class="strong" onclick="changeReadStatus(\''.encrypt($val['id']).'\',\'sender\',this);"';
            }
            else
            {
                $strong   =   '';
    ...
    <ul <?php echo $strong ;?> >
    <li class="box01">To:</li>
    <li class="box02"><?php echo $val['receiver_first_name'].' '.$val['receiver_last_name']; ?></strong></li>
    <li class="box03"><?php echo $val['s_subject']; ?></li>
    <li class="box04"><?php echo $val['dt_date_send']; ?></li>
    <li class="box05">
    <a href="javascript:void(0);" onclick="move_to_trash('<?php echo  encrypt($val['id']); ?>','sender');"><img src="images/fe/del.png" alt="del"  /></a>
