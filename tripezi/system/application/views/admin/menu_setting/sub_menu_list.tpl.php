...
$("#btn_submit").click(function(){
    ...
    {
        if($.trim($("#txt_menu_link").val())=='' || $.trim($("#txt_menu_link").val())=='Enter menu link' )
        {
            $("#txt_menu_link").val('Enter menu link');
...
$("a[id^=edit_row_]").click(function(){
    ...
    obj_tr.find('td:first').find('span').text('');
    obj_tr.find('td:first').find('input').show();
    if(obj_tr.find('td:eq(1)').find('span').text()!='')
    {
        obj_tr.find('td:eq(1)').find('span').text('');
        obj_tr.find('td:eq(1)').find('input').show();
    }
    $(this).hide().next().show();
});
...
$("a[id^=save_row_]").click(function(){
    var row_id  =   $(this).attr('id').split('_').pop();
    var obj_td  =   $(this).parent().parent();
    var edited_data =   $(obj_td).find('td:eq(0)').find('input').val();
    var link_data   =   $(obj_td).find('td:eq(1)').find('input').val();
    
   $.ajax({
                type: "POST",
                url: base_url+'admin/menu_setting/ajax_edit_main_menu',
                data: "edited_data="+edited_data+"&link_data="+link_data+"&row_id="+row_id,
                success: function(msg){
                    if(msg=='ok')
                    {
                         $(obj_td).find('td:eq(0)').find('input').hide();
                         $(obj_td).find('td:eq(0)').find('span').text(edited_data);
                         $(obj_td).find('td:eq(1)').find('input').hide();
                         $(obj_td).find('td:eq(1)').find('span').text(link_data);
                    }
...
$('input[type=radio]').click(function(){
   if($(this).index())
   {
       $("#hidden_row").find('td:eq(2)').show();  
       $("#hidden_row").find('td:eq(3)').show();  
   } 
   else
   {
       $("#hidden_row").find('td:eq(2)').hide();
       $("#hidden_row").find('td:eq(3)').hide();
   }
});
            ...
            buttons: {
                'Delete': function() {
                    ...
                    $.blockUI({ message: 'Just a moment please...' });
                     ...
                     $.ajax({
                    type: "POST",
                    url: base_url+'admin/menu_setting/ajax_delete_sub_menu',
                    data: "row_id="+row_id,
                    success: function(msg){
                        if(msg=='ok')
                        {  
...
<div id="right_panel">
	...
	<div class="info_box">Here we set Sub menu For the Project .</div>
	<div class="clr"></div>
        ...
        <table width="100%" border="0" cellspacing="0" cellpadding="0">
          ...
          <tr>
            ...
            <td >
                ...
                <select name="opt_status" id="opt_status" style="width: 150px;">
            ...
            <td colspan="2">
                <input type="radio" name="radio_label" value="1" checked="checked">&nbsp;&nbsp;Label 1 Sub menu
                <input type="radio" name="radio_label" value="2">&nbsp;&nbsp;Label 2 Sub Menu 
          ...
          <tr id="hidden_row">
            ...
            <td width="23%">
                ...
                <input id="txt_menu_link" name="txt_menu_link" value="" type="text" size="28" style="background: none;" />
            </td>
            
             <td width="12%" style="display: none;"> Sub Menus :</td>

            <td  style="display: none;" colspan="3" >
                <select name="opt_submenu" id="opt_submenu" style="width: 150px;">
                    <option value="">Select Submenu</option>
                    <?php echo makeOption($arr_menus)?>
                </select>
            </td>
          </tr>
