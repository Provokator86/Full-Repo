...
class CI_Image_lib {
	...
	var $x_axis				= '';
	var	$y_axis				= '';
    var $canvas_color       = null;
    var $canvas_width       = 0;
    var $canvas_height      = 0;
    var $do_not_upsize      = false;
	...
	function clear()
	{
		$props = array('source_folder', 'dest_folder', 'source_image', 'full_src_path', 'full_dst_path', 'new_image', 'image_type', 'size_str', 'quality', 'orig_width', 'orig_height', 'rotation_angle', 'x_axis', 'y_axis', 'create_fnc', 'copy_fnc', 'wm_overlay_path', 'wm_use_truetype', 'dynamic_output', 'wm_font_size', 'wm_text', 'wm_vrt_alignment', 'wm_hor_alignment', 'wm_padding', 'wm_hor_offset', 'wm_vrt_offset', 'wm_font_color', 'wm_use_drop_shadow', 'wm_shadow_color', 'wm_shadow_distance', 'wm_opacity','canvas_color','canvas_width','canvas_height','do_not_upsize');
	...
	function image_process_gd($action = 'resize')
	{
        ...
        if(
            $this->maintain_ratio
            && $this->canvas_color 
            && isset($this->canvas_color['red'])
            && isset($this->canvas_color['green'])
            && isset($this->canvas_color['blue'])
        )
        {
                $dst_img = $create($this->canvas_width, $this->canvas_height);
                
                
                $canvas_fill_color_Index    = imagecolorallocate($dst_img, $this->canvas_color['red'], $this->canvas_color['green'], $this->canvas_color['blue']);
                imagefill($dst_img, 0, 0, $canvas_fill_color_Index);
                $xxx=abs(floor(($this->canvas_width-$this->width)/2));
                $yyy=abs(floor(($this->canvas_height-$this->height)/2));
                $copy($dst_img, $src_img, $xxx, $yyy, $this->x_axis, $this->y_axis, $this->width, $this->height, $this->orig_width, $this->orig_height);
        }
        else
        {
	...
	function image_reproportion()
	{
        ...
        if($this->canvas_color && $this->do_not_upsize)
        {
            if( ($this->orig_width<=$this->width) && ($this->orig_height<=$this->height) )
            {
                $this->width=$this->orig_width+0.000001;
                $this->height=$this->orig_height+0.000001;
                return;
            }
