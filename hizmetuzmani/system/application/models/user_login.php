...
class User_login extends MY_Model implements InfModel
{
    ...
    public function __construct()
    {
        try
        {
		  ...
		  $this->tbl_frontend_user=$this->db->MST_USER;   
		  $this->tbl_language=$this->db->LANGUAGE;    
    ...
    */
	 ...
	 public function front_login($login_data)
    {
        ...
        try
        {
          $ret_=array();
          ////Using Prepared Statement///
          $s_qry="Select lang.s_short_name as signup_lang_prefix,id,s_username,s_name,s_email,i_role,i_is_active,i_signup_lang,
		  i_created_date,i_last_login_date From ".$this->tbl_frontend_user." LEFT JOIN ".$this->tbl_language." AS lang 
		  ON lang.i_id = i_signup_lang Where s_username=?";
          $s_qry.=" And s_password=? ";
          ...
          if($rs->num_rows()>0)
          {
              foreach($rs->result() as $row)
              {
                 ...
                 // $ret_["s_first_name"]=trim($row->s_first_name); 
				  ...
				  $ret_["signup_lang_prefix"]=stripslashes($row->signup_lang_prefix); 
                  if($row->i_last_login_date>0)
                  {
                      $ret_["i_last_login_date"]    =   date($this->conf["site_date_format"],intval($row->i_last_login_date)); 
                  }
                  else
                  {
                      $ret_["i_last_login_date"]    =   $row->i_last_login_date;
                  }
                  ...
                  $this->session->set_userdata(array(
													...
													"user_signup_lang"=> $ret_["i_signup_lang"],
													"signup_lang_prefix"=> $ret_["signup_lang_prefix"])     
                                                ));
                  ////////end saving logged in user data into session////
                  //////////log report///
