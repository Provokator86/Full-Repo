        ...
        try
        {
          ...
          if($rs->num_rows()>0)
          {
              foreach($rs->result() as $row)
              {
				  ...
				  $ret_[$i_cnt]["s_revisit_after"]	=	stripslashes(htmlspecialchars_decode($row->s_revisit_after)); 
				  $ret_[$i_cnt]["s_robots"]			=	stripslashes(htmlspecialchars_decode($row->s_robots)); 
				  $ret_[$i_cnt]["s_classifications"]=	stripslashes(htmlspecialchars_decode($row->s_classifications)); 
				  $ret_[$i_cnt]["s_expires"]		=	stripslashes(htmlspecialchars_decode($row->s_expires)); 
				  $ret_[$i_cnt]["s_google_site_verification"]=	stripslashes(htmlspecialchars_decode($row->s_google_site_verification)); 
        ...
        try
        {
          ...
          if($rs->num_rows()>0)
          {
              foreach($rs->result() as $row)
              {
				  ...
				  $ret_["s_description"]				=	stripslashes(htmlspecialchars_decode($row->s_description)); 
				  //$ret_["s_revisit_after"]				=	stripslashes(htmlspecialchars_decode($row->s_revisit_after)); 
				  //$ret_["s_robots"]						=	stripslashes(htmlspecialchars_decode($row->s_robots)); 
				  $ret_["s_language"]					=	stripslashes(htmlspecialchars_decode($row->s_language)); 
				  //$ret_["s_classifications"]			=	stripslashes(htmlspecialchars_decode($row->s_classifications)); 
				  //$ret_["s_expires"]					=	stripslashes(htmlspecialchars_decode($row->s_expires)); 
				  //$ret_["s_google_site_verification"]	=	stripslashes(htmlspecialchars_decode($row->s_google_site_verification)); 
    ...
    public function add_info($info)
    {
        try
        {
            ...
            if(!empty($info))
            {
				...
				$s_qry.=", s_keywords=? ";
                $s_qry.=", s_description=? ";
				//$s_qry.=", s_revisit_after=? ";
				//$s_qry.=", s_robots=? ";
				$s_qry.=", s_language=? ";
				//$s_qry.=", s_classifications=? ";
                //$s_qry.=", s_expires=? ";
				//$s_qry.=", s_google_site_verification=? ";
                $s_qry.=", i_is_active=? ";
                $s_qry.=", dt_entry_date=? ";
                ...
                $this->db->query($s_qry,array(
												  ...
												  //trim(htmlspecialchars($info["s_revisit_after"], ENT_QUOTES, 'utf-8')),
												  //trim(htmlspecialchars($info["s_robots"], ENT_QUOTES, 'utf-8')),
												  trim(htmlspecialchars($info["s_language"], ENT_QUOTES, 'utf-8')),
												 // trim(htmlspecialchars($info["s_classifications"], ENT_QUOTES, 'utf-8')),
												  //trim(htmlspecialchars($info["s_expires"], ENT_QUOTES, 'utf-8')),
												  //trim(htmlspecialchars($info["s_google_site_verification"], ENT_QUOTES, 'utf-8')),
