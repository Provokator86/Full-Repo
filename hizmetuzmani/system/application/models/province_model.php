...
* @link InfModel.php 
* @link MY_Model.php
* @link controllers/Province.php
* @link views/admin/Province/
*/


class Province_model extends MY_Model implements InfModel
{
    private $conf;
    ...
    */
    public function fetch_multi($s_where=null,$i_start=null,$i_limit=null)
    {
        ...
        {
          	$ret_=array();
			$s_qry="SELECT n.*,c.city FROM ".$this->tbl." n 
          ...
          if($rs->num_rows()>0)
          {
              foreach($rs->result() as $row)
              {
                  ...
                  $ret_[$i_cnt]["province"]     =stripslashes(html_entity_decode($row->province)); 
                  $ret_[$i_cnt]["city"]		    =stripslashes(html_entity_decode($row->city)); 
				  $ret_[$i_cnt]["i_city_id"]	=intval($row->i_city_id); 
	...
	public function fetch_multi_sorted_list($s_where=null,$order_name,$order_by,$i_start=null,$i_limit=null)
    {
        try
        {
          ...
          if($rs->num_rows()>0)
          {
              ...
              {
                  $ret_[$i_cnt]["id"]=$row->id;////always integer
                  $ret_[$i_cnt]["province"]		= stripslashes(htmlspecialchars_decode($row->province));
                  $ret_[$i_cnt]["city"]         =stripslashes(html_entity_decode($row->city));
				  $ret_[$i_cnt]["i_city_id"]    =intval($row->i_city_id);
                  $ret_[$i_cnt]["i_is_active"]	= intval($row->i_status); 
				  $ret_[$i_cnt]["s_is_active"]	= (intval($row->i_status)==1?"Active":"Inactive");
        ...
        try
        {
          ...
          if($rs->num_rows()>0)
          {
              foreach($rs->result() as $row)
              {
                  ...
                  //$ret_["province"]=	stripslashes(htmlspecialchars_decode($row->province)); 
				  $ret_["province"]     =	htmlspecialchars_decode($row->province);
                  $ret_["city"]         =   stripslashes(html_entity_decode($row->city)); 
        ...
        try
        {
            ...
            if(!empty($info))
            {
                ...
                $this->db->query($s_qry,array(
												  trim(htmlspecialchars($info["province"], ENT_QUOTES, 'utf-8')),
    ...
    public function edit_info($info,$i_id)
    {
        try
        {
            ...
            if(!empty($info))
            {
                ...
                $this->db->query($s_qry,array(trim(htmlspecialchars($info["province"], ENT_QUOTES, 'utf-8')),
                                                      intval($info["i_city_id"]),
                                                      //intval($info["i_status"]),
                                                      //intval($info["dt_entry_date"]),
													  intval($i_id)
                ...
                if($i_ret_)
                {
                    ...
                    $logi["sql"]= serialize(array($s_qry,array(trim(htmlspecialchars($info["province"], ENT_QUOTES, 'utf-8')),
                                                      intval($info["i_city_id"]),
                                                      //intval($info["i_status"]),
                                                      //intval($info["dt_entry_date"]),
													  intval($i_id)
    ...
    */
    ...
    function get_province_selectlist($id='')
    {
        try
        {
          $ret_=array();
          ////Using Prepared Statement///
          $s_qry="Select id,province "
                ."From ".$this->tbl." AS n "
                ." Where n.i_city_id=?"
    ." ORDER BY province";
