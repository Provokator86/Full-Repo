...
* @link InfModel.php 
* @link MY_Model.php
* @link controllers/membership_plan.php
* @link views/admin/membership_plan/
*/


class Membership_model extends MY_Model implements InfModel
{
    private $conf;
    ...
    public function __construct()
    {
        ...
        {
          parent::__construct();
          $this->tbl = $this->db->MEMBERSHIPPLAN;          
        ...
        try
        {
          ...
          if($rs->num_rows()>0)
          {
              foreach($rs->result() as $row)
              {
				  ...
				  $ret_[$i_cnt]["i_type"]			=	intval($row->i_type);
				  $ret_[$i_cnt]["s_type"]			=	$this->db->MEMBERPLAN[$row->i_type];
				  ...
				  $ret_[$i_cnt]["add_price"]		=	$row->d_additional_contact_price;				  
        ...
        try
        {
          ...
          if($rs->num_rows()>0)
          {
              foreach($rs->result() as $row)
              {
				  ...
				  $ret_["s_type"]			=	$this->db->MEMBERPLAN[$row->i_type];
				  ...
				  $ret_["add_price"]		=	$row->d_additional_contact_price;				  
    ...
    public function add_info($info)
    {
        try
        {
            ...
            if(!empty($info))
            {
				...
				$s_qry.=", d_price=? ";
				$s_qry.=", i_duration=? ";
				$s_qry.=", d_additional_contact_price=? ";
                $s_qry.=", dt_created_on=? ";
                
                $this->db->query($s_qry,array(
												  intval($info['i_type']),
												  intval($info['i_quotes']),
												  intval($info['i_contact_info']),
												  $info['d_price'],
												  intval($info['i_duration']),
												  $info['d_additional_contact_price'],
