...
class Cms_model extends MY_Model implements InfModel
{
        ...
        try
        {
          ...
          if($rs->num_rows()>0)
          {
              foreach($rs->result() as $row)
              {
                  ...
                  $ret_[$i_cnt]["s_title"]=htmlspecialchars_decode(get_unformatted_string($row->s_title)); 
				  $s_desc = htmlspecialchars_decode(get_unformatted_string($row->s_description));
        ...
        try
        {
            ...
            if(!empty($info))
            {
                ...
                $this->db->query($s_qry,array(	
													  ...
													  intval(trim(decrypt($info["i_lang_id"]))),
    ...
    * @returns array
    */
    public function fetch_content_using_cms_type_id($i_id,$lang_id)
    {
        try
        {
          $ret_=array();
          ...
          $rs=$this->db->query($s_qry,array(intval($i_id),intval($lang_id)));
          if($rs->num_rows()>0)
          {
              foreach($rs->result() as $row)
              {
                  $ret_["id"]=$row->i_id;////always integer
                  //$ret_["s_name"]=stripslashes($row->s_name);
				  $ret_["s_title"]		=	html_entity_decode($row->s_title,ENT_QUOTES,'utf-8');	
				  $ret_["s_description"]=	htmlspecialchars_decode(get_unformatted_string($row->s_description));
                  $ret_["dt_created_on"]=	date($this->conf["site_date_format"],intval($row->dt_entry_date)); 
                  $ret_["i_is_active"]	=	intval($row->i_status); 
				  $ret_["s_is_active"]	=	(intval($row->i_status)==1?"Active":"Inactive");
