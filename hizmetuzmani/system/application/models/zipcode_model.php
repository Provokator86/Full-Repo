...
class Zipcode_model extends MY_Model implements InfModel
{
    ...
    */
    public function fetch_multi($s_where=null,$i_start=null,$i_limit=null)
    {
        ...
        {
          	$ret_=array();
			$s_qry="SELECT n.*,c.city,s.province FROM ".$this->tbl." n "
					."LEFT JOIN ".$this->tbl_city." c "
					."ON n.city_id=c.id "
					."LEFT JOIN ".$this->tbl_province." s "
					."ON n.province_id=s.id "
        ...
        try
        {
          ...
          if($rs->num_rows()>0)
          {
              foreach($rs->result() as $row)
              {
				  ...
				  $ret_["province_id"] = stripslashes($row->province_id);				  
    ...
    */
	...
	function get_zip_selectlist($province_id='',$city_id='')
    {
        try
        {
          $ret_=array();
          ...
          $res=$this->db->query($s_qry,array(intval($province_id),intval($city_id))); 
