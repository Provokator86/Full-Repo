...
class News_model extends MY_Model implements InfModel
{
    private $conf;
        ...
        try
        {
          ...
          if($rs->num_rows()>0)
          {
              foreach($rs->result() as $row)
              {
				  ...
				  $s_title							=	stripslashes(htmlspecialchars_decode($row->s_title));
                  $ret_[$i_cnt]["s_title"]			=	$s_title; 
				 					
				  $s_desc 							=	strip_tags(stripslashes(htmlspecialchars_decode($row->s_description)));
				  $ret_[$i_cnt]["s_desc_full"] 		= 	$s_desc;
				  if(strlen($s_desc)>50)
				  	$s_desc = substr_replace($s_desc,'...',50);
    ...
    * @returns array
    */
    public function fetch_multi_rss_news($s_where=null,$i_start=null,$i_limit=null)
    {
        try
        {
          	$ret_=array();
          ...
          if($rs->num_rows()>0)
          {
              ...
              {
                  $ret_[$i_cnt]["id"]=$row->i_id;////always integer
				  $s_title							=	get_unformatted_string($row->s_title);
                  $ret_[$i_cnt]["s_title"]			=	$s_title; 
				  ...
				  $ret_[$i_cnt]["rss_title"]		=	get_unformatted_string($row->s_title);		  
				  $ret_[$i_cnt]["rss_desc_link"]	=	base_url().'news/'.encrypt($ret_[$i_cnt]["id"]);
				  $ret_[$i_cnt]["rss_long_description"]=	$ret_[$i_cnt]["s_desc_full"];				  
				  $ret_[$i_cnt]["rss_short_description"]=	strip_tags($s_desc);
