...
class MY_Controller extends Controller
{
        ...
        try{
          ...
          /*$rs =   $this->db->query('SHOW TABLES FROM hizmetuzmani');
            ...
            foreach($row as $value)
            {
                ...
                if(preg_match('/^hizmetuzmani_/',$old_table_name))
                {
                    $new_table_name =   preg_replace('/^hizmetuzmani_/','hizmetuzmani_',$old_table_name) ;
           ...
           /************ End Change Charecter set and collation ******/ 
			...
			/****************** START FETCHING SITE SETTING INFORMATIONS ************************ /
			...
			*/
			...
			//print_r($info); exit;
			$this->site_default_lang_prefix = $info['s_short_name']; 
			//used to fetch default lang content when there is no content in a selected language
			
			$this->s_admin_email  = $info["s_admin_email"];
			$this->s_paypal_email = $info["s_paypal_email"];
			$this->s_facebook_link = $info["s_facebook_address"];
			...
			$this->i_default_language_prefix = ($this->session->userdata('current_language')) ? $this->session->userdata('current_language') : $this->site_default_lang_prefix;
			$this->data['i_lang_id'] = $this->i_default_language;
			$this->data['language_now'] = $this->i_default_language_prefix;
			$this->data['site_setting']=$info;
			/****************** END FETCHING SITE SETTING INFORMATIONS ************************/
			
			
			/****************** START FETCHING LANGUAGE INFORMATIONS ************************/
            ...
            /* added by mrinmoy on 29 march 2012
            * This block is used for selecting the default prefix
            * Prefix store in session   lang_prefix  
            */
            if(!empty($info))
            {
                 foreach($info as $val)
                 {
                     ...
                     if($val['id']==$this->i_default_language)
                     {
                         $this->session->set_userdata('lang_prefix',$val['s_short_name']);
                         $this->s_default_lang_name   =   $val['s_language']; 
                     }
                 }
                   
            } ////End of  lang_prefix
			/****************** END FETCHING LANGUAGE INFORMATIONS ************************ /
			
			/**START FETCHING TOTAL NEW MESSAGES AND NEW QUOTES FOR BUYER AND TRADESMAN FOR DASHBOARD BLINK ALERT ****/
			if(!empty($this->data['loggedin']))
			{
			$this->load->model('manage_private_message_model','mod_pmb');
            $s_where  =   " WHERE i_receiver_id=".decrypt($this->data['loggedin']['user_id'])." AND i_receiver_view_status = 0 ";
            $this->data['i_new_message']  =   $this->mod_pmb->gettotal_msg_info($s_where);
			
		    $this->load->model('job_model');
			$s_where = " WHERE n.i_buyer_user_id=".decrypt($this->data['loggedin']['user_id'])." AND n.i_status=1 AND n.i_is_deleted!=1 "; 
		    $this->data['active_open_jobs'] = $this->job_model->fetch_multi_completed($s_where,intval($start),$limit);
		    $tot_new_quotes = 0;
			if(!empty($this->data['active_open_jobs']))
			{
				foreach($this->data['active_open_jobs'] as $val)
				{
					$s_where = " WHERE n.i_job_id = ".$val['id']." AND n.i_status = 1 ";
					$tot_new_quotes+= $this->job_model->gettotal_quote_info($s_where);
				}
			}
            $this->data['tot_new_quotes'] = $tot_new_quotes;
			
			}
			
			/** END FETCHING TOTAL NEW MESSAGES AND NEW QUOTES FOR BUYER AND TRADESMAN *** /
    ...
    */
        ...
        try
        {
			...
			else
			{
				...
				if(empty($this->data['admin_loggedin']) 
				  && ($this->uri->rsegment(1)!='home' && $this->uri->rsegment(1)!='forgot_password' && $this->uri->rsegment(1)!='newsletter_send' && $this->uri->rsegment(1)!='frozen_job_alert' && $this->uri->rsegment(1)!='frozen_job_to_active' && $this->uri->rsegment(1)!='job_expire' && $this->uri->rsegment(1)!='quote_approve_day_expire' && $this->uri->rsegment(1)!='radar_job'))
				{
	...
	/** To store search data for job and tradesman **/
