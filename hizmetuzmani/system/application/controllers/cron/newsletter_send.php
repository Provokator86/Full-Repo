...
*/
class Newsletter_send extends My_Controller
{
			...
			if($total_db_records>0)
			{
				for($i=0;$i<$total_db_records;$i++)
				{
					...
					foreach($user_type_id as $key=>$param)
					{
						...
						{
							case "1":
								$arr_search[] = " SELECT id,s_name,s_email FROM {$this->db->MST_USER} WHERE i_role=1 AND i_is_active=1 AND i_inform_news =1 ";
								
								break;
								
							case "2":
								$arr_search[] = " SELECT id,s_name,s_email FROM {$this->db->MST_USER} WHERE i_role=2 AND i_is_active=1 AND i_inform_news =1 ";
								break;
								
							case "4":
								$arr_search[] = " SELECT 0 AS id, s_name,s_email FROM {$this->db->NEWSLETTERSUBCRIPTION} WHERE i_user_type=4 AND i_subscribe_status=1 ";
								break;	
					...
					if(count($rowUser)>0)
					{
						...
						{									
							 ...
							 $html = str_replace("[UNSUBSCRIBE_URL]",base_url().'user/unsubscribe_newsletter/'.$to,$html);
