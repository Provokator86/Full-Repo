    ...
    public function add_information()           
    {
		...
		try
        {
            ...
            if($_POST)
            {
				...
				$posted["i_buyers"]		=	trim($this->input->post("i_buyers"));
				$posted["i_tradesman"]	=	trim($this->input->post("i_tradesman"));
				...
				{
				$array = array($posted["i_buyers"],$posted["i_tradesman"],$posted["i_general"]);
				$str = implode(",", $array);
				}
                ...
                else///validated, now save into DB
                {
					...
					echo date("Y-m-d",$info["i_send_date"]);
					exit;*/
    ...
    public function modify_information($i_id=0)
    {
        ...
        try
        {
            ...
            {	
				...
				$posted["i_all"] = count($comma_separated);
				foreach($comma_separated as $val)
				{
					if($val==1){$posted["i_buyers"]=1;}
					if($val==2){$posted["i_tradesman"]=2;}
					if($val==4){$posted["i_general"]=4;}
				}				
				
				$posted["dt_send_on"]	= trim($info["dt_send_on"]);
		...
		try
		{
            ...
            if($_POST)
            {
                ...
                else///validated, now save into DB
                {
					...
					$info = $this->mod_rect->fetch_this(decrypt($id));
					...
					$this->email->from('admin@quoteurjob.ca');
