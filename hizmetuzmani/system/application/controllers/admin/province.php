...
* @link model/province_model.php
* @link views/admin/province/
*/


class Province extends My_Controller implements InfController
{
    ...
    public function __construct()
    {
        ...
        {
          parent::__construct();
          $this->data['title']="Province Management";////Browser Title

          ////////Define Errors Here//////
          $this->cls_msg = array();
          $this->cls_msg["no_result"]	="No information found about province.";
          $this->cls_msg["save_err"]	="Information about province failed to save.";
          $this->cls_msg["save_succ"]	="Information about province saved successfully.";
          $this->cls_msg["delete_err"]	="Information about province failed to remove.";
          $this->cls_msg["delete_succ"]	="Information about province removed successfully.";
		  $this->cls_msg["status_succ"]="Status of province saved successfully.";
		  $this->cls_msg["status_err"]	="Status of province failed to save.";
          ////////end Define Errors Here//////
          $this->pathtoclass = admin_base_url().$this->router->fetch_class()."/";//for redirecting from this class
		  
		  //////// loading default model here //////////////
          $this->load->model("province_model","mod_rect");
    ...
    */
    public function show_list($order_name='',$order_by='asc',$start=NULL,$limit=NULL)
    {
        try
        {
            $this->data['heading']="Province";////Package Name[@package] Panel Heading

            ///////////generating search query///////
			
            ////////Getting Posted or session values for search///
            $s_search		=(isset($_POST["h_search"])?$this->input->post("h_search"):$this->session->userdata("h_search"));
            $s_province_title	=($this->input->post("h_search")?$this->input->post("txt_province_title"):$this->session->userdata("txt_province_title")); 
            ...
            if($s_search=="basic")
            {
                $s_where=" WHERE n.province LIKE '%".addslashes(htmlentities($s_province_title))."%' ";
                /////Storing search values into session///
                $this->session->set_userdata("txt_province_title",$s_province_title);
                $this->session->set_userdata("h_search",$s_search);
                
                $this->data["h_search"]=$s_search;
                $this->data["txt_province_title"]=$s_province_title;
                /////end Storing search values into session///
            }
            elseif($s_search=="advanced")
            {
                $s_where=" WHERE n.province LIKE '%".get_formatted_string($s_province_title)."%' ";
               /* if(trim($dt_created_on)!="")
				...
				if(trim($s_province_short)!="")
                {
                    
                    $s_where.=" And n.abbreviation LIKE '%".get_formatted_string($s_province_short)."%' ";
                    unset($dt_start);
                }
            ...
            else////List all records, **not done
            {
                ...
                $this->session->unset_userdata("txt_province_title");
                $this->session->unset_userdata("h_search");
                
                $this->data["h_search"]=$s_search;
                $this->data["txt_province_title"]="";                          
                /////end Storing search values into session///                 
			...
			$arr_sort = array(0=>'province',2=>'i_status'); 
            ...
            $table_view["headers"][0]["val"]	="Province Name";
            ...
            for($i=0; $i<$table_view["total_rows"]; $i++)
            {
				...
				/*if($this->data['action_allowed']["Status"])
				{
					...
					{
						$action ='<a  href="'.admin_base_url().'province/change_status/'.encrypt($info[$i]["id"]).'/0">
						<img width="12" height="12" title="Inactive" alt="Inactive" src="images/admin/reject.png"></a>';
					}
					else
					{
						$action =' <a  href="'.admin_base_url().'province/change_status/'.encrypt($info[$i]["id"]).'/1">
    ...
    */
    public function add_information()           
    {
        ...
        {
            $this->data['title']="Province Management";////Browser Title
            $this->data['heading']="Add Province";
            ...
            {
				$posted=array();
                $posted["txt_province_title"]		=	trim($this->input->post("txt_province_title"));
				$posted["opt_city"]	                =	trim($this->input->post("opt_city"));
				$i_active_val 						=	trim($this->input->post("i_province_is_active"));
                $posted["i_province_is_active"]		=	($i_active_val==1)?$i_active_val:2;
                $posted["h_mode"]					=	$this->data['mode'];
                $posted["h_id"]= "";
               
                $this->form_validation->set_rules('txt_province_title', 'province name', 'required');
                ...
                {
                    $info=array();
                    $info["province"]			=	$posted["txt_province_title"];
                    $info["i_city_id"]	        =	decrypt($posted["opt_city"]);
                    $info["i_status"]		    =	$posted["i_province_is_active"];
    ...
    */      
    public function modify_information($i_id=0)
    {
        ...
        try
        {
            $this->data['title']="Edit Province Details";////Browser Title
            $this->data['heading']="Edit province";
            $this->data['pathtoclass']=$this->pathtoclass;
            $this->data['mode']="edit";
            ...
            else
            {
                ...
                $posted=array();
                $posted["txt_province_title"]        = trim($info["province"]);
				$posted["opt_city"]		             = encrypt($info["i_city_id"]);
				$posted["i_province_is_active"]		 = trim($info["i_is_active"]);
				$posted["h_id"]= $i_id;
                $this->data["posted"]=$posted;       
                unset($info,$posted);      
        ...
        try
        {
            if(trim($i_id)!="")
            {
                ...
                if(!empty($info))
                {
                    ...
                    $temp["s_province"] = trim($info["province"]);
					$temp["s_city"]     = trim($info["city"]);
            ...
            $this->render("province/show_detail",TRUE);
            unset($i_id);
        }
        catch(Exception $err_obj)
        {
    ...
    */
    public function ajax_checkduplicate()
    {
        try
        {
            ...
            $posted["i_city_id"]= decrypt($this->input->post("i_city_id"));
            ...
            if($posted["duplicate_value"]!="")
            {
                $qry=" Where ".(intval($posted["id"])>0 ? " n.id!=".intval($posted["id"])." And " : "" )
                    ." n.province='".$posted["duplicate_value"]."' AND n.i_city_id=".$posted["i_city_id"];
