    ...
    */
    ...
    public function access_control($i_id=0)
    {
        try
        {
			...
			$s_cond = " WHERE n.i_parent_id=0 ";
			$menu= $this->menu_model->fetch_all_menus($s_cond,'','',$user_type_id);
            //pr($menu,1);
            ////////////Submitted Form///////////
            if($_POST)
            {
				...
				//$i_delete_rows = $this->obj_mod->delete_existing_control($i_user_type_id);
						
				foreach($menu as $k=>$value )
				 {	// pr($value['s_sub_menu']);
				 	foreach($value['s_sub_menu'] as $key=>$val)
					{
						$key = str_replace('/','',$val['s_link']);
						$info=array();
						$info["i_user_type_id"] 	= $i_user_type_id;
						$info["s_controller"] 		= $key;
						$arr["i_action_view"] 		= trim($this->input->post("chk_action_view_".strtolower($key)));
						$arr["i_action_add"] 		= trim($this->input->post("chk_action_add_".strtolower($key)));
						$arr["i_action_edit"] 		= trim($this->input->post("chk_action_edit_".strtolower($key)));
						$arr["i_action_delete"] 	= trim($this->input->post("chk_action_delete_".strtolower($key)));
						$arr["i_action_approve"] 	= trim($this->input->post("chk_action_approve_".strtolower($key)));
						$arr["i_action_active"] 	= trim($this->input->post("chk_action_active_".strtolower($key)));
						...
						foreach($arr as $key1=>$val1)
						{
							...
							if($val1)
							{
								...
								switch($arr2[2])
								{
									case 'add':
											$info['s_action'] = 'Add';
											$info['s_link']  = $key.'/add_information/';
											break;
											
									case 'edit':
											$info['s_action'] = 'Edit';
											$info['s_link']  = $key.'/edit_information/';
											break;
											
									case 'delete':
											$info['s_action'] = 'Delete';
											$info['s_link']  = $key.'/remove_information/';
											break;
											
									case 'view':
											$info['s_action'] = 'View List';
											$info['s_link']  = $key.'/show_list/';
											break;
											
									case 'approve':
											$info['s_action'] = 'Approve';
											$info['s_link']  = $key.'/approve/';
											break;	
											
									case 'active':
											$info['s_action'] = 'Status';
											$info['s_link']  = $key.'/change_status/';
											break;
						...
						//pr($info);
						//$i_aff=$this->obj_mod->update_new_access_control($info,intval($i_user_type_id),intval($this->input->post("txt_controller_".strtolower($key))),$key);/*don't change*/
						unset($info,$posted);/*don't change*/
					}						
				}
    ...
    */
    public function ajax_update_user_access()
    {
        try
        {
			...
			$posted["s_action"]= $this->input->post("s_action");
			$posted["i_cur_status"]= $this->input->post("cur_status");
            $posted["s_controler_name"]= $this->input->post("s_controler_name"); 
            if($posted["i_cur_status"]=="true") //Add  accsess with each checked
            {	
            ...
            switch($posted["s_action"])
				...
				/*switch($posted["s_action"])
				{
					case 'add':
							$info['s_action'] = 'Add';
							$info['s_link']  = $posted["s_controler_name"].'/add_information/';
							break;
							
					case 'edit':
							$info['s_action'] = 'Edit';
							$info['s_link']  = $posted["s_controler_name"].'/edit_information/';
							break;
							
					case 'delete':
							$info['s_action'] = 'Delete';
							$info['s_link']  = $posted["s_controler_name"].'/remove_information/';
							break;
							
					case 'view':
							$info['s_action'] = 'View List';
							$info['s_link']  = $posted["s_controler_name"].'/show_list/';
							break;
							
					case 'approve':
							$info['s_action'] = 'Approve';
							$info['s_link']  = $posted["s_controler_name"].'/approve/';
							break;	
							
					case 'active':
							$info['s_action'] = 'Status';
							$info['s_link']  = $posted["s_controler_name"].'/change_status/';
							break;
            ...
            {
                $s_where    =   " WHERE i_menu_id={$posted["i_menu_id"]} AND i_user_type={$posted["i_user_type_id"]} AND s_action='".ucfirst($posted["s_action"])."' " ;
