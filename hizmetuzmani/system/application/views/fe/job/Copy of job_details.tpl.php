			...
			<div class="right_box_all_inner" style="padding-top:0px;">
				 <?php /*?> <div class="heading01"><?php echo t('Status')?> </div>
				  ...
				  <?php foreach($job_details['job_files'] as $val){ ?>
						...
						<a href="<?php echo base_url().'job/download_job_files/'.encrypt($val['s_file_name']);?>" class="grey_link_line"><?php echo t('Download')?></a> <br />
				  <?php }?>
				  </div>							  	
				  <?php 
				  }
				  ...
				  <input name="" type="button" class="button3" value="<?php echo t('Private Messages Board')?>"  href="javascript:void(0);" onclick="window.location.href='<?php echo base_url().'private_message/private_msg_board_land/'.encrypt($job_details['id'])?>'"  style="font-size:13px;"/>
				  &nbsp;
				 	<?php } ?>
				  <?php
