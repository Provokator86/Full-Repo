...
class Cart_model extends My_Model
{
	...
	public function mani_cart_view($userid=''){
		...
		/****************************** Gift Card Displays Language start **************************************/
		
		
		
		
		/****************************** Gift Card Displays Language end **************************************/
		...
		if($this->lang->line('cart_message') != '') 
			$cart_message =  stripslashes($this->lang->line('cart_message')); 
		else  
		...
		if($this->lang->line('cart_cont_paymnt') != '') 
			$cart_cont_paymnt =  stripslashes($this->lang->line('cart_cont_paymnt')); 
		else  
			$cart_cont_paymnt =  "Continue to Payment";
			
		if($this->lang->line('cart_chose_ship') != '') 
			$cart_chose_ship =  stripslashes($this->lang->line('cart_chose_ship')); 
		else  
			$cart_chose_ship =  "Choose Your Shipping Address";
			
		if($this->lang->line('cart_delete_addrs') != '') 
			$cart_delete_addrs =  stripslashes($this->lang->line('cart_delete_addrs')); 
		else  
			$cart_delete_addrs =  "Delete this address";
			
		if($this->lang->line('cart_add_ship') != '') 
			$cart_add_ship =  stripslashes($this->lang->line('cart_add_ship')); 
		else  
			$cart_add_ship =  "Add new shipping address";
		...
		if($this->lang->line('cart_of') != '') 
			$cart_of =  stripslashes($this->lang->line('cart_of')); 
		else  
			$cart_of =  "of";
			
		if($this->lang->line('cart_ship_to') != '') 
			$cart_ship_to =  stripslashes($this->lang->line('cart_ship_to')); 
		else  
			$cart_ship_to =  "Ship to";
			
			
		if($this->lang->line('cart_retail_price') != '') 
			$cart_retail_price =  stripslashes($this->lang->line('cart_retail_price')); 
		else  
			$cart_retail_price =  "retail price";
		
		if($this->lang->line('cart_update') != '') 
			$cart_update =  stripslashes($this->lang->line('cart_update')); 
		else  
			$cart_update =  "Update";
		...
		if($this->lang->line('cart_ord_before') != '') 
			$cart_ord_before =  stripslashes($this->lang->line('cart_ord_before')); 
		else  
			$cart_ord_before =  "Order before 11 AM and get it today!";
			
		if($this->lang->line('cart_avail_ny') != '') 
			$cart_avail_ny =  stripslashes($this->lang->line('cart_avail_ny')); 
		else  
			$cart_avail_ny =  "Available in New York, NY";
			
		if($this->lang->line('cart_note_to') != '') 
			$cart_note_to =  stripslashes($this->lang->line('cart_note_to')); 
		else  
			$cart_note_to =  "Note to";
			
		if($this->lang->line('cart_merchant') != '') 
			$cart_merchant =  stripslashes($this->lang->line('cart_merchant')); 
		else  
			$cart_merchant =  "Merchant";
			
		if($this->lang->line('cart_optional') != '') 
			$cart_optional =  stripslashes($this->lang->line('cart_optional')); 
		else  
			$cart_optional =  "Optional";
			
		if($this->lang->line('cart_note_here') != '') 
			$cart_note_here =  stripslashes($this->lang->line('cart_note_here')); 
		else  
			$cart_note_here =  "You can leave a personalized note here";
			
		if($this->lang->line('cart_ship_speed') != '') 
			$cart_ship_speed =  stripslashes($this->lang->line('cart_ship_speed')); 
		else  
			$cart_ship_speed =  "Shipping Speed";
			
		if($this->lang->line('cart_standard') != '') 
			$cart_standard =  stripslashes($this->lang->line('cart_standard')); 
		else  
			$cart_standard =  "Standard";
			
		if($this->lang->line('cart_same_delivry') != '') 
			$cart_same_delivry =  stripslashes($this->lang->line('cart_same_delivry')); 
		else  
			$cart_same_delivry =  "Same-day Delivery";
			
		if($this->lang->line('cart_gifr') != '') 
			$cart_gifr =  stripslashes($this->lang->line('cart_gifr')); 
		else  
			$cart_gifr =  "Gift";
			
		if($this->lang->line('cart_ord_gifr') != '') 
			$cart_ord_gifr =  stripslashes($this->lang->line('cart_ord_gifr')); 
		else  
			$cart_ord_gifr =  "This order is a gift";
			
		if($this->lang->line('cart_coupon_code') != '') 
			$cart_coupon_code =  stripslashes($this->lang->line('cart_coupon_code')); 
		else  
			$cart_coupon_code =  "Coupon Codes";
			
		if($this->lang->line('cart_have_code') != '') 
			$cart_have_code =  stripslashes($this->lang->line('cart_have_code')); 
		else  
			$cart_have_code =  "Have a coupon code?";
			
		if($this->lang->line('cart_apply') != '') 
			$cart_apply =  stripslashes($this->lang->line('cart_apply')); 
		else  
			$cart_apply =  "Apply";
		
		if($this->lang->line('checkout_discount') != '') 
			$checkout_discount =  stripslashes($this->lang->line('checkout_discount')); 
		else  
			$checkout_discount =  "Discount";
			
		if($this->lang->line('cart_item_shop_cart') != '') 
			$cart_item_shop_cart =  stripslashes($this->lang->line('cart_item_shop_cart')); 
		else  
			$cart_item_shop_cart =  "items in shopping cart";
			
		if($this->lang->line('cart_empty') != '') 
			$cart_empty =  stripslashes($this->lang->line('cart_empty')); 
		else  
			$cart_empty =  "Shopping Cart Empty";
			
		if($this->lang->line('cart_shop_empty') != '') 
			$cart_shop_empty =  stripslashes($this->lang->line('cart_shop_empty')); 
		else  
			$cart_shop_empty =  "Your Shopping Cart is Empty";
			
		if($this->lang->line('cart_dont_miss') != '') 
			$cart_dont_miss =  stripslashes($this->lang->line('cart_dont_miss')); 
		else  
			$cart_dont_miss =  "Don't miss out on awesome sales right here on";
			
		if($this->lang->line('cart_lets_fill') != '') 
			$cart_lets_fill =  stripslashes($this->lang->line('cart_lets_fill')); 
		else  
			$cart_lets_fill =  "Let's fill that cart, shall we?";
		...
		if($giftRes -> num_rows() > 0 ){ 
			...
			<form method="post" name="giftSubmit" id="giftSubmit" class="continue_payment" enctype="multipart/form-data" action="checkout/gift">
				...
				<thead><tr class="bg-cart-payment"><th width="51%" colspan="2" class="product">'.$cart_product.'</th><th width="18%">'.$giftcard_price.'</th><th width="15%">'.$product_quantity.'</th><th width="21%">'.$purchases_total.'</th></tr></thead></table>';	
			$giftAmt = 0; $g=0;
                ...
                <tr>
            		<td class="optional" colspan="4"><div class="relative"><span></span>
						<ul class="optional-list">
    	                    ...
    	                    <li><span class="option-tit">'.$cart_message.':</span><span class="option-txt">'.$giftRow->description.'</span></li>
						</ul>
						</div>
					</td>
				</tr></tbody></table></div>';
		...
		$GiftValue.= '</div>
		    ...
		    <dl class="cart-payment-order">
				...
				<li class="total"><span class="order-payment-type"><b>'.$purchases_total.'</b></span><span class="order-payment-usd"><b>'.$this->data['currencySymbol'].'<span id="total_item">'.number_format($giftAmt,2,'.','').'</span></b> '.$this->data['currencyType'].'</span></li>
				</ul>
		      	</dd>
			</dl>
		    <button type="submit" class="start_btn_2" id="button-submit-giftcard">'.$cart_cont_paymnt.'</button>
		  	</div></div></form></div>';
		}
		...
		$SubscribValue.= '</div>
			...
			<select id="address-cart" class="select-round select-shipping-addr" onchange="SubscribeChangeAddress(this.value);">
				  <option value="" id="address-select">'.$cart_chose_ship.'</option>
			';
		...
		$SubscribValue.='</select>
			...
			<a href="javascript:void(0);" class="delete_addr" onclick="shipping_cart_address_delete();">'.$cart_delete_addrs.'</a>
			
			<a href="javascript:void(0);" class="add_addr add_" onclick="shipping_address_cart();">'.$cart_add_ship.'</a>
			...
			<ul>
			  ...
			  <li>
			    <span class="order-payment-type">'.$checkout_tax.' (<span id="SubTamt">'.$MainTaxCost.'</span>%) '.$cart_of.' '.$this->data['currencySymbol'].$SubscribAmt.'</span>
			 ...
			 $SubscribValue.='
			  <ul>
			 <li class="total">
			    <span class="order-payment-type"><b>'.$purchases_total.'</b></span>
		    ...
		    <input type="submit" class="start_btn_2" name="SubscribePayment" id="button-submit-merchant" value="'.$cart_cont_paymnt.'" />
		...
		if($cartVal -> num_rows() > 0 ){
			$CartValue.='<div id="CartTable" style="display:block;"><!--<p class="cart-list-from">Order from <b>'.$this->config->item('email_title').' '.$cart_merchant.'</b></p>-->
				...
				<div class="cart-payment-wrap cart-note"><span class="cart-payment-top"><b></b></span><div class="table-cart-wrap"><table class="table-cart">
		<thead><tr class="bg-cart-payment"><th width="64%" colspan="2" class="product">'.$cart_product.'</th><th width="15%">'.$giftcard_price.'</th><th width="8%">'.$product_quantity.'</th><th width="13%">'.$purchases_total.'</th></tr></thead></table>
       ';
		...
		$CartValue.='<div id="cartdivId_'.$s.'"> <table class="table-cart"><tbody><tr class="first">
			<td rowspan="2" class="thumnail"><a href="things/'.$CartRow->prdid.'/'.$CartRow->seourl.'"><img src="'.PRODUCTPATH.$newImg[0].'" alt="'.$CartRow->product_name.'"></a><a href="javascript:void(0);" onclick="javascript:delete_cart('.$CartRow->id.','.$s.')" class="remove_item">'.$product_remove.'</a></td>
			...
			<td class="quantity"><input class="text" value="'.$CartRow->quantity.'" name="quantity'.$s.'" data-mqty="'.$CartRow->mqty.'" id="quantity'.$s.'" type="text"><br><a href="javascript:void(0);" onclick="javascript:update_cart('.$CartRow->id.','.$s.')" class="update_quantity">'.$cart_update.'</a></td>
		...
		<tr>
			...
			<div class="show_detail">
				...
				<span class="tooltip delivery" style="display:none"><i class="ic-delivery"></i><small>'.$cart_ord_before.'<br>'.$cart_avail_ny.'<b></b></small></span>
			</div>
			</div>
			</td>
		</tr></tbody></table></div>';
		...
		$CartValue.='<dl class="note">
		      <dt>'.$cart_note_to.' '.$this->config->item('email_title').' '.$cart_merchant.' <small>'.$cart_optional.'</small></dt>
		      <dd><textarea class="note-to-seller" name="note" data-id="cart-note-1192557-616001" placeholder="'.$cart_note_here.'"></textarea></dd>
		    </dl>
		...
		$CartValue.='</select>
			...
			<dl class="ship-speed" style="display:none;border-bottom:1px solid #D4D6DF;">
			    <dt>'.$cart_ship_speed.'</dt>
			    <dd>
				    <input id="speed2-val1" name="shipping_speed" value="0" type="radio"> <label for="speed2-val1">'.$cart_standard.'</label><br>
					<input id="speed2-val3" name="shipping_speed" value="3" type="radio"> <label for="speed2-val3">'.$cart_same_delivry.'</label>
				</dd>
			</dl>
			...
			$CartValue.='<dl class="cart-coupon">
                ...
                <dd><input id="is_coupon" name="is_coupon" class="text coupon-code" readonly="readonly" placeholder="'.$cart_have_code.'" data-sid="616001" type="text" value="'.$discountQuery->row()->couponCode.'">
			...
			$CartValue.='<dl class="cart-coupon">
				<dt>'.$cart_coupon_code.'</dt>
                <dd><input id="is_coupon" name="is_coupon" class="text coupon-code" placeholder="'.$cart_have_code.'" data-sid="616001" type="text">
				<input id="CheckCodeButton" type="button" class="btn-blue-apply apply-coupon" onclick="javascript:checkCode();" value="'.$cart_apply.'" style="cursor:pointer;" /></dd>
				<dd><span id="CouponErr" style="color:#FF0000;"></span></dd>
			...
			<ul>
			  ...
			  <li>
			    <span class="order-payment-type">'.$checkout_tax.' (<span id="carTamt">'.$MainTaxCost.'</span>%) '.$cart_of.' '.$this->data['currencySymbol'].'<span id="CartAmtDup">'.$cartAmt.'</span></span>
			  ...
			  $CartValue.='<div id="disAmtValDiv"><ul><li>
			    <span class="order-payment-type">'.$checkout_discount.'</span>
		    ...
		    $CartValue.='<input type="submit" class="start_btn_2" name="cartPayment" id="button-submit-merchant" value="'.$cart_cont_paymnt.'" />
		...
		if($countVal >0 ){
			$CartDisp = '<h2><span id="Shop_id_count">'.$countVal.'</span> '.$cart_item_shop_cart.'</h2>'.$GiftValue.$SubscribValue.$CartValue.'<div id="EmptyCart" style="border-bottom: none; display:none;" class="empty-alert" >
					<p style="text-align:center;"><img src="images/site/shopping_empty.jpg" alt="'.$cart_empty.'"></p>
					<p style="text-align:center;"><b>'.$cart_shop_empty.'</b></p>
					<p style="text-align:center;">'.$cart_dont_miss.' '.ucwords($this->config->item('email_title')).'. '.$cart_lets_fill.'</p>
				</div>';
		}else{
	...
	public function delete_unwanted_records(){
		$cart_products = $this->get_all_details(SHOPPING_CART,array());
		if ($cart_products->num_rows()>0){
			foreach ($cart_products->result() as $row){
				$product_avail = $this->get_all_details(PRODUCT,array('id'=>$row->product_id));
				if ($product_avail->num_rows == 0){
					$this->commonDelete(SHOPPING_CART,array('product_id'=>$row->product_id));
				}
			}
		}
	}
