...
class My_Model extends CI_Model {
	 ...
	 * return Array
	 */
	public function get_all_details($table='',$condition='',$sortArr='',$limit=''){
	...
	public function mini_cart_view($userid = ''){
		...
		if($countMiniVal == 0){
			$cartMiniDisp= '<ul class="cart_tab"><li id="cart-new"><a href="cart" class="mn-cart"><em class="ic-cart"></em><span id="Shop_MiniId_count">0 items</span></a></li></ul>';
		}else{

			$_SESSION['countMiniVal']=$countMiniVal;
			$minCartVal.= '<ul class="cart_tab"><li id="cart-new"><a href="cart" class="mn-cart"><em class="ic-cart"></em><span id="Shop_MiniId_count">'.$countMiniVal.' items</span></a><ul class="menu-contain-cart" id="cart_popup">
		<table><thead><tr><th>DESCRIPTION</th><td>QTY</td><td class="price">PRICE</td></tr></thead></table>';
		...
		<a href="cart/" class="proceed">Proceed to Checkout</a>
		</div></li></ul>';
		...
		//			<ul class="gnb-wrap"><li class="gnb" id="cart-new"><a href="cart" class="mn-cart"><span class="hide">cart</span> <em class="ic-cart"></em> <span>0 items</span></a></li></ul>
    ...
    ## fn written for new coding used in user controller
    public function count_tbl_total_records($table='',$condition=''){
        $ret_=0;
        $ret_ = $this->db->get_where($table,$condition) ; 
	...
	function selectRecordsFromTableNew($tableName,$paraArr)
	{
		extract($paraArr);
		$this->db->select($selectValues);
		$this->db->from($tableName);
		...
		if(!empty($likeQuery))
		{
			///echo "<pre>";print_r($likeQuery);die;
			//echo "empty";die;
			$this->db->like($likeQuery);
		}
		$query = $this->db->get();
