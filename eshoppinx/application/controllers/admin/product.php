    ...
    {        
        if(!empty($data_arr))
        {
            ...
            for($i=$start_key;$i<count($data_arr); $i++)
            {
                ...
                // mens footwear
                $prod_link = str_replace('&UID=MYDEALFOUNDXXX','',$arr[13]);
                $web_links = explode('&r=',$prod_link);
                if($web_links[1])
                $web_link  = urldecode($web_links[1]);
                else
                {
                    $prod_link = str_replace('&aff_sub=MYDEALFOUNDXXX','',$arr[13]);
                ...
                {                
                    ...
                    $image_name = '';
                    $img_url = explode(',',$arr[31]);
                    
                    $arr[31] = $img_url[8]?$img_url[8]:$img_url[0];
                    if ($arr[31] && $arr[31]!=''){
                        /****----------Move image to server-------------****/

                        $image_url = trim(addslashes($arr[31]));
                    ...
                    */
                   $category_id = 16; // women footwear
                    
                    $info["image"]              = $image_name;
                    $info["product_name"]       = $arr[8]?$arr[8]:"shoe Footwear";
