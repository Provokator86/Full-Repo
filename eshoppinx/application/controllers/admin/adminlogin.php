	...
	public function admin_global_settings(){
		if (!$demoserverChk){
			...
			if ($form_mode == 'main_settings'){
				...
				$excludeArr = array('form_mode','payment_details','logo_image','logo_icon','fevicon_image','site_contact_mail','email_title','footer_content','like_text','liked_text','unlike_text','logo_icon_option');
				...
				if ( $this->upload->do_upload('logo_icon')){
					$LogoIconDetails = $this->upload->data();
					$dataArr['logo_icon'] = $LogoIconDetails['file_name'];
				}
				$excludeArr = array('form_mode','logo_icon','logo_image','fevicon_image','email','admin_name');
			...
			$this->setErrorMessage('error','You are in demo mode. Settings cannot be updated');
			redirect('admin/adminlogin/admin_global_settings_form');
		}
	}
