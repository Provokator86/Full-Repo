...
class Contactseller extends MY_Controller {
	...
	public function delete_contact_record($rid='0'){
		if ($this->checkLogin('A') == ''){
			redirect('admin');
		}else {
			$this->contactseller_model->commonDelete(CONTACTSELLER,array('id'=>$rid));
			$this->setErrorMessage('success','Contact record deleted successfully');
			redirect('admin/contactseller/display_contact_seller');
		}
	}
}
