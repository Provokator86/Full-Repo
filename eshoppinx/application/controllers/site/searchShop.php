...
class SearchShop extends MY_Controller {
	...
	public function search_shopby(){
		...
		$this->data['heading'] = 'Search Products';
		if ($this->input->get('q')!=''){
			$condition = " where p.status='Publish' and u.group='Seller' and u.status='Active' and p.product_name LIKE '%".my_receive($this->input->get('q'))."%' or p.status='Publish' and p.user_id=0 and p.product_name LIKE '%".my_receive($this->input->get('q'))."%' order by p.created desc";
			$products_list_s = $this->product_model->view_product_details($condition);

			$condition = " where p.status='Publish' and  p.product_name LIKE '%".my_receive($this->input->get('q'))."%'";
			$products_list_u = $this->product_model->view_notsell_product_details($condition);
            // my_receive @see common_helper
			$this->data['productList'] = $this->product_model->get_sorted_array($products_list_s,$products_list_u,'created','desc');

			$this->data['user_list'] = $this->product_model->get_people(my_receive($this->input->get('q')));
			$this->data['sellers_list'] = $this->product_model->get_sellers(my_receive($this->input->get('q')));
			//echo '<pre>';print_r($this->data['productList']);die;
		}
		$this->load->view('site/product/product_search',$this->data);
	}
	public function search_shopby1111(){
	...
	public function search_priceby(){
		...
		if (count($searchArr)>0){
			$condition = ' where p.id in ('.implode(',', $searchArr).') and p.quantity>0 and p.status="Publish" and u.group="Seller" and u.status="Active" or p.id in ('.implode(',', $searchArr).') and p.status="Publish" and p.quantity > 0 and p.user_id=0 order by p.created desc limit 20';
	...
	public function ajax_load_more_price(){
		...
		$Query = 'select p.*,u.user_name,u.full_name from '.PRODUCT.' p
					...
					or p.id in ('.$searchIds.') and p.status="Publish" and p.quantity > 0 and p.user_id=0 order by p.created desc 
					limit '.$limitStr;
	...
	public function search_people(){
		...
		$user_list = $this->data['sellers_list'] = '';
		$this->data['productList'] = array();
		if ($key != ''){
			$user_list = $this->product_model->get_people(my_receive($key));
			$this->data['sellers_list'] = $this->product_model->get_sellers(my_receive($key));
			$condition = " where p.status='Publish' and u.group='Seller' and u.status='Active' and p.product_name LIKE '%".my_receive($key)."%' or p.status='Publish' and p.user_id=0 and p.product_name LIKE '%".my_receive($key)."%' order by p.created desc";
			$products_list_s = $this->product_model->view_product_details($condition);

			$condition = " where p.status='Publish' and  p.product_name LIKE '%".my_receive($key)."%'";
			$products_list_u = $this->product_model->view_notsell_product_details($condition);
		...
		if ($user_list!='' && $user_list->num_rows()>0){
			foreach ($user_list->result() as $store_lists_row){
				$sellProdDetails = $this->user_model->get_like_details_fully($store_lists_row->id,'8');
				$affilProdDetails = $this->user_model->get_like_details_fully_user_products($store_lists_row->id,'8');
				$this->data['prodDetails'][$store_lists_row->id] = $this->product_model->get_sorted_array($sellProdDetails,$affilProdDetails,'created','desc');
			}
		}

		$this->load->view('site/user/user_search',$this->data);
	}
	...
	public function search_sellers(){
		...
		$this->data['productList'] = array();
		if ($key != ''){
			$user_list = $this->product_model->get_sellers(my_receive($key));
			$this->data['user_list'] = $this->product_model->get_people(my_receive($key));
			$condition = " where p.status='Publish' and u.group='Seller' and u.status='Active' and p.product_name LIKE '%".my_receive($key)."%' or p.status='Publish' and p.user_id=0 and p.product_name LIKE '%".my_receive($key)."%' order by p.created desc";
			$products_list_s = $this->product_model->view_product_details($condition);
		...
		$this->data['sellers_list'] = $user_list;
		if ($user_list!='' && $user_list->num_rows()>0){
			foreach ($user_list->result() as $store_lists_row){
				$pids = array_filter(explode(',', $store_lists_row->products));
				if (count($pids)>0){
					$this->data['prodDetails'][$store_lists_row->id] = $this->store_model->get_product_details($pids);
				}else {
					$this->data['prodDetails'][$store_lists_row->id] = '';
				}
			}
		}
		$this->load->view('site/user/seller_search',$this->data);
	}
