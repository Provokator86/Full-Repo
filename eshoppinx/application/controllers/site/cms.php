...
class Cms extends MY_Controller {
	...
	public function index(){
    	...
    	$sortArr = array('field'=>'priority','type'=>'asc');
		$pageDetails = $this->product_model->get_all_details(CMS,array('seourl'=>$seourl,'status'=>'Publish'),array($sortArr));
    	if ($pageDetails->num_rows() == 0){
    		show_404();
    	}else {
    		if ($pageDetails->row()->meta_title == ''){
				$this->data['heading'] = $pageDetails->row()->page_name;
    		}else {
