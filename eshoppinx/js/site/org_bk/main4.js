	...
	$fancy_share
		...
		.on('open_thing', function(event, btn){
			...
			function setAnywhere(img){
				$('#share-anywhere').val('<a href="'+$this.data('url')+'"><img src="'+timage+'" width="'+img.width+'" height="'+img.height+'" class="fancy-id-'+tid+'" alt="'+tname+'"></a>');
			};
...
jQuery(function($){
	var dlg_signin = $.dialog('signin-overlay'),
		...
		dlg_login = $.dialog('login-overlay'),
		dlg_forgot = $.dialog('forgot-overlay');
	$('#navigation-test .mn-signup,.pp-signup').click(function(event){
		event.preventDefault();
		dlg_signin.open();
	});

	$('.cart_tab .mn-signin,.pp-login').click(function(event){
		event.preventDefault();
		dlg_login.open();
	});
	
	$('.forgot-pwd').click(function(event){
		event.preventDefault();
		dlg_forgot.open();
	});
