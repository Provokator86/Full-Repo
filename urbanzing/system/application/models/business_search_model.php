...
class Business_search_model extends My_model
{
	...
	function get_search_business($toshow = -1, $page = 0, $arr = array(), $order_by = 'b.name', $order_type = 'asc')
	{
		...
		if(isset($arr['vegetarian']) && $arr['vegetarian']!='')
			$subcond .= " AND b.vegetarian = '{$arr['vegetarian']}' ";
		
		if(isset($arr['air_conditioned']) && $arr['air_conditioned']!='')
			$subcond .= " AND b.air_conditioned = '{$arr['air_conditioned']}' ";
		
		if(isset($arr['credit_card']) && $arr['credit_card']!='')
			$subcond .= " AND b.credit_card = '{$arr['credit_card']}' ";
		
		if(isset($arr['take_reservation']) && $arr['take_reservation']!='')
			$subcond .= " AND b.take_reservation = '{$arr['take_reservation']}' ";
		
		if(isset($arr['parking']) && $arr['parking']!='')
			$subcond .= " AND b.parking = '{$arr['parking']}' ";
			
		if(isset($arr['alcohol']) && $arr['alcohol']!='' &&
				isset($arr['business_category']) && $arr['business_category'] == 1)
			$subcond .= " AND b.serving_alcohol = '{$arr['alcohol']}' ";
			
		if(isset($arr['menu']) && ($arr['menu']==1) &&
				isset($arr['business_category']) && $arr['business_category'] == 1)
		{		
			$subsel = ', bm.img_name menu ';
			$subsql .= " LEFT JOIN urban_business_menu bm ON b.id = bm.business_id  ";
			$subcond .= ' AND bm.img_name IS NOT NULL';
			
		}	
			
		if(isset($arr['menu']) && ($arr['menu']==0) &&
				isset($arr['business_category']) && $arr['business_category'] == 1)
		{	
			$subsel = ', bm.img_name ';
			$subsql .= " LEFT JOIN urban_business_menu bm ON b.id = bm.business_id  ";
			$subcond .= ' AND bm.img_name IS NULL';
		}	
		...
		if(isset($arr['business_category']) && $arr['business_category'] == 1) {
			$sql = "SELECT b.id, b.name, b.address, b.land_mark, b.tags, b.phone_number, b.website, b.avg_review,
							...
							z.zipcode, bp.img_name, p.price_from, p.price_to, z.place, bt.name bt_name{$subsel}
	...
	function get_business_type_list($business_category = 1, $arr = array(), $general_search = FALSE)
	{
		...
		$sql = "SELECT COUNT(DISTINCT bt.id) tot, t.id, t.name
				...
				LEFT JOIN (SELECT b.id,b.business_type_id{$subsel} FROM {$this->db->dbprefix}business b 
							".$subsql."
							WHERE 1 ".$subcond."
							AND b.status = '1' ) bt 
		...
		foreach($result_arr as $k => $v)
		{
			if(isset($arr['business_type_id']) && !empty($arr['business_type_id']))
			{
				$arrHtml['type_out'] .= "<li><a style='cursor:pointer;' onclick=\"business_type_search('".$v['id']."', '{$v['name']}');\">{$v['name']}</a> ({$v['tot']}) <a  style='cursor:pointer;' rel='imgtip[2]' onclick=\"business_search_remove_session('".$v['id']."', 'business_type');\"><img src='".base_url()."images/front/small_close.png' alt='Remove filter' ></a></li>";
			}
			else
			{
				if($k <= 5)
					$arrHtml['type_out'] .= "<li><a style='cursor:pointer;' onclick=\"business_type_search('".$v['id']."', '{$v['name']}');\">{$v['name']}</a> ({$v['tot']})</li>";
	...
	function get_price_range_list($business_category = 1, $arr = array(), $general_search = FALSE)
	{
		...
		$sql = "SELECT COUNT(DISTINCT bt.id) tot, p.id, p.price_from, p.price_to
				...
				LEFT JOIN (SELECT b.id,b.price_range_id{$subsel} FROM {$this->db->dbprefix}business b 
							".$subsql."
							WHERE 1 ".$subcond."
							AND b.status = '1' ) bt 
		...
		foreach($result_arr as $k => $v)
		{
			...
			if(isset($arr['price_range_id']) && !empty($arr['price_range_id']) &&
				isset($arr['business_category']) && $arr['business_category'] == 1)
			{
				$arrHtml['type_out'] .= "<li><a style='cursor:pointer;' onclick=\"autoload_ajax_no_jsn('".base_url()."search/search_business_list_ajax', 'div_search_business_list', 'price_range={$v['id']}&price_range_text={$price_range_text}');\">{$price_range_text}</a> ({$v['tot']}) <a style='cursor:pointer;' rel='imgtip[2]' onclick=\"business_search_remove_session('".$v['id']."', 'price_range');\"><img src='".base_url()."images/front/small_close.png' alt='Remove filter' ></a></li>";
			
			}	
			else
			{
				if($k <= 5)
					$arrHtml['type_out'] .= "<li><a style='cursor:pointer;' onclick=\"autoload_ajax_no_jsn('".base_url()."search/search_business_list_ajax', 'div_search_business_list', 'price_range={$v['id']}&price_range_text={$price_range_text}');\">{$price_range_text} </a>({$v['tot']})</li>";
	...
	function get_avg_review_list($business_category = 1, $arr = array(), $general_search = FALSE)
	{
		...
		$sql = "SELECT COUNT(DISTINCT b.id) tot, avg_review {$subsel}
				FROM {$this->db->dbprefix}business b
				".$subsql."
				WHERE 1 ".$subcond."
					AND b.status = '1'
		...
		for($j = 0; $j <= $max_avg_review; $j++)
		{
			...
			$num_stars = ($j > 0)?'':'0 star';
			if(isset($arr['avg_review']) && $arr['avg_review'] != '')
			{
				$arrHtml[] = "<li><a style='cursor:pointer;' onclick=\"autoload_ajax_no_jsn('".base_url()."search/search_business_list_ajax', 'div_search_business_list', 'avg_review=".$j."');\">".$num_stars.$img."</a> ({$avg_review_tot}) <a style='cursor:pointer;' rel='imgtip[2]' onclick=\"business_search_remove_session('".$j."', 'avg_review');\"><img src='".base_url()."images/front/small_close.png' alt='Remove filter' ></a> </li>";
			}
			else
			{
				$arrHtml[] = "<li><a style='cursor:pointer;' onclick=\"autoload_ajax_no_jsn('".base_url()."search/search_business_list_ajax', 'div_search_business_list', 'avg_review=".$j."');\">".$num_stars.$img." </a>({$avg_review_tot})</li>";
			}	
		}
		if(isset($arr['avg_review']) && $arr['avg_review'] != '')
	...
	function get_area_list($business_category = 1, $arr = array(), $general_search = FALSE)
	{
		...
		$sql = "SELECT COUNT(DISTINCT bt.id) tot, place 
				...
				LEFT JOIN (SELECT b.id,b.zipcode {$subsel} FROM {$this->db->dbprefix}business b 
							".$subsql."
							WHERE 1 ".$subcond."
							AND b.status = '1' ) bt 
				ON bt.zipcode = z.id
...
/*		$sql = "SELECT COUNT(DISTINCT b.id) tot, place
		...
		foreach($result_arr as $k => $v)
		{
			if(isset($arr['zipcode']) && !empty($arr['zipcode']))
			{
				$arrHtml['type_out'] .= "<li><a style='cursor:pointer;' onclick=\"autoload_ajax_no_jsn('".base_url()."search/search_business_list_ajax', 'div_search_business_list', 'zipcode=".$v['place']."');\">{$v['place']}</a> ({$v['tot']})  <a style='cursor:pointer;' rel='imgtip[2]' onclick=\"business_search_remove_session('".$v['place']."', 'zipcode');\"><img src='".base_url()."images/front/small_close.png' rel='imgtip[2]' alt='Remove filter' ></a></li>";
			}
			else
			{
			
				if($k <= 5)
					$arrHtml['type_out'] .= "<li><a style='cursor:pointer;' onclick=\"autoload_ajax_no_jsn('".base_url()."search/search_business_list_ajax', 'div_search_business_list', 'zipcode=".$v['place']."');\">{$v['place']}</a> ({$v['tot']})</li>";
	...
	function get_cuisine_list($business_category = 1, $arr = array(), $general_search = FALSE)
	{
		...
		$sql = "SELECT COUNT(DISTINCT bt.id) tot,c.id,c.cuisine
				...
				LEFT JOIN  (SELECT b.id {$subsel} FROM {$this->db->dbprefix}business b 
							{$subsql}
							WHERE 1 ".$subcond."
							AND b.status = '1' ) bt
		...
		foreach($result_arr as $k => $v)
		{
			if(isset($arr['cuisine_id']) && !empty($arr['cuisine_id']) &&
				isset($arr['business_category']) && $arr['business_category'] == 1)
			{
				$arrHtml['type_out'] .= "<li><a style='cursor:pointer;' onclick=\"autoload_ajax_no_jsn('".base_url()."search/search_business_list_ajax', 'div_search_business_list', 'cuisine_id=".$v['id']."');\">{$v['cuisine']}</a> ({$v['tot']})<a style='cursor:pointer;' rel='imgtip[2]' onclick=\"business_search_remove_session('".$v['id']."', 'cuisine_id');\"> <img src='".base_url()."images/front/small_close.png' rel='imgtip[2]' alt='Remove filter' ></a></li>";
			}	
			else
			{
				if($k <= 5)
					$arrHtml['type_out'] .= "<li><a style='cursor:pointer;' onclick=\"autoload_ajax_no_jsn('".base_url()."search/search_business_list_ajax', 'div_search_business_list', 'cuisine_id=".$v['id']."');\">{$v['cuisine']}</a> ({$v['tot']})</li>";
	...
	function get_business_menu_count($business_category = 1, $arr = array(), $general_search = FALSE)
 {
  $subsql = '';
  $subcond = '';
  $subsel ='';
  ...
  $sql = "SELECT COUNT(DISTINCT b.id) tot
      FROM {$this->db->dbprefix}business b
      LEFT JOIN urban_business_menu bm ON b.id = bm.business_id
      {$subsql}
      WHERE 1 AND bm.img_name IS NOT NULL $subcond
      GROUP BY b.id";  
