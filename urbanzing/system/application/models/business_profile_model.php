...
/*	function get_menu_list($business_id=-1)
	...
	function get_menu_list($business_id = 0, $menu_id = 0, $str_name_prefix = 'Uploaded by')
    {
		...
		if (!empty($business_id)) {
	        $sql = "SELECT menu.*, CONCAT('".$extra_col_text_prefix."', user.screen_name, '".$extra_col_text_suffix."') full_name
    ...
    function get_business_detail($arr = array(), $str_name_prefix = 'Uploaded by')
    {
		...
		{
			$sql = "SELECT b.*, CONCAT('".$extra_col_text_prefix."', user.screen_name, '".$extra_col_text_suffix."') full_name, ct.name ct_name, s.name s_name, c.name c_name, z.zipcode, bp.img_name, p.price_from, p.price_to, z.place
		...
		{
			$sql = "SELECT b.*, CONCAT('".$extra_col_text_prefix."', user.screen_name, '".$extra_col_text_suffix."') full_name, ct.name ct_name, s.name s_name, c.name c_name, z.zipcode, bp.img_name, z.place
					FROM {$this->db->dbprefix}business b
						INNER JOIN {$this->db->dbprefix}city ct ON ct.id = b.city_id
						INNER JOIN {$this->db->dbprefix}state s ON s.id = b.state_id
						INNER JOIN {$this->db->dbprefix}country c ON c.id = b.country_id
    ...
    function get_business_photo($arr = array(), $str_name_prefix = 'Uploaded by')
    {
		...
		$sql = "SELECT bp.*, CONCAT('".$extra_col_text_prefix."', user.screen_name, '".$extra_col_text_suffix."') full_name
...
/**			Purnendu   ( 10.11.2010 ) 
		"As  Clients wanted to show the screen name instead of  user name."

*		CONCAT('".$extra_col_text_prefix."', f_name, l_name, '".$extra_col_text_suffix."')	
is replaced by this.
CONCAT('".$extra_col_text_prefix."', user.screen_name, '".$extra_col_text_suffix."') 
*/