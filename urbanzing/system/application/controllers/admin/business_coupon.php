...
class Business_coupon extends MY_Controller
{
 	public $upload_path = '';
	...
	function index($order_name='cr_date',$order_type='asc',$page=0)
    {
		...
		$cond_arr = array('name'=>$sessArrTmp['src_business_title'],'status'=>$sessArrTmp['src_business_status']);
		$this->data['business_coupon'] = $this->business_model->get_business_coupon_list($cond_arr,$this->admin_page_limit,($page)?$page:0,$order_name,$order_type);
        $totRow = $this->business_model->get_business_coupon_list_count($cond_arr);
        $this->session->set_userdata(array('model_session'=>$sessArrTmp));
        if(!$this->data['business_coupon'])
        {
            if($page>=$totRow && $page!=0)
            {
                ...
                if($page<0)
                    $page=0;
                header('location:'.base_url().'admin/business_coupon/index/'.$order_name.'/'.$order_type.'/'.$page);
                exit;
            }
        }
        paggingInitialization($this,
            array('base_url'=>base_url().'admin/business_coupon/index/'.$order_name.'/'.$order_type,
                    'total_row'=>$totRow,
        ...
        $this->data['page']=$page;
        $sessArrTmp['redirect_url']=base_url().'admin/business_coupon/index/'.$order_name.'/'.$order_type.'/'.$page;
        //$this->session->set_userdata(array('model_session'=>$sessArrTmp));
        $this->add_js('ajax_helper');
        $this->set_include_files(array('common/admin_menu','business_image/business_coupon_list'));
		$this->render();
	}
    ...
    function ajax_change_status()
    {
        ...
        if($this->business_model->change_data_status('business_picture',$id,$status))
        {
            ...
            echo "<a onclick='call_ajax_status_change(\"".base_url()."admin/business_coupon/ajax_change_status\",".$jsnArr.",\"status".$id."\");' style='cursor:pointer; ".$style."'> ".$txt."</a>";
        }
    }

    function delete_business_coupon($id)
    {
        if($this->business_model->set_data_delete('request_for_coupon',$id))
		    $this->session->set_userdata(array('message'=>'Business coupon deleted successfully..','message_type'=>'err'));
        else
            $this->session->set_userdata(array('message'=>'Unable to delete Business coupon..','message_type'=>'err'));
