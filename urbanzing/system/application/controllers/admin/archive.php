...
class archive extends MY_Controller
{
    ...
    function index($order_name = 'cr_date', $page=0 )
	{
		...
		$config['base_url'] = base_url()."admin/archive/index/".$order_name;
	...
	function save_to_archive()
	{
		...
		$this->session->set_userdata(array('message'=>"Archived successfully.",'message_type'=>'succ'));
        header("location:".base_url().'admin/archive/index');
        exit;
		
			
	}
	
	function delete_archive( $id = -1)
	{
		...
		header("location:".base_url().'admin/archive/index/'.$order_name);
        exit;
	}
