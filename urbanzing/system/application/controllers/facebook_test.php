...
function index() 
 {
	...
	if (!preg_match("/facebook/i", $previous_url))
	{
		...
		if( $business_details[0]['land_mark'] != '')
		$wall_message .= $business_details[0]['land_mark'].", ";
		$wall_message .= $business_details[0]['ct_name']."-".$business_details[0]['zipcode'];
		$hour 		= htmlspecialchars($this->input->post('hour'), ENT_QUOTES, 'utf-8');
		$minute 	= htmlspecialchars($this->input->post('minute'), ENT_QUOTES, 'utf-8');
		$am_pm 		= htmlspecialchars($this->input->post('am_pm'), ENT_QUOTES, 'utf-8');
