...
class project_yuva extends MY_Controller
{
	...
	{ 
		...
		elseif(trim($this->input->post('pwd1'))=='') 
		...
		if($message != '')		
		{
			...
			header('location:'.base_url().'project_yuva');
			exit();
		}
		...
		$arr= array(
						...
						"source"=>"yuva project",
						"other_reasons"=>htmlspecialchars($this->input->post('other'), ENT_QUOTES, 'utf-8'),
						"user_type_id"=>2,
                        "date_created"=>time(),
		...
		if($userId)
            {
                ...
                else
                {
				    ...
				    $this->session->set_userdata(array('message'=>'Registered successfully, Please check your mail to confirm your email id.','message_type'=>'succ'));
