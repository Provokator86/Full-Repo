...
<div class="club_pub">
    <div class="right_cell_02">
        ...
        <div class="comment_box">
		    ...
		    <h2><?=$row[0]['name']?></h2>
			<?php
			if(isset($business_category) && $business_category!='')
			{
			?>
			<span style="float:right;">
			
			<input type="button" class="button_back_to_search_result" value="Back To Search Result" onclick="javascript:window.location='<?=base_url().'search/index/'.$business_category?>'" />
			</span>
			...
			<div class="margin5"></div>
			 <div>
			 <div style="float:left;margin-right:10px;"><a onclick="tb_show('','<?=base_url()?>ajax_controller/ajax_checkin/<?=$row[0]['id']?>?height=300&amp;width=450');" style="cursor: pointer;"><img src="<?=base_url()?>images/front/check_in.png" /></a>
			 </div>
...
<span><img  align="absmiddle"src="<?=base_url()?>images/front/face_book_icon.png" alt="" /> 39 people like this</span>-->
        ...
        <div class="comment_edit">
            <div class="edit_left">
			  <?php
   				 if(isset($business_type_name[0]['name']) && !empty($business_type_name[0]['name']))
   				 { 
    			?>
    			 <p><b>Business Type:</b> <span><?=$business_type_name[0]['name']?></span></p>
	...
	if($row[0]['other_cuisine']!='') {
                ...
                <p><?=$row[0]['address']?><br />
				<?=($row[0]['land_mark'])?$row[0]['land_mark'].'<br>':''?>
	...
	foreach ($row[0]['business_hour'] as $k=>$v) {
		if($v['hour_from']!='') {
			echo substr($v['day'], 0,3).' '.$v['hour_from'].(($v['hour_from']!='closed')?' - '.$v['hour_to']:'').'<br/>';
				...
				<?php
				if($row[0]['business_category'] ==1) { //var_dump($row[0]);
				
					?>
			<div class="cell_14">
				<p>Average Entree Price <br /><?=$row[0]['price_from']?>-<?=$row[0]['price_to']?></p>
				<div class="margin15"></div>
				<p>Takes Reservations: <?=($row[0]['take_reservation']==1)?'Yes': (( $row[0]['take_reservation']==0)?'No':'N/A')?></p>
			
				<p>Delivery: <?=($row[0]['delivery']==1)?'Yes':(( $row[0]['delivery']==0)?'No':'N/A')?></p>
			</div>
	<?php } ?>
			<div class="cell_15">
<?php
				if($row[0]['business_category'] ==1) {
						?>
				<p>Vegetarian: <?=($row[0]['vegetarian']==1)?'Yes': (( $row[0]['vegetarian']==0)?'No':'N/A')?> </p>
						<?php } ?>
				<p>Accepts Credit Cards: <?=($row[0]['credit_card']==1)?'Yes':(( $row[0]['credit_card']==0)?'No':'N/A')?> </p>
				<p>Parking: <?=($row[0]['parking']==1)?'Yes':(( $row[0]['parking']==0)?'No':'N/A')?></p>
				<div class="margin15"></div>
				<p>Air Conditioned: <?=($row[0]['air_conditioned']==1)?'Yes': (( $row[0]['air_conditioned']==0)?'No':'N/A')?> </p>
					<?php
					if($row[0]['business_category'] ==1) {
						?>
				<p>Serving Alcohol: <?=($row[0]['serving_alcohol']==1)?'Yes':(( $row[0]['serving_alcohol']==0)?'No':'N/A')?></p>
	<?php } ?>
			</div>
			<br />
