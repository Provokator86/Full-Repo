...
<div class="left_panel">
    <div class="section_01">
		...
		<div class="left_section">
			<div id="mygallery" class="stepcarousel">
				<div class="belt">
					...
					<div class="panel">
						<div style="height:168px; width:213px;background:url(<?=base_url().'images/uploaded/business/thumb/'.$value['img_name']?>) center no-repeat;">&nbsp;</div>
		...
		<div class="right_section">
			...
			<?=html_entity_decode( $home_page_text[0]['description'])?>
		</div>
		<div class="clear"></div>
	</div>
	...
	<div class="section_02">
		...
		<?php
		$i = 0;
		foreach($featured_business as $value) {
			$bg = ($i%2) ? '#f3f3f3' : '';
		?>
		<div class="event_box" style="background:<?=$bg?>">
			<div class="cell_01"> <a href="<?=base_url().'business/'.$value['id'].'/'.urldecode($value['name'])?>">
				<?php $img_name = ($value['cover_image'])?'uploaded/business/thumb/'.$value['cover_image'] : 'front/img_03.jpg'; ?>
				<img src="<?=base_url().'images/'.$img_name?>" alt="" width="77" />
			</a></div>
			<div class="cell_02">
				<h5><a href="<?=base_url().'business/'.$value['id'].'/'.str_replace(' ','_',$value['name'])?>"><?=$value['name']?></a></h5>
				<?php if($value['business_category'] == 1) { ?>
				<h6>Cuisine: <?=($value['cuisine'])?$value['cuisine']:'----' ?></h6>
				<?php } ?>
				<h5 style="padding-top:8px;">Editors comments</h5>
				<p style="padding-top:2px;">
				<?=($value['editorial_comments']) ? $value['editorial_comments'] : 'Not yet posted.'?>
				</p>
			</div>
			<div class="clear"></div>
		</div>
