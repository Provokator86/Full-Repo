...
class Blog_comment_model extends MY_Model implements InfModel
{
    private $conf;
        ...
        try
        {
          ...
          if($rs->num_rows()>0)
          {
              foreach($rs->result() as $row)
              {
                  ...
                  $ret_[$i_cnt]["i_blog_id"]        =   intval($row->i_blog_id); 
                  ...
                  $ret_[$i_cnt]["s_comment"]        =   nl2br(stripslashes(htmlspecialchars_decode($row->s_comment))); 
                  $ret_[$i_cnt]["dt_created_on"]    =   date("d/m/Y",intval($row->dt_entry_date)); 
