        ...
        try
        {
          ...
          if($rs->num_rows()>0)
          {
              foreach($rs->result() as $row)
              {
                  ...
                  //$ret_["s_sitename"]				=	stripslashes($row->s_sitename); 
         ...
         // $rs=$this->db->query($s_qry);
          if($rs->num_rows()>0)
          {
              foreach($rs->result() as $row)
              {
				  ...
				  $ret_["i_records_per_page"]		=	stripslashes($row->i_records_per_page); 
				  $ret_["i_default_language"]		=	intval($row->i_default_language);   
				  $ret_["d_subscription_amt"]		=	doubleval($row->d_subscription_amt);
				  $ret_["i_subscrption_status"]		=	intval($row->i_subscrption_status);    
    ...
    public function edit_info($info,$i_id)
    {
        try
        {
            ...
            if(!empty($info))
            {
				...
				$s_qry.=", d_subscription_amt=? ";
				$s_qry.=", i_subscrption_status =? ";
                //$s_qry.=", s_sitename=? ";
				//$s_qry.=", s_title=? ";
                ...
                $this->db->query($s_qry,array(		
												...
												trim($info["d_subscription_amt"]),
												trim($info["i_subscrption_status"]),
