...
class Manage_blog_model extends MY_Model implements InfModel
{
    private $conf;
          ...
          if($rs->num_rows()>0)
          {
              foreach($rs->result() as $row)
              {
                  ...
                  if(strlen($s_desc)>500)
                      $s_desc = substr_replace($s_desc,'...',503);
                  $ret_[$i_cnt]["s_description"]    = nl2br($s_desc);
                  $ret_[$i_cnt]["s_image"]                  =stripslashes(htmlspecialchars_decode($row->s_image));   
                  $ret_[$i_cnt]["dt_created_on"]    =date($this->conf["site_date_format"],intval($row->dt_entry_date)); 
                  $ret_[$i_cnt]["fn_created_on"]    =date("d/m/Y",intval($row->dt_entry_date));
				  
				  $ret_[$i_cnt]["blog_created_on"]    =date("d F,Y",intval($row->dt_entry_date));
        ...
        try
        {
          ...
          if($rs->num_rows()>0)
          {
              foreach($rs->result() as $row)
              {
                  ...
                  $s_desc = stripslashes(htmlspecialchars_decode($row->s_description));
                  $ret_["s_desc_full"] = nl2br($s_desc);
                  if(strlen($s_desc)>500)
                      $s_desc = substr_replace($s_desc,'...',503);
                  $ret_["s_description"]= nl2br($s_desc) ;   
                  ...
                  $ret_["blog_created_on"]    =date("d F,Y",intval($row->dt_entry_date)); 
        ...
        try
        {
            ...
            if(!empty($info))
            {
                ...
                $this->db->query($s_qry,array(
                                                  ...
                                                  trim(htmlspecialchars($info["s_image"], ENT_QUOTES, 'utf-8')),
    ...
    public function edit_info($info,$i_id)
    {
        try
        {
            ...
            if(!empty($info))
            {
                ...
                $this->db->query($s_qry,array(trim(htmlspecialchars($info["s_title"], ENT_QUOTES, 'utf-8')),
                                                      trim(htmlspecialchars($info["s_description"], ENT_QUOTES, 'utf-8')),
                                                      trim(htmlspecialchars($info["s_image"], ENT_QUOTES, 'utf-8')),
                                                      intval($info["i_status"]),
                                                      intval($info["dt_entry_date"]),
                ...
                if($i_ret_)
                {
                    ...
                    $logi["sql"]= serialize(array($s_qry,array(trim(htmlspecialchars($info["s_title"], ENT_QUOTES, 'utf-8')),
                                                      trim(htmlspecialchars($info["s_description"], ENT_QUOTES, 'utf-8')),
                                                      trim(htmlspecialchars($info["s_image"], ENT_QUOTES, 'utf-8')),
                                                      intval($info["i_status"]),
                                                      intval($info["dt_entry_date"]),
