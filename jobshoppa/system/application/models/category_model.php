...
class Category_model extends MY_Model implements InfModel
{
    ...
    */
	...
	public function fetch_buyer_faq_content($s_where=null,$i_start=null,$i_limit=null,$lang_query)
    {
        try
        {
          $ret_=array();
	...
	public function fetch_tradesman_faq_content($s_where=null,$i_start=null,$i_limit=null,$lang_query)
    {
        try
        {
          $ret_=array();
    ...
    public function fetch_this($i_id)
    {
        try
        {
         ...
         $s_qry="SELECT c.s_category_name, c.s_category_type, c.i_parent_id, 
		 			c.dt_created_on,c.i_status FROM 
		  		    ".$this->tbl." c "
	...
	*/
    ...
    function get_cat_selectlist($item_type='',$current_cat_id=0, $count=0,$show_child=-1)
    {
		...
		/* FE listing of top category */
	 public function fetch_top_category($s_where=null,$i_start=null,$i_limit=null)
    {
        try
        {
          $ret_=array();
		 // $cond = " AND j.i_status!=0 AND (j.i_status=1 OR j.i_status=8 OR j.i_status=4) AND j.i_is_deleted=0";
		  /*$s_qry="SELECT DISTINCT c.s_category_name,c.* FROM ".$this->tbl." c "
		  		." INNER JOIN ".$this->tbl_jobs." j ON c.id = j.i_category_id  "
                .($s_where!=""?$s_where:"" )
				."{$cond}".(is_numeric($i_start) && is_numeric($i_limit)?" Limit ".intval($i_start).",".intval($i_limit):"" );*/
				
				
				
			$s_qry = "SELECT DISTINCT c.s_category_name,j.i_no_job,c.* FROM jobshoppa_category c "
				."INNER JOIN (SELECT COUNT(*) i_no_job,jj.i_category_id,jj.i_is_deleted,jj.i_status FROM jobshoppa_jobs jj 
				WHERE jj.i_status!=0 AND (jj.i_status=1 || jj.i_status=4 || jj.i_status=8) AND jj.i_is_deleted=0 
				GROUP BY(i_category_id) ) j "
				." ON c.id = j.i_category_id "
				.($s_where!=""?$s_where:"" )				
				." ORDER BY j.i_no_job DESC "
				.(is_numeric($i_start) && is_numeric($i_limit)?" Limit ".intval($i_start).",".intval($i_limit):"" );	
