...
class Subscribe_payment_model extends MY_Model implements InfModel
{
    private $conf;
    ...
    public function __construct()
    {
        try
        {
		  ...
		  $this->tbl_pay = $this->db->SUBSCRIPTION_PAYMENT;   
    ...
    */
	 ...
	 public function fetch_multi_sorted_list($s_where=null,$order_name,$order_by,$i_start=null,$i_limit=null)
    {
        ...
        {
          	$ret_=array();
		    $s_qry="SELECT n.*,n.i_user_id AS i_tradesman_id,
          ...
          if($rs->num_rows()>0)
          {
              foreach($rs->result() as $row)
              {
				  ...
				  $ret_[$i_cnt]["d_pay_amount"] 		= doubleval($row->d_amt);
				  $ret_[$i_cnt]["s_pay_amount"] 		= doubleval($row->d_amt).' '.$this->conf["default_currency"];
				  $ret_[$i_cnt]["s_transaction_id"] 	= get_unformatted_string($row->s_transaction_id);
				  $ret_[$i_cnt]["i_invoice_no"] 		= get_unformatted_string($row->s_invoice_no);
    ...
    */
	 ...
	 public function gettotal_amount($s_where=null)
    {
        ...
        {
          $ret_=0;
          $s_qry="Select sum(d_amt) as i_total "
                ."From ".$this->tbl_pay." n "
                .($s_where!=""?$s_where:"" );
    ...
    */
	...
	public function fetch_this_payment($i_id)
    {
        try
        {
          $ret_=array();
          ////Using Prepared Statement///
          $s_qry="Select * "
                ."From ".$this->tbl_pay." AS n "
                ." Where n.s_invoice_no=?";
          ...
          if($rs->num_rows()>0)
          {
              foreach($rs->result() as $row)
              {
				  ...
				  $ret_["d_pay_amount"] 		= doubleval($row->d_amt);
				  $ret_["s_pay_amount"] 		= doubleval($row->d_amt).' '.$this->conf["default_currency"];
				  $ret_["s_transaction_id"] 	= get_unformatted_string($row->s_transaction_id);
				  $ret_["i_invoice_no"] 		= get_unformatted_string($row->s_invoice_no);
                  $ret_["dt_payment_date"]		= date($this->conf["site_date_format"],intval($row->i_created_on));
