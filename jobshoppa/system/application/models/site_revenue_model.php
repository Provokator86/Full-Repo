...
class Site_revenue_model extends MY_Model implements InfModel
{
    private $conf;
    ...
    public function __construct()
    {
        try
        {
		  ...
		  $this->tbl_sub_pay = $this->db->SUBSCRIPTION_PAYMENT; 
    ...
    */
        ...
        try
        {
          ...
          if($rs->num_rows()>0)
          {
              foreach($rs->result() as $row)
              {
				  ...
				  $ret_[$i_cnt]["s_pay_amount"] 		= doubleval($row->d_pay_amount).' '.$this->conf["default_currency"];
	...
	/* query for both commission report plus subscription report */
	public function fetch_multi_total_site_revenue($s_where=null,$i_start=null,$i_limit=null)
    {
        try
        {
          	$ret_=array();
			$s_qry="SELECT pay.user_id,pay.i_job_id,pay.amount,pay.payment_date,IFNULL(j.s_title,'membership') job_title,ut.s_name FROM ( SELECT com.i_user_id user_id,com.i_job_id ,com.d_pay_amount amount,com.i_invoice_no invoice,com.i_payment_date payment_date  FROM jobshoppa_payment_history com  
UNION 
SELECT tsp.i_user_id user_id,'',tsp.d_amt amount,tsp.s_invoice_no invoice,tsp.i_created_on payment_date  FROM jobshoppa_subcription_payment tsp ) pay  
LEFT JOIN jobshoppa_jobs j ON j.id = pay.i_job_id LEFT JOIN jobshoppa_user ut ON ut.id = pay.user_id   "
                .($s_where!=""?$s_where:"" )." ORDER BY payment_date DESC ".(is_numeric($i_start) && is_numeric($i_limit)?" Limit ".intval($i_start).",".intval($i_limit):"" );				
          ...
          if($rs->num_rows()>0)
          {
              ...
              {
                  //$ret_[$i_cnt]["id"]=$row->id;////always integer
                  $ret_[$i_cnt]["s_job_title"] 			= get_unformatted_string($row->job_title); 
				  //$ret_[$i_cnt]["s_buyer_name"] 		= get_unformatted_string($row->s_buyer_name); 
				  //$ret_[$i_cnt]["i_buyer_id"] 			= intval($row->i_buyer_id); 
				  $ret_[$i_cnt]["i_job_id"] 			= intval($row->i_job_id); 
				  $ret_[$i_cnt]["i_tradesman_id"] 		= intval($row->user_id); 
				  $ret_[$i_cnt]["s_tradesman_name"] 	= get_unformatted_string($row->s_name);
				  $ret_[$i_cnt]["d_pay_amount"] 		= doubleval($row->amount);
				  
				  $ret_[$i_cnt]["s_quote_amount"] 		= doubleval($row->d_quote_amount ).' '.$this->conf["default_currency"];
				  $ret_[$i_cnt]["s_pay_amount"] 		= doubleval($row->amount).' '.$this->conf["default_currency"];
				  $ret_[$i_cnt]["s_transaction_id "] 	= get_unformatted_string($row->s_transaction_id);
				  $ret_[$i_cnt]["i_invoice_no"] 		= get_unformatted_string($row->invoice);
                  $ret_[$i_cnt]["dt_payment_date"]		= date($this->conf["site_date_format"],intval($row->payment_date ));
                  $i_cnt++;
              }    
              $rs->free_result();          
          }
