    ...
    public function fetch_buyer_image($s_where=null,$i_start=null,$i_limit=null)
    {
        ...
        {	
			...
			$s_qry="SELECT n.* 					
                ...
                .($s_where!=""?$s_where:"" )." ORDER BY n.id DESC".(is_numeric($i_start) && is_numeric($i_limit)?" Limit ".intval($i_start).",".intval($i_limit):"" );
          //echo $s_qry;
		  $rs=$this->db->query($s_qry);
          $i_cnt=0;
          if($rs->num_rows()>0)
    ...
    */
    public function fetch_this($i_id)
    {
        ...
        {			
          ...
          $CI->load->model('newsletter_subscribers_model');
          $ret_=array();
		...
		//echo $this->db->last_query();exit;
          ...
          if($rs->num_rows()>0)
          {
              foreach($rs->result() as $row)
              {
				  ...
				  $ret_["image"]			= 	$this->fetch_buyer_image($s_where,0,1);
                  $ret_["chk_newletter"]    =    $CI->newsletter_subscribers_model->gettotal_info($s_where);  
				  $ret_["i_dob"]			=	intval($row->i_dob);
				  $ret_["dt_dob"]			=	date($this->conf["site_date_format"],intval($row->i_dob)); 
				  $ret_["s_dob"]			=	($row->i_dob>0)?date('d/m/Y',intval($row->i_dob)):''; 
            ...
            if(!empty($info))
            {
                ...
                $s_qry.=" Where id=? ";
                 ...
                 $i_ret_ = $this->db->query($s_qry,array(
                                                  ...
                                                  get_formatted_string($info["s_landline"]),
                                                  get_formatted_string($info["s_address"]),
                                                  intval(decrypt($info["i_province_id"])),
                                                  intval(decrypt($info["i_city_id"])),
                                                  intval(decrypt($info["i_zipcode_id"])),
                                                  get_formatted_string($info["s_lat"]),
                                                  get_formatted_string($info["s_lng"]),
												  intval($i_id)
        ...
        try
        {
            ...
            if(!empty($info))
            {
                ...
                if($i_ret_)
                {
                    ...
                    $this->log_info($logi); 
                    $this->db->query(" DELETE FROM ".$this->tbl_news_subscript." WHERE i_user_id=".$i_id);
                    if($info["chk_newletter"])
                    {
                        $CI = get_instance();
                        $CI->load->model('newsletter_subscribers_model','mod_news');
						//$new_id = $CI->mod_news->add_info($info);
						/* added by mrinmoy */
						$s_sub = " WHERE n.s_email='".$info['s_email']."' ";
						$is_subscribed = $CI->newsletter_subscribers_model->gettotal_info($s_sub);
						$exist_id = $CI->newsletter_subscribers_model->fetch_multi($s_sub);
						if($is_subscribed<=0)
						{
                        	$new_id = $CI->mod_news->add_info($info);
						}
						...
						{
							$chk_id = $CI->mod_news->edit_info($info,$exist_id[0]['id']);
						}
                    }   
