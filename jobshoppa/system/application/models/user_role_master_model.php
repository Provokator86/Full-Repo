...
class User_role_master_model extends MY_Model implements InfModel
{
    private $conf;
        ...
        try
        {
          ...
          if($rs->num_rows()>0)
          {
              foreach($rs->result() as $row)
              {
                  ...
                  $ret_[$i_cnt]["s_role"]=stripslashes(htmlspecialchars_decode($row->s_role)); 
        ...
        try
        {
          ...
          if($rs->num_rows()>0)
          {
              foreach($rs->result() as $row)
              {
                  ...
                  $ret_["s_role"]=stripslashes(htmlspecialchars_decode($row->s_role)); 
        ...
        try
        {
            ...
            if(!empty($info))
            {
                ...
                $this->db->query($s_qry,array(
												  trim(htmlspecialchars($info["s_role"], ENT_QUOTES, 'utf-8')),
    ...
    public function edit_info($info,$i_id)
    {
        try
        {
            ...
            if(!empty($info))
            {
                ...
                $this->db->query($s_qry,array(trim(htmlspecialchars($info["state"], ENT_QUOTES, 'utf-8')),
                                                      intval($info["i_status"]),
                                                      intval($info["dt_entry_date"]),
													  intval($i_id)
                ...
                if($i_ret_)
                {
                    ...
                    $logi["sql"]= serialize(array($s_qry,array(trim(htmlspecialchars($info["state"], ENT_QUOTES, 'utf-8')),
                                                      intval($info["i_status"]),
                                                      intval($info["dt_entry_date"]),
													  intval($i_id)
