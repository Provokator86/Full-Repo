...
class Job_model extends MY_Model implements InfModel
{
    ...
    public function __construct()
    {
        try
        {
		  ...
		  $this->tbl_subscription_pmt=$this->db->SUBSCRIPTION_PAYMENT;
    ...
    */
    public function fetch_multi($s_where=null,$i_start=null,$i_limit=null)
    {
        ...
        {
          	$ret_=array();
			$s_qry = "SELECT n.*, z.postal_code, s.state, c.city, z.latitude , z.longitude, 
					         cat.s_category_name AS s_category_name, tradesman.s_username,  tradesman.s_email, 
			                 tradesman.s_address, tradesman.s_contact_no, tradesman.s_skype_id, tradesman.s_msn_id, 		
							 tradesman.s_yahoo_id,  
							 buyer.s_username s_buyer_name, buyer.s_email s_buyer_email,
							 buyer.s_address s_buyer_address , 
							 buyer.s_contact_no s_buyer_contact_no, buyer.s_skype_id s_buyer_skype_id, 
							 buyer.s_msn_id s_buyer_msn_id, buyer.s_yahoo_id s_buyer_yahoo_id
					 FROM ".$this->tbl." n "
          ...
          if($rs->num_rows()>0)
          {
              ...
              foreach($rs->result() as $row)
              {
				  ...
				  $ret_[$i_cnt]["s_days_left"] = ($ret_[$i_cnt]["i_days_left"]>0) ? $ret_[$i_cnt]["i_days_left"].' days' :' 0 day';
				  ...
				  $ret_[$i_cnt]["dt_assigned_date"]= ($row->i_assigned_date) ? date($this->conf["site_date_format"],intval($row->i_assigned_date)):''; 
				  $s_where = " WHERE i_job_id={$row->id} And i_status!=3";
				  $ret_[$i_cnt]["i_quotes"]=$this->gettotal_job_quote_info($s_where);
				  $s_where = " WHERE i_job_id={$row->id} And i_status=1";
				  $ret_[$i_cnt]["i_interested"]=$this->gettotal_job_watchlist($s_where);
				  ...
				  $ret_[$i_cnt]["s_skype_id"]= stripslashes(htmlspecialchars_decode($row->s_skype_id)); 
				  $ret_[$i_cnt]["s_msn_id"]= stripslashes(htmlspecialchars_decode($row->s_msn_id)); 
				  $ret_[$i_cnt]["s_yahoo_id"]= stripslashes(htmlspecialchars_decode($row->s_yahoo_id)); 
				  
				  $ret_[$i_cnt]["s_buyer_name"]=stripslashes(htmlspecialchars_decode($row->s_buyer_name)); 
				  $ret_[$i_cnt]["s_buyer_email"]=stripslashes(htmlspecialchars_decode($row->s_buyer_email)); 
				  $ret_[$i_cnt]["s_buyer_address"]=stripslashes(htmlspecialchars_decode($row->s_buyer_address)); 
				  $ret_[$i_cnt]["s_buyer_contact_no"]= stripslashes(htmlspecialchars_decode($row->s_buyer_contact_no)); 
				  $ret_[$i_cnt]["s_buyer_skype_id"]= stripslashes(htmlspecialchars_decode($row->s_buyer_skype_id)); 
				  $ret_[$i_cnt]["s_buyer_msn_id"]= stripslashes(htmlspecialchars_decode($row->s_buyer_msn_id)); 
				  $ret_[$i_cnt]["s_buyer_yahoo_id"]= stripslashes(htmlspecialchars_decode($row->s_buyer_yahoo_id)); 
				  $ret_[$i_cnt]["buyer_dtails"]= $CI->manage_buyers_model->fetch_this($row->i_buyer_user_id);				  
				  ...
				  $ret_[$i_cnt]["s_job_status"]= $this->db->ALIAS_JOBSTATUS[$row->i_status];
    ...
    public function fetch_home_page_multi($s_where=null,$i_start=null,$i_limit=null)
    {
        ...
        try
        {
			...
			$s_qry = "SELECT n.*, z.postal_code, s.state, c.city, z.latitude , z.longitude, 
					...
					." LEFT JOIN {$this->tbl_user} buyer ON buyer.id = n.i_buyer_user_id"
					//." LEFT JOIN {$this->tbl_job_invitation} inv ON n.id = inv.i_job_id"
                .($s_where!=""?$s_where:"" )." ORDER BY n.i_completed_date DESC" .(is_numeric($i_start) && is_numeric($i_limit)?" Limit ".intval($i_start).",".intval($i_limit):"" );
          $rs=$this->db->query($s_qry);
		  //echo $s_qry;
          $i_cnt=0;
          if($rs->num_rows()>0)
    ...
    public function fetch_job_invitation_multi($s_where=null,$i_start=null,$i_limit=null)
    {		
        try
        {
          	$ret_=array();
	...
	public function fetch_multi_completed($s_where=null,$i_start=null,$i_limit=null)
    {
        ...
        try
        {
			...
			$s_qry = "SELECT n.*, z.postal_code, s.state, c.city, z.latitude , z.longitude,cat.s_category_name AS s_category_name, 
	...
	public function fetch_multi_completed_for_radar_cron($s_where=null,$i_start=null,$i_limit=null)
    {
        ...
        {
          	$ret_=array();
			$start_time =time() -21600;//strtotime(date('Y-m-d H:i:s'));	
			$end_time	= 	time();
			$s_qry = "SELECT n.*, z.postal_code, s.state, c.city, z.latitude , z.longitude, cat.s_category_name AS s_category_name,
					  tradesman.s_username,  tradesman.s_email, tradesman.s_address, tradesman.s_contact_no, buyer.s_username AS s_buyer_name
					 FROM ".$this->tbl." n "
	...
	public function fetch_multi_subscription_payment($s_where=null,$order_name,$order_by,$i_start=null,$i_limit=null)
    {
		
        try
        {
          	$ret_=array();
			$s_qry = "SELECT n.*,t.s_username,t.s_email,t.s_address							
					 FROM ".$this->tbl_subscription_pmt." n "
					//." LEFT JOIN {$this->tbl_cat} cat ON n.i_category_id = cat.id"
					//." LEFT JOIN {$this->tbl_zipcode} z ON n.i_zipcode_id = z.id"
					//." LEFT JOIN {$this->tbl_state} s ON n.i_province_id = s.id"
					//." LEFT JOIN {$this->tbl_city} c ON n.i_city_id = c.id"
					." LEFT JOIN {$this->tbl_user} AS t ON t.id = n.i_user_id"
          ...
          {	  
              ...
              {
                  $ret_[$i_cnt]["id"]=$row->id;////always integer	
				  $ret_[$i_cnt]["dt_payment_date"]		= date($this->conf["site_date_format"],intval($row->i_created_on));	
				  $ret_[$i_cnt]["s_transaction_id"]	=	stripslashes(htmlspecialchars_decode($row->s_transaction_id)); 	
				  $ret_[$i_cnt]["s_pay_amount"]	=	doubleval($row->d_amt);   
	...
	public function gettotal_subscription_payment_info($s_where=null)
    {
        try
        {
          $ret_=0;
          $s_qry="Select count(*) as i_total "
                ."From ".$this->tbl_subscription_pmt." n "
					...
					//." LEFT JOIN {$this->tbl_state} s ON n.i_province_id = s.id"
					//." LEFT JOIN {$this->tbl_city} c ON n.i_city_id = c.id"
					." LEFT JOIN {$this->tbl_user} t ON t.id = n.i_user_id"
					.($s_where!=""?$s_where:"" );
          $rs=$this->db->query($s_qry);
          $i_cnt=0;
          if($rs->num_rows()>0)
    ...
    */
    public function fetch_this($i_id,$i_lang_id=1)
    {
        try
        {
          ...
          $s_qry="Select n.*, z.postal_code, s.state, c.city, z.latitude s_latitude, z.longitude s_longitude, 
		  			u.s_username,u.s_email, u.s_lat s_buyer_latitide, u.s_lng s_buyer_longitude,trades.id AS i_tradesman_id, trades.s_username s_tradesman_name"
                ." From ".$this->tbl." AS n "
          ...
          if($rs->num_rows()>0)
          {
              foreach($rs->result() as $row)
              {
                  ...
                  $ret_["i_province_id"]                =   intval($row->i_province_id); 
				  $ret_["i_city_id"]				=	intval($row->i_city_id);
                  $ret_["i_zipcode_id"]             =   intval($row->i_zipcode_id); 
				  $ret_['s_category']				=	$this->get_category_name($ret_["i_category_id"]);	  
                  $ret_["s_title"]					=	stripslashes(htmlspecialchars_decode($row->s_title)); 
				  $ret_["s_description"]			=	strip_tags(stripslashes(htmlspecialchars_decode($row->s_description))); 
				  ...
				  $s_where = " WHERE i_job_id={$row->id}"; 
				  $ret_["i_interested"]				=	$this->gettotal_job_watchlist($s_where); 
                  ...
                  $ret_["i_available_option"]       =   intval($row->i_available_option); 
                  $ret_["s_available_time"]         =   get_unformatted_string($row->s_available_time); 
	...
	public function fetch_invoice_details($s_where=null,$i_start=null,$i_limit=null)
    {
        ...
        {
          	$ret_=array();
			$s_qry = "SELECT n.*, z.postal_code, s.state, c.city,pay.d_quote_amount,trades.s_business_name,pay.i_payment_date,pay.d_pay_amount,pay.i_invoice_no,tradesman.s_username,tradesman.s_name, tradesman.s_address 
					 FROM ".$this->tbl." n "
					." LEFT JOIN {$this->tbl_zipcode} z ON n.i_zipcode_id = z.id"
					." LEFT JOIN {$this->tbl_state} s ON n.i_province_id = s.id"
					." LEFT JOIN {$this->tbl_city} c ON n.i_city_id = c.id"
					." LEFT JOIN {$this->tbl_user} tradesman ON tradesman.id = n.i_tradesman_id"
					." LEFT JOIN {$this->tbl_tradesman_detail} trades ON trades.i_user_id = n.i_tradesman_id"
          ...
          if($rs->num_rows()>0)
          {
              foreach($rs->result() as $row)
              {
				  ...
				  $ret_[$i_cnt]["s_quote_price"]= doubleval($row->d_quote_amount).' '.$this->conf["default_currency"];
				  $ret_[$i_cnt]["dt_payment_date"]= date($this->conf["site_date_format"],intval($row->i_payment_date));
				  $ret_[$i_cnt]["s_paid_amount"]= ($row->d_pay_amount);
	...
	function get_category_name($i_id)
	{
        ...
        try
        {
          $ret_=array();
          ////Using Prepared Statement///
          $s_qry="Select n.id,n.s_category_name AS s_category_name "
    ...
    public function add_info($info)
    {
        ...
        try
        {
            ...
            if(!empty($info))
            {
				...
				$s_qry.=", s_keyword=? ";
				$s_qry.=", i_available_option=? ";
				$s_qry.=", s_available_time=? ";
                ...
                $this->db->query($s_qry,array(	 
												  ...
												  intval($info["i_available_option"]),
												  get_formatted_string($info["s_available_time"])
    ...
    */
    ...
    function set_job_update_all($arr,$i_id)
    {
        ...
        if($arr['job']['i_buyer_user_id'] && $arr['job']['i_buyer_user_id']!='' && is_numeric($arr['job']['i_buyer_user_id']))
        {
            ...
            if($arr['img'])
            {
                foreach($arr['img'] as $value)
                {
                    ...
                    $arr_img = array("i_job_id"=>$i_id,"s_file_name"=>$value,"s_type"=>$type);
                    $this->add_job_file_info($arr_img);
                }
            }
...
/*            $arr1 = array();
	...
	/* set datas for repost the expire job as  new job*/
	function set_expire_to_new_job_update_all($arr,$i_id)
    {
        ...
        if($arr['job']['i_buyer_user_id'] && $arr['job']['i_buyer_user_id']!='' && is_numeric($arr['job']['i_buyer_user_id']))
        {
            ...
            $this->update_expire_job_to_new_info($arr['job'],$i_id);    
            if($arr['img'])
            {
                foreach($arr['img'] as $value)
                {
...
/*            $arr1 = array();
	 ...
	 public function update_expire_job_to_new_info($info,$i_id)
    {
        //var_dump($info);exit;
        try
        {
    ...
    */
	...
	function gettotal_job_watchlist($s_where=null)
	{
        try
        {
          $ret_=0;
          $s_qry="Select count(*) as i_total "
                ."From ".$this->db->JOB_WATCHLIST." n "
                .($s_where!=""?$s_where:"" );
          $rs=$this->db->query($s_qry);
		 // echo $this->db->last_query(); exit;
          $i_cnt=0;
	...
	/* get quote list*/
    public function fetch_quote_multi($s_where=null,$i_start=null,$i_limit=null,$order_by="n.i_created_date DESC")
    {
		
        try
        {
          	$ret_=array();
			$s_qry = "SELECT n.*, tradesman.s_username, tradesman.s_name, tradesman.s_email, tradesman.id i_tradesman_id, 
					 		c.city, tradsman_detail.f_positive_feedback_percentage, tradsman_detail.i_feedback_rating
					 FROM ".$this->tbl_job_quotes." n "
					." LEFT JOIN {$this->tbl_user} tradesman ON n.i_tradesman_user_id = tradesman.id"
					." LEFT JOIN {$this->db->TRADESMANDETAILS} tradsman_detail ON tradsman_detail.i_user_id=tradesman.id"
					." LEFT JOIN {$this->tbl_city} c ON tradesman.i_city = c.id"
          ...
          if($rs->num_rows()>0)
          {
              foreach($rs->result() as $row)
              {
				  ...
				  $ret_[$i_cnt]["s_status"]	=$this->db->TESTIMONIALSTATE[$row->i_status]; 
				  $ret_[$i_cnt]["s_city"]	=get_unformatted_string($row->city); 
				  $ret_[$i_cnt]["i_feedback_rating"]=intval($row->i_feedback_rating);
				  $ret_[$i_cnt]["f_positive_feedback_percentage"]=floatval($row->f_positive_feedback_percentage);
	...
	/* get watch list*/
    public function fetch_watch_multi($s_where=null,$i_start=null,$i_limit=null)
    {
        ...
        try
        {
			...
			$s_qry = "SELECT n.*
					 FROM ".$this->db->JOB_WATCHLIST." n "
                .($s_where!=""?$s_where:"" )." ORDER BY i_created_on ASC" .(is_numeric($i_start) && is_numeric($i_limit)?" Limit ".intval($i_start).",".intval($i_limit):"" );
          $rs=$this->db->query($s_qry);
		  //echo $s_qry;
          $i_cnt=0;
          if($rs->num_rows()>0)
    ...
    public function gettotal_watchjob_info($s_where=null)
    {
        try
        {
          $ret_=0;
          $s_qry="Select count(*) as i_total "
                ."FROM ".$this->db->JOB_WATCHLIST." n "
					//." INNER JOIN {$this->tbl} job ON n.i_job_id = job.id"
	...
	public function gettotal_job_invitation($s_where=null)
    {
        try
        {
          $ret_=0;
          $s_qry="Select count(*) as i_total "
                ."FROM ".$this->tbl_job_invitation." inv "
					//." LEFT JOIN {$this->tbl_user} tradesman ON n.i_tradesman_user_id = tradesman.id"
					//." LEFT JOIN {$this->tbl_city} c ON tradesman.i_city = c.id"
                .($s_where!=""?$s_where:"" );
          $rs=$this->db->query($s_qry);
	...
	public function radar_job($id)
	{
		try
		{
			...
			if(count($info)>0)
			{
				...
				//$arr_search[] = " n.i_status=1 AND n.i_is_deleted!=1 AND n.i_category_id=".$info['i_category_id']." ";
...
/*    public function gettotal_job_watchlist($s_where=null)
    {
        try
        {
          $ret_=0;
	...
	/* get quote list*/
    public function fetch_lost_jobs_multi($s_where=null,$i_start=null,$i_limit=null)
    {		
        try
        {
          	$ret_=array();
			$s_qry = "SELECT n.*, job.s_title, job.s_description, 
							tradesman.s_username, tradesman.s_name, tradesman.s_email, tradesman.id i_tradesman_id, 
					 		c.city, tradsman_detail.f_positive_feedback_percentage, tradsman_detail.i_feedback_rating
					 FROM ".$this->tbl_job_quotes." n "
					." INNER JOIN {$this->tbl} job ON job.id = n.i_job_id" 
					." LEFT JOIN {$this->tbl_user} tradesman ON n.i_tradesman_user_id = tradesman.id"
					." LEFT JOIN {$this->db->TRADESMANDETAILS} tradsman_detail ON tradsman_detail.i_user_id=tradesman.id"
					." LEFT JOIN {$this->tbl_city} c ON tradesman.i_city = c.id"
                .($s_where!=""?$s_where:"" )." ORDER BY i_created_date DESC" .(is_numeric($i_start) && is_numeric($i_limit)?" Limit ".intval($i_start).",".intval($i_limit):"" );
    ...
    public function gettotal_lost_jobs_info($s_where=null)
    {
        try
        {
          $ret_=0;
