...
class Private_message_model extends MY_Model implements InfModel
{
    ...
    */
    public function fetch_multi($s_where=null,$i_start=null,$i_limit=null)
    {
        ...
        {	
          	$ret_=array();
			$s_qry="SELECT u.s_name as SENDER_NAME,us.s_name as RECEIVER_NAME,j.s_title,j.i_created_date AS job_create_date,n.* FROM ".$this->tbl." n 
          ...
          if($rs->num_rows()>0)
          {
              foreach($rs->result() as $row)
              {
				 ...
				 //$ret_[$i_cnt]["i_total_pmb"]				=	intval($row->i_total_pmb); 
				  //$ret_[$i_cnt]["i_new_pmb"]				=	intval($row->i_new_pmb); 
				  ...
				  $ret_[$i_cnt]["dt_created_job"]		=	date($this->conf["site_date_format"],intval($row->job_create_date));
    ...
    */
    public function fetch_multi_msg_brd($s_where=null,$i_start=null,$i_limit=null)
    {
        ...
        {	
          	$ret_=array();
			$s_qry="SELECT (SELECT COUNT(id) FROM {$this->tbl_pmb_details} WHERE i_msg_board_id=n.id) AS i_total_pmb,(SELECT COUNT(id) FROM {$this->tbl_pmb_details} WHERE i_msg_board_id=n.id AND i_status=0) AS i_new_pmb,u.s_name as SENDER_NAME,us.s_name as RECEIVER_NAME,j.s_title,n.* FROM ".$this->tbl." n "
				." INNER JOIN {$this->tbl_jobs} j ON j.id = n.i_job_id "
    ...
    */
	...
	public function total_pmb($i_job_id,$id=0)
	{
		...
		$s_qry = "SELECT COUNT(mbd1.id) AS i_total FROM jobshoppa_msg_board_details mbd1 INNER JOIN jobshoppa_msg_board mb ON  
