...
class Customer_support_model extends MY_Model implements InfModel
{
    private $conf;
    ...
    public function __construct()
    {
        ...
        {
          parent::__construct();
          $this->tbl = $this->db->CUSTOMER_SUPPORT;  
        ...
        try
        {
          ...
          if($rs->num_rows()>0)
          {
              foreach($rs->result() as $row)
              {
                  ...
                  $ret_[$i_cnt]["s_question"]=stripslashes(htmlspecialchars_decode($row->s_question)); 
                  ...
                  $ret_[$i_cnt]["i_question_cat"]= intval($row->i_question_cat); 
          ...
          unset($s_qry,$rs,$row,$i_cnt,$s_where,$s_order,$i_start,$i_limit, $s_desc);
          return $ret_;
        ...
        try
        {
          ...
          if($rs->num_rows()>0)
          {
              foreach($rs->result() as $row)
              {
                  ...
                  $ret_["i_question_cat"]= intval($row->i_question_cat);
        ...
        try
        {
          ...
          if($rs->num_rows()>0)
          {
              foreach($rs->result() as $row)
              {
                  $ret_[$i_cnt]["id"]=$row->i_id;////always integer
                  $ret_[$i_cnt]["s_question"]=stripslashes(htmlspecialchars_decode($row->s_question));     
                  $ret_[$i_cnt]["s_answer"]=stripslashes(htmlspecialchars_decode($row->s_answer)); 
    ...
    public function add_info($info)
    {
        try
        {
            ...
            if(!empty($info))
            {
                ...
                $s_qry.=", i_question_cat=? ";
                $s_qry.=", i_status=? ";
                $s_qry.=", dt_entry_date=? ";
                
                $this->db->query($s_qry,array(
                                                  trim(htmlspecialchars($info["s_question"], ENT_QUOTES, 'utf-8')),
                                                  trim(htmlspecialchars($info["s_answer"], ENT_QUOTES, 'utf-8')),
                                                  intval(decrypt($this->session->userdata("ses_question_cat"))),
    ...
    public function edit_info($info,$i_id)
    {
        try
        {
            ...
            if(!empty($info))
            {
                ...
                $this->db->query($s_qry,array(trim(htmlspecialchars($info["s_question"], ENT_QUOTES, 'utf-8')),
                                                      trim(htmlspecialchars($info["s_answer"], ENT_QUOTES, 'utf-8')), 
                ...
                if($i_ret_)
                {
                    ...
                    $logi["sql"]= serialize(array($s_qry,array(trim(htmlspecialchars($info["s_question"], ENT_QUOTES, 'utf-8')),
                                                      trim(htmlspecialchars($info["s_answer"], ENT_QUOTES, 'utf-8')), 
    ...
    */
    ...
    public function fetch_content_by_ajax_call($cat_id,$s_question)
    {
        try
        {
          $ret_=array();
          ...
          $s_qry="Select * "
                ."From ".$this->tbl." AS cs "
                ." Where cs.i_question_cat=?"
                ." And cs.s_question=?";
                
          $rs=$this->db->query($s_qry,array(intval($cat_id),$s_question));
          if($rs->num_rows()>0)
          {
              foreach($rs->result() as $row)
              {
