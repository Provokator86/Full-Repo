...
class Faq_tradesmen_model extends MY_Model implements InfModel
{
    private $conf;
        ...
        try
        {
          ...
          if($rs->num_rows()>0)
          {
              foreach($rs->result() as $row)
              {
				  ...
				  $ret_[$i_cnt]["i_faq_cat"]= intval($row->i_faq_cat); 
				  //$ret_[$i_cnt]["s_faq_cat"]=	$this->db->FAQCATEGORYTRADE[$row->i_faq_cat];
				  $ret_[$i_cnt]["s_faq_cat"]=	$this->get_category_name($row->i_faq_cat);
        ...
        try
        {
          ...
          if($rs->num_rows()>0)
          {
              foreach($rs->result() as $row)
              {
				  ...
				  $ret_["i_faq_cat"]= intval($row->i_faq_cat); 
				  //$ret_["s_faq_cat"]=$this->db->FAQCATEGORYTRADE[$row->i_faq_cat];
				  $ret_["s_faq_cat"]=	$this->get_category_name($row->i_faq_cat);
