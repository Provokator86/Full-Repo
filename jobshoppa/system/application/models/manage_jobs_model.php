        ...
        {	
          ...
          if($rs->num_rows()>0)
          {
              foreach($rs->result() as $row)
              {
				  ...
				  $ret_[$i_cnt]["i_buyer_id"]		=	intval($row->i_buyer_user_id); 
    ...
    public function fetch_multi_sorted_list($s_where=null,$order_name,$order_by,$i_start=null,$i_limit=null)
    {
        ...
        {	
			...
			$CI->load->model("job_model");
			 $s_qry="SELECT n.*,qu.s_username AS s_name,qt.s_username AS s_tradesman_name,cat.s_category_name AS s_category_name, z.postal_code, s.state, c.city, z.latitude , z.longitude FROM ".$this->tbl." n "
    ...
    public function gettotal_info($s_where=null)
    {
        try
        {
          ...
          $s_qry="Select count(*) as i_total "
                ."From ".$this->tbl." n "
				//." LEFT JOIN {$this->db->CATEGORYCHILD} cat_c ON cat_c.i_cat_id = n.i_category_id"
				." LEFT JOIN {$this->db->CATEGORY} cat ON cat.id = n.i_category_id"
                .($s_where!=""?$s_where:"" );
          $rs=$this->db->query($s_qry);
          $i_cnt=0;
          if($rs->num_rows()>0)
		...
		try
		{
			if($info['i_is_active']==1)
			{
				...
				if($is_mail_need)
				{
					...
					$tect = sendMail($details['s_email'],$mail_subject,$mail_html);					
