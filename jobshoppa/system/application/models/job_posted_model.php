...
class Job_posted_model extends MY_Model implements InfModel
{
    private $conf;
        ...
        try
        {
          ...
          if($rs->num_rows()>0)
          {
              foreach($rs->result() as $row)
              {
                  ...
                  $ret_["dt_entry_date"]			=	($row->i_created_date)?date($this->conf["site_date_format"],intval($row->i_created_date)) : '-';
				  $ret_["dt_approval_date"]			=	($row->i_admin_approval_date)?date($this->conf["site_date_format"],intval($row->i_admin_approval_date)):'-';
				  $ret_["dt_expire_date"]			=	($row->i_expire_date)?date($this->conf["site_date_format"],intval($row->i_expire_date)):'-';
    ...
    */
    ...
    public function fetch_report_multi($s_where=null,$order_name,$order_by,$i_start=null,$i_limit=null)
    {
        try
        {
			...
			$s_qry = "SELECT n.id, n.s_title, n.s_description, n.i_created_date, n.d_budget_price, n.i_status,
						...
						cat.s_category_name, u.s_username, trades_user.s_username s_tradesman_name, s.state, p.postal_code, c.city   
					 FROM ".$this->tbl." n ".
					 ...
					 " LEFT JOIN {$this->tbl_cat} cat ON cat.id = n.i_category_id".
					 " LEFT JOIN {$this->tbl_state} s ON s.id = n.i_province_id".
	...
	public function fetch_in_progress_report($s_where=null,$order_name,$order_by,$i_start=null,$i_limit=null)
    {
        try
        {
			 ...
			 $s_qry="SELECT DISTINCT n.id,n.*,u.s_name AS s_buyer_name, 
			 		p.d_pay_amount,q.d_quote,cat_c.s_category_name, w.d_waiver_amt,
				...
				." LEFT JOIN {$this->db->WAIVER_PAYMENT} w ON w.i_job_id = n.id  "
				." INNER JOIN {$this->tbl_job_quotes} q ON q.id = n.i_quote_id  "
				//." LEFT JOIN {$this->tbl_job_quotes} q ON q.id = n.i_quote_id  "
				." LEFT JOIN {$this->tbl_cat} cat_c ON cat_c.id = n.i_category_id"
				." LEFT JOIN {$this->tbl_user} u ON u.id = n.i_buyer_user_id  "
	...
	public function gettotal_in_progress_info($s_where=null)
    {
        try
        {
          ...
          $s_qry="Select count(*) as i_total "
				...
				." LEFT JOIN {$this->db->WAIVER_PAYMENT} w ON w.id = n.id  "
				." INNER JOIN {$this->tbl_job_quotes} q ON q.id = n.i_quote_id  "
				//." LEFT JOIN {$this->tbl_job_quotes} q ON q.id = n.i_quote_id  "
				." LEFT JOIN {$this->tbl_cat} cat_c ON cat_c.id = n.i_category_id"
				." LEFT JOIN {$this->tbl_user} u ON u.id = n.i_buyer_user_id  "
    ...
    public function fetch_quote_multi($s_where=null,$order_name,$order_by,$i_start=null,$i_limit=null)
    {
        ...
        {
          	$ret_=array();
		    $s_qry = "SELECT DISTINCT n.id, n.*,u.s_name, cat_c.s_category_name AS s_category_name
					 FROM {$this->tbl} n "
					 ." LEFT JOIN {$this->db->CATEGORY} cat_c ON cat_c.id = n.i_category_id "
