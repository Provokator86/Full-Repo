        ...
        try
        {
          ...
          if($rs->num_rows()>0)
          {
              foreach($rs->result() as $row)
              {
				  ...
				  $s_type=get_unformatted_string($row->s_type);
				  if($s_type == 'buyer')
				  {
				  	$ret_[$i_cnt]["s_type"]='Client'; 
				  }
				  else if($s_type == 'tradesman')
				  {
				  	$ret_[$i_cnt]["s_type"]='Professional'; 
				  }
				  else
				  {
				  	$ret_[$i_cnt]["s_type"]='General';
				  }
    ...
    public function edit_info($info,$id)
    {
        try
        {
            ...
            if(!empty($info))
            {
                ...
                $this->db->query($s_qry,array(trim(htmlspecialchars($info["s_subject"], ENT_QUOTES, 'utf-8')),
                                                      trim(htmlspecialchars($info["s_content"], ENT_QUOTES, 'utf-8')),
                                                      intval($info["dt_entry_date"]),
													  intval($info["i_status"]),
													  intval($id)
                ...
                if($i_ret_)
                {
                    ...
                    $logi["sql"]= serialize(array($s_qry,array(trim(htmlspecialchars($info["s_subject"], ENT_QUOTES, 'utf-8')),
                                                      trim(htmlspecialchars($info["s_content"], ENT_QUOTES, 'utf-8')),
                                                      intval($info["dt_entry_date"]),
													  intval($id)
