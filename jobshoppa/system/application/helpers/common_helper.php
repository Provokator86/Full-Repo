		...
		function genVerificationCode() 
				{
					 ...
					 $sql = "SELECT * FROM {$CI->db->USERMANAGE} WHERE s_verification_code='".$final_string."'";
...
/* added by mrinmoyt on 19-03-2012
* return two parts of a string with first word as first part
* and the remaining words as second part
*/

function pre_next_string($s_input)
{
	try
	{
		//echo $s_input; exit;
		preg_match('~^([\w%$#@*&-.]*)\s?([\w\s%$#@*&-.]*)?$~',$s_input,$matches);
...
function showThumbImage($image_catagory,$file_name,$width=110,$height=110)
                                {
                                    try
                                    {
                                        if($file_name=="")
                                        ...
                                        else
                                        {
                                            ...
                                            {
                                                return '<img src="'.$thumbDisplayPath.'thumb_'.$file_name.'" width="'.$width.'" height="'.$height.'">';    
                                            }
                                            else
                                            {
                                                
                                                return '<img src="'.$thumbDisplayPath.'img_not_found.gif'.'" width="'.$width.'" height="'.$height.'" alt="image not available">';    
                                            }    
                                        }
                                    }
                                    catch(Exception $err_obj)
...
function showThumbImageDefault($image_catagory,$file_name,$width=110,$height=110)
{
	try
	{
		$OBJ_CI = & get_instance();
		...
		if($file_name=="")
		{
			return '<img src="images/fe/profile_photo.png" width="'.$width.'" height="'.$height.'" alt="image not available">';    
		}
		else
		{
...
function PPHttpPost($methodName_, $nvpStr_) {
	...
	$API_UserName = urlencode('acumen_1315912442_biz_api1.gmail.com');
	$API_Password = urlencode('1315912478');
	$API_Signature = urlencode('AEHcRbYl-a7Jjp3401BP7o9AN3.XAM7OAepTrQuaOQJHU36qrefFUm3J');
