...
* Modified Date: 03Jan2011 
* Modified Date: 04 Jan 2011 [added InfControllerFe.php]
* Modified Date: 07 Jan 2011 [added FE Pagination] {changing in fe/style.css}
*
* Modified By: Iman Biswas
* Modified Date: 26 Sep 2011
    ...
    */
    public function __construct()
    {
        try{
            ...
            $this->load_defaults();
            //$this->add_css(array('style','dropdown','cssverticalmenu','jquery.tabs','jquery.tabs-ie'));
            //$this->add_js(array('jquery/jquery-1.4.2.js','jquery/ui/jquery.ui.core.js','jquery/ui/jquery-ui-1.8.4.custom.js','jquery/ui/jquery.ui.tooltip.js'));
        	...
        	if($this->router->fetch_directory() == '') // for front_end
			{
				$this->form_validation->set_error_delimiters('<div class="error_massage"><strong>','</strong></div>');
			}	
			else
			{
			...
			/*
			*Added by Iman Biswas on 27 Sep 2011
			*/
			...
			$this->i_main_default_language = $info['i_default_language'];
			$this->i_default_language = ($this->session->userdata('lang')) ? $this->session->userdata('lang') : $info['i_default_language'];
			...
			*/
			...
			$info = $this->language_model->fetch_multi(" WHERE i_status=1");
			$this->data['language_list']=$info;
			...
			$this->data['facebook_app_id'] =  $this->config->item('facebook_app_id');
			$this->data['facebook_sekret_key'] =  $this->config->item('facebook_sekret_key');
			
			
			$this->data['button'] = '<fb:login-button scope="user_activities,user_birthday,user_education_history,user_hometown,user_interests,user_likes,user_location,user_notes,user_photos,user_videos,user_photo_video_tags,user_relationships,user_relationship_details,user_religion_politics,user_status,user_website,user_work_history,email,read_stream,publish_stream,offline_access,user_events,friends_events,rsvp_event" size="large"  on-login="fblogincheck()">Connect</fb:login-button>';
  			$this->data['facebook_button'] = '<fb:login-button scope="user_activities,user_birthday,user_education_history,user_hometown,user_interests,user_likes,user_location,user_notes,user_photos,user_videos,user_photo_video_tags,user_relationships,user_relationship_details,user_religion_politics,user_status,user_website,user_work_history,email,read_stream,publish_stream,offline_access,user_events,friends_events,rsvp_event" size="xlarge"  on-login="fblogincheck()">Connect with Facebook</fb:login-button>';
  
  			$this->data['facebook_root_div'] ='<div id="fb-root"></div>';			
	...
	public function set_meta()
	{
		try
		{
			...
			if(!empty($info))
			{
				...
				$this->data['meta_revisit_after'] = $info[0]['s_revisit_after'];
				$this->data['meta_robots'] = $info[0]['s_robots'];
				$this->data['meta_classification'] = $info[0]['s_classifications'];
				$this->data['meta_expires'] = $info[0]['s_expires'];
			}
			else
			{
    ...
    protected function admin_showin_table($tbldata,$no_action= FALSE)
    {
        ...
        {	
            ...
            if(!empty($tbldata))
            {
                ...
                $tbldata["controllers_selected"]=$this->data["controllers_selected"];
    ...
    */
	public function chk_access_control()
	{
        ...
        {   
            ...
            //////Checking Access Control////     
            /////////Fetching controllers from usertype access/////
            /********For certain times*********/
            //pr($this->data['loggedin']);
            $this->load->model("User_type_model","mod_utm");
            /*product_section refers to which section "Finance" or "Insurance"*/
            $this->controller_admin=$this->mod_utm->fetch_menus(decrypt($this->data['admin_loggedin']["user_type_id"]));  
			//var_dump($this->controller_admin);			exit;									   
           /********For certain times*********/                           
            $this->data["controllers_access"]=$this->controller_admin;
            //pr($this->data["controllers_access"]) ;
            /**
            ...
            */
            ...
            ////used for showlist table format
            $this->data["controllers_selected"]=$this->data["controllers_access"][$load_controller];
            //pr($this->data["controllers_access"][$load_controller]);
            ...
            elseif(!array_key_exists($load_controller,$this->controller_admin) 
                    && !in_array($load_controller,$this->db->CONTROLLER_NO_ACCESS_REQUIRED) 
                    )////Default pages are not allowed
            {
                unset($load_controller);
