...
class Tradesman extends My_Controller
{
	 ...
	 public function edit_profile()
    {
        ...
        {	
			...
			$str_payment_type = $info['i_payment_type'];
			$posted['i_payment_type'] = explode(',',$str_payment_type);
			...
			if($_POST)
			{
				...
				/* modified by mrinmoy 15-03-2012 as required */
				//$posted["payment_type"]		    =	    trim($this->input->post("RadioGroup1"));
				$result_pay  = array();
				foreach($_POST["RadioGroup1"] as $val)
				{
				   $result_pay[]    =    $val;   
				}	
				$result_pay = implode(",", $result_pay);
				$str_pay = format_csv_string($result_pay);
				$posted["payment_type"]		    =	    $str_pay;
				/* modified by mrinmoy 15-03-2012 as required */
