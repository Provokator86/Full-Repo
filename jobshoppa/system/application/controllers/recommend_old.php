...
class Recommend extends My_Controller
{
    ...
    public function __construct()
    {
        ...
        { 
		  ...
		  $this->cls_msg=array();
		  $this->cls_msg["save_recommend"]=t("recommend send successfully.");
		  $this->cls_msg["save_recommend_err"]=t("failed to send recommend.");
		  
		  $this->cls_msg["msg_del"]	=	t("message deleted successfull");
		  $this->cls_msg["msg_del_err"]	=	t("unable to delete message");
	...
	public function buyer_recommend()
    {
        try
        {
		  ...
		  /* end job count list*/			
		  $this->data['breadcrumb'] = array('Recommend Us'=>'');
				   ...
				   $content = $this->mod_auto->fetch_contact_us_content('referral_mail','general');					
					...
					if(!empty($content))
						{							
							...
							$description = str_replace("[Buyer/Service Professional name]", $this->data['name'],$description);								
							$description = str_replace("[Click here for register]",'<a href="'.base_url().'recommend/active_account'.'/'.$s_verification_code.'">click here</a>',$description); 
				 ...
				 if($i_nwid)////saved successfully
                    {
						...
						$this->session->set_userdata(array('message'=>$this->cls_msg["save_recommend"],'message_type'=>'succ'));
