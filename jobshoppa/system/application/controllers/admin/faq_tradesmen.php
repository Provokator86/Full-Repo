...
class Faq_tradesmen extends My_Controller implements InfController
{
    ...
    public function __construct()
    {
        ...
        try
        {
          ...
          $this->load->model("faq_tradesmen_model","mod_rect");
    ...
    */
    public function show_list($order_name='',$order_by='asc',$start=NULL,$limit=NULL)
    {
        try
        {
            $this->data['heading']="FAQ For Professionals";////Package Name[@package] Panel Heading
    ...
    */
    public function add_information()           
    {
		...
		try
        {
            ...
            {
				$posted=array();
				$posted["opt_cat"]				=	trim($this->input->post("opt_cat"));
				...
				$this->form_validation->set_rules('opt_cat', 'category', 'required');
				...
				$is_data_exist = $this->mod_rect->fetch_content_by_ajax_call(decrypt($posted["opt_cat"]),$posted["txt_faq_question"]);
                ...
                else///validated, now save into DB
                {
					...
					$info["i_user_type"]	=	2;
					$info["i_faq_cat"]		=	decrypt($posted["opt_cat"]);
            ...
            ////////////end Submitted Form///////////
			$this->data['arr_cat'] = $this->db->FAQCATEGORYTRADE;
            $this->render("faq_tradesmen/add-edit");
        }
        catch(Exception $err_obj)
        {
            show_error($err_obj->getMessage());
    ...
    public function modify_information($i_id=0)
    {
        ...
        try
        {
            ...
            else
            {
				...
				$posted["opt_cat"]  		= encrypt($info["i_faq_cat"]);
    ...
    */
    public function show_detail($i_id=0)
    {
        try
        {
            ...
            $this->render("faq_tradesmen/show_detail",TRUE);
            unset($i_id);
        }
        catch(Exception $err_obj)
        {
    ...
    */
    public function ajax_checkduplicate()
    {
        try
        {
            ...
            $posted["cat_id"] = decrypt($this->input->post("h_cat_id"));
			
			
            if($posted["duplicate_value"]!="")
            {
                $qry=" Where ".(intval($posted["id"])>0 ? " n.i_id!=".intval($posted["id"])." And " : "" )
                    ." n.s_question='".$posted["duplicate_value"]."' And n.i_faq_cat=".intval($posted["cat_id"])." And n.i_user_type=2 ";
