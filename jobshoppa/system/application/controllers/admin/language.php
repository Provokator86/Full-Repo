    ...
    */
    public function show_list($start=NULL,$limit=NULL)
    {
        try
        {
            ...
            if($s_search=="basic")
            {
                $s_where=" WHERE n.s_language LIKE '%".get_formatted_string($s_language_title)."%' ";
			...
			$table_view["headers"][1]["val"]	="Default";
            ...
            for($i=0; $i<$table_view["total_rows"]; $i++)
            {
				...
				$table_view["tablerows"][$i][$i_col++]	=$info[$i]["s_selected"];
    ...
    public function add_information()           
    {
		...
		try
        {
            ...
            if($_POST)
            {
				...
				if($posted["i_default_select"]==1 && $posted["i_is_active"]==2)
				{
        ...
        try
        {
            ...
            if($posted["duplicate_value"]!="")
            {
                $qry=" Where ".(intval($posted["id"])>0 ? " n.i_id!=".intval($posted["id"])." And " : "" )
                    ." n.s_language='".$posted["duplicate_value"]."'";
