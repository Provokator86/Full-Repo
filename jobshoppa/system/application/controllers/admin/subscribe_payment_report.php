...
class Subscribe_payment_report extends My_Controller implements InfController
{
    ...
    public function __construct()
    {
        ...
        {
          parent::__construct();
          $this->data['title']="Subscription Payment Report";////Browser Title
          ...
          $this->load->model("subscribe_payment_model","mod_rect");
    ...
    */
    public function show_list($order_name='',$order_by='desc',$start=NULL,$limit=NULL)
    {
        try
        {
            $this->data['heading']="Subscription Payment";////Package Name[@package] Panel Heading
            ...
            {             
				if($opt_buyer_id!="")
				{
					$s_where.=" And n.i_user_id=".decrypt($opt_buyer_id)." ";
				}
			...
			$arr_sort = array(0=>'i_created_on',1=>'s_invoice_no');   
			// echo $order_name.'---';
			...
			//pr($info);
			 $total_amount = $this->mod_rect->gettotal_amount($s_where);
            ...
            for($i=0; $i<$table_view["total_rows"]; $i++)
            {
				...
				$link = '<a href="'.$this->pathtoclass.'showPdfInvoice/'.encrypt($info[$i]['i_tradesman_id']).'/'.$info[$i]["i_invoice_no"].'"><img src="images/fe/pdf_icon_small.gif" alt="Download Invoice" title="Download Invoice" /></a>';
    ...
    */
    ...
    {}    
	public function showPdfInvoice($i_user_id,$i_invoice_no)
    {
		
		$this->load->model('tradesman_model');
		$user_deatils = $this->tradesman_model->fetch_this(decrypt($i_user_id));
				
		//pr($user_deatils,1);			
		//$exp_date = (time()>$user_deatils['i_account_expire_date']) ? strtotime("+365 day") : strtotime ( '+365 day' , $user_deatils['i_account_expire_date'] ) ;
		$exp_date = $user_deatils['i_account_expire_date'];
		
		
        $customer_name 	= $user_deatils['s_business_name'];
		$address 		= $user_deatils['s_address'];
		$state 			= $user_deatils['s_state'];
		$city 			= $user_deatils['s_city'];
		$postal 		= $user_deatils['s_postal_code'];
		
		//$pay_detail = $this->mod_rect->fetch_this_payment(decrypt($i_user_id));
		$pay_detail = $this->mod_rect->fetch_this_payment($i_invoice_no);
		//pr($pay_detail); exit;
		$payment_date	= $pay_detail['dt_payment_date'];
		$start_date 	= $pay_detail['dt_payment_date'];
		$end_date 	 	= date("d/m/Y", $exp_date);
		$invoice_no = $pay_detail['i_invoice_no'];
		
		$paid_amount	= $pay_detail['d_pay_amount'];
