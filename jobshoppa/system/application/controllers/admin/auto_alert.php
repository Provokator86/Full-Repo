...
* @link model/auto_alert_model.php
* @link views/admin/auto_alert/
*/


class Auto_alert extends My_Controller implements InfController
{
    ...
    public function __construct()
    {
        ...
        {
          parent::__construct();
          $this->data['title']="Auto Alert Management";////Browser Title

          ////////Define Errors Here//////
          $this->cls_msg = array();
          $this->cls_msg["no_result"]="No information found about auto alert.";
          $this->cls_msg["save_err"]="Information about auto alert failed to save.";
          $this->cls_msg["save_succ"]="Information about auto alert saved successfully.";
          $this->cls_msg["delete_err"]="Information about auto alert failed to remove.";
          $this->cls_msg["delete_succ"]="Information about auto alert removed successfully.";
          ////////end Define Errors Here//////
          $this->pathtoclass = admin_base_url().$this->router->fetch_class()."/";//for redirecting from this class
		  
		  //////// loading default model here //////////////
          $this->load->model("auto_alert_model","mod_rect");
    ...
    */
    public function show_list($start=NULL,$limit=NULL)
    {
        try
        {
            $this->data['heading']="Auto Alert";////Package Name[@package] Panel Heading

            ///////////generating search query///////
			
            ////////Getting Posted or session values for search///
            $s_search=(isset($_POST["h_search"])?$this->input->post("h_search"):$this->session->userdata("h_search"));
            $s_title=($this->input->post("h_search")?$this->input->post("txt_role_title"):$this->session->userdata("txt_role_title")); 
            //$dt_created_on=($this->input->post("h_search")?$this->input->post("txt_created_on"):$this->session->userdata("txt_created_on"));
			$s_subject=($this->input->post("h_search")?$this->input->post("txt_subject"):$this->session->userdata("txt_subject")); 
            ...
            if($s_search=="basic")
            {
                $s_where=" WHERE n.s_alert_for LIKE '%".get_formatted_string($s_title)."%' ";
                /////Storing search values into session///
                $this->session->set_userdata("txt_role_title",$s_title);
                $this->session->set_userdata("h_search",$s_search);
                
                $this->data["h_search"]=$s_search;
                $this->data["txt_role_title"]=$s_title;
            ...
            elseif($s_search=="advanced")
            {
				...
				if(trim($s_subject)!="")
				{
					$s_where.=" And n.s_subject LIKE '%".get_formatted_string($s_subject)."%' ";
				}
				
                
                /////Storing search values into session///
                $this->session->set_userdata("txt_role_title",$s_title);
				$this->session->set_userdata("txt_subject",$s_subject);
                //$this->session->set_userdata("txt_created_on",$dt_created_on);
                $this->session->set_userdata("h_search",$s_search);
                
                $this->data["h_search"]=$s_search;
                $this->data["txt_role_title"]=$s_title;                
                $this->data["txt_subject"]=$s_subject;         
                /////end Storing search values into session///                
            ...
            else////List all records, **not done
            {
                ...
                $this->session->unset_userdata("txt_role_title");
                $this->session->unset_userdata("txt_subject");
                $this->session->unset_userdata("h_search");
                
                $this->data["h_search"]=$s_search;
                $this->data["txt_role_title"]="";                
            ...
            for($i=0; $i<$table_view["total_rows"]; $i++)
            {
                ...
                $table_view["tablerows"][$i][$i_col++]	=$info[$i]["s_alert_for"];
    ...
    */
    public function add_information()           
    {
        ...
        {
            $this->data['title']="Auto Alert Management";////Browser Title
            $this->data['heading']="Add Auto Alert";
            ...
            {
				$posted=array();
                $posted["txt_alert_type"]	= trim($this->input->post("txt_alert_type"));
				$posted["opt_alert_type"]	= trim($this->input->post("opt_alert_type"));
                ...
                $this->form_validation->set_rules('txt_alert_type', 'type', 'required');
				$this->form_validation->set_rules('opt_alert_type', 'type', 'required');
                ...
                {
                    $info=array();
					$info["s_alert_for"]	=	$posted["txt_alert_type"];
                    $info["s_type"]			=	decrypt($posted["opt_alert_type"]);
					$info["s_subject"]		=	$posted["txt_subject"];
					$info["s_placeholder"]	=	$posted["txt_content"];
            ...
            ////////////end Submitted Form///////////
			$this->data['arr_alert'] = $this->db->ALERTTYPE;
            $this->render("auto_alert/add-edit");
        }
        catch(Exception $err_obj)
        {
            show_error($err_obj->getMessage());
    ...
    */      
    public function modify_information($i_id=0)
    {
        ...
        try
        {
            $this->data['title']="Edit Auto Alert Details";////Browser Title
            $this->data['heading']="Edit Auto Alert";
            $this->data['pathtoclass']=$this->pathtoclass;
            $this->data['mode']="edit";
            ...
            else
            {
                ...
                $posted=array();
				$posted["txt_alert_type"]		= trim($info["s_alert_for"]);
				$posted["opt_alert_type"]		= encrypt(trim($info["s_type"]));
				$posted["txt_subject"]			= trim($info["s_subject"]);
				$posted["txt_content"]			= trim($info["s_placeholder"]);
        ...
        try
        {
            if(trim($i_id)!="")
            {
                ...
                if(!empty($info))
                {
					...
					$temp["s_alert_for"]= trim($info["s_alert_for"]);
					$temp["s_subject"]= trim($info["s_subject"]);
					$temp["s_content"]= trim($info["s_placeholder"]);
            ...
            $this->render("auto_alert/show_detail",TRUE);
            unset($i_id);
        }
        catch(Exception $err_obj)
        {
        ...
        try
        {
            ...
            if($posted["duplicate_value"]!="")
            {
                $qry=" Where ".(intval($posted["id"])>0 ? " n.i_id!=".intval($posted["id"])." And " : "" )
                    ." n.s_type='".$posted["duplicate_value"]."'";
