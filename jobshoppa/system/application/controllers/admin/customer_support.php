...
* 
* Purpose:
*  Controller For Customer Support
* 
* @package Content Management
* @subpackage Customer Support
* 
* @link InfController.php 
* @link My_Controller.php
* @link model/customer_support_model.php
* @link views/admin/customer_support/
*/


class Customer_support extends My_Controller implements InfController
{
    ...
    public function __construct()
    {
        ...
        {
          parent::__construct();
          $this->data['title']="Customer support Management";////Browser Title

          ////////Define Errors Here//////
          $this->cls_msg = array();
          $this->cls_msg["no_result"]="No information found about Customer support.";
          $this->cls_msg["save_err"]="Information about Customer support failed to save.";
          $this->cls_msg["save_succ"]="Information about Customer support saved successfully.";
          $this->cls_msg["delete_err"]="Information about Customer support failed to remove.";
          $this->cls_msg["delete_succ"]="Information about Customer support removed successfully.";
          ////////end Define Errors Here//////
          $this->pathtoclass = admin_base_url().$this->router->fetch_class()."/";//for redirecting from this class
          
          //////// loading default model here //////////////
          $this->load->model("customer_support_model","mod_rect");
    ...
    * 
    */
    public function show_list($param=NULL,$order_name='',$order_by='asc',$start=NULL,$limit=NULL)
    {
        try
        {
            
            $param=($param)?$param:$this->session->userdata("ses_question_cat");
            $this->session->set_userdata("ses_question_cat",$param);
            $type = decrypt($this->session->userdata("ses_question_cat")); 
            if($type==1)
            {
                $this->data['heading']="About professional";////Package Name[@package] Panel Heading
            } 
            else if($type==2)
            {
                $this->data['heading']="General";////Package Name[@package] Panel Heading
            }
            else
            {
                $this->data['heading']="Posting jobs";////Package Name[@package] Panel Heading
            }  
            ...
            $s_where="WHERE cs.i_del_status != 2 AND cs.i_question_cat=".$type." ";
            if($s_search=="basic")
            {
                $s_where.=" AND cs.s_question LIKE '%".get_formatted_string($s_title)."%' ";
            ...
            elseif($s_search=="advanced")
            {
                ...
                if(trim($dt_created_on)!="")
                {
                    ...
                    $s_where.=" And FROM_UNIXTIME( cs.dt_entry_date , '%Y-%m-%d' ) ='".$dt_start."' ";
                    unset($dt_start);
                }
            ...
            $table_view["caption"]="Customer support";
    ...
    public function add_information()           
    {
        ...
        try
        {
            ...
            if($_POST)
            {
                ...
                $posted["txt_question"]             =    trim($this->input->post("txt_question"));
                $posted["txt_answer"]               =    trim($this->input->post("txt_answer"));
                ...
                $this->form_validation->set_rules('txt_question', 'question', 'required');
                $this->form_validation->set_rules('txt_answer', 'answer', 'required');
                
                $is_data_exist = $this->mod_rect->fetch_content_by_ajax_call(decrypt($this->session->userdata("ses_question_cat")),$posted["txt_question"]);
                ...
                {
                    $info["s_question"]             =    $posted["txt_question"];
                    $info["s_answer"]               =    $posted["txt_answer"];
                    $info["i_status"]               =    $posted["i_is_active"];
                    $info["dt_entry_date"]          =    strtotime(date("Y-m-d H:i:s"));

                    if(!$is_data_exist)  //  INSERTING NEW  RECORD IF NOT EXIST AFTER CHECKING 
                        {
            ...
            ////////////end Submitted Form///////////
            //$this->data['arr_cat'] = $this->db->FAQCATEGORYBUYER;
            $this->render("customer_support/add-edit");
        }
        catch(Exception $err_obj)
        {
            show_error($err_obj->getMessage());
    ...
    */      
    public function modify_information($i_id=0)
    {
        ...
        try
        {
            $this->data['title']="Edit Customer support Details";////Browser Title
            ...
            else
            {
                ...
                $posted=array();
                $posted["txt_question"]     = trim($info["s_question"]);
                $posted["txt_answer"]       = trim($info["s_answer"]);
    ...
    */
    public function show_detail($i_id=0)
    {
        try
        {
            ...
            $this->render("customer_support/show_detail",TRUE);
            unset($i_id);
        }
        catch(Exception $err_obj)
        {
    ...
    function ajax_get_content()
    {
        ...
        $info = $this->mod_rect->fetch_content_by_ajax_call(decrypt($cat_id),$s_question);
        if(!empty($info))
        {
    ...
    */
    public function ajax_checkduplicate()
    {
        try
        {
            ...
            {
                $qry=" Where ".(intval($posted["id"])>0 ? " cs.i_id!=".intval($posted["id"])." And " : "" )
                    ." cs.s_question='".$posted["duplicate_value"]."' ";
