...
class State extends My_Controller implements InfController
{
    ...
    public function __construct()
    {
        ...
        {
          parent::__construct();
          $this->data['title']="County Management";////Browser Title

          ////////Define Errors Here//////
          $this->cls_msg = array();
          $this->cls_msg["no_result"]="No information found about county.";
          $this->cls_msg["save_err"]="Information about county failed to save.";
          $this->cls_msg["save_succ"]="Information about county saved successfully.";
          $this->cls_msg["delete_err"]="Information about county failed to remove.";
          $this->cls_msg["delete_succ"]="Information about county removed successfully.";
    ...
    */
    public function show_list($order_name='',$order_by='asc',$start=NULL,$limit=NULL)
    {
        try
        {
            $this->data['heading']="County";////Package Name[@package] Panel Heading

            ///////////generating search query///////
			
            ////////Getting Posted or session values for search///
            $s_search=(isset($_POST["h_search"])?$this->input->post("h_search"):$this->session->userdata("h_search"));
            $s_state_title=($this->input->post("h_search")?$this->input->post("txt_state_title"):$this->session->userdata("txt_state_title")); 
            //$dt_created_on=($this->input->post("h_search")?$this->input->post("txt_created_on"):$this->session->userdata("txt_created_on"));
            $s_state_short=($this->input->post("h_search")?$this->input->post("txt_abbreviation"):$this->session->userdata("txt_abbreviation"));
            ...
            if($s_search=="basic")
            {
                $s_where=" WHERE n.state LIKE '%".get_formatted_string($s_state_title)."%' ";
                /////Storing search values into session///
                $this->session->set_userdata("txt_state_title",$s_state_title);
                $this->session->set_userdata("h_search",$s_search);
                
                $this->data["h_search"]=$s_search;
                $this->data["txt_state_title"]=$s_state_title;
            ...
            elseif($s_search=="advanced")
            {
               ...
               /* if(trim($dt_created_on)!="")
				...
				if(trim($s_state_short)!="")
                {
                    
                    $s_where.=" And n.abbreviation LIKE '%".get_formatted_string($s_state_short)."%' ";
                    unset($dt_start);
                }
				...
				$this->session->set_userdata("txt_abbreviation",$s_state_short);
                $this->session->set_userdata("h_search",$s_search);
                
                $this->data["h_search"]=$s_search;
                $this->data["txt_state_title"]=$s_state_title;                
                //$this->data["txt_created_on"]=$dt_created_on;  
				$this->data["txt_abbreviation"]=$s_state_short;            
                /////end Storing search values into session///                
            ...
            else////List all records, **not done
            {
                ...
                $this->session->unset_userdata("txt_state_title");
                //$this->session->unset_userdata("txt_created_on");
				$this->session->unset_userdata("txt_abbreviation");
                $this->session->unset_userdata("h_search");
                
                $this->data["h_search"]=$s_search;
                $this->data["txt_state_title"]="";                
                //$this->data["txt_created_on"]=""; 
				$this->data["txt_abbreviation"]="";            
                /////end Storing search values into session///                 
			...
			$arr_sort = array(0=>'state',1=>'abbreviation',2=>'i_status'); 
            ...
            $table_view["headers"][0]["val"]	="County Name";
			//$table_view["headers"][1]["sort"]	= array('field_name'=>encrypt($arr_sort[1]));
			//$table_view["headers"][1]["val"]	="Abbreviation";
            ...
            for($i=0; $i<$table_view["total_rows"]; $i++)
            {
				...
				//$table_view["tablerows"][$i][$i_col++]	=$info[$i]["abbreviation"];
    ...
    */
    public function add_information()           
    {
		...
		try
        {
            ...
            {
				$posted=array();
                $posted["txt_state_title"]			=	trim($this->input->post("txt_state_title"));
				$posted["txt_state_description"]	=	trim($this->input->post("txt_state_description"));
				$i_active_val 						=	trim($this->input->post("i_state_is_active"));
                $posted["i_state_is_active"]		=	($i_active_val==1)?$i_active_val:2;
                $posted["h_mode"]					=	$this->data['mode'];
                $posted["h_id"]= "";
				
				
               
                $this->form_validation->set_rules('txt_state_title', 'county name', 'required');
                ...
                {
                    $info=array();
                    $info["state"]			=	$posted["txt_state_title"];
                    $info["abbreviation"]	=	$posted["txt_state_description"];
    ...
    */      
    public function modify_information($i_id=0)
    {
        ...
        try
        {
            $this->data['title']="Edit County Details";////Browser Title
            $this->data['heading']="Edit County";
            $this->data['pathtoclass']=$this->pathtoclass;
            $this->data['mode']="edit";
            ...
            else
            {
                ...
                $posted=array();
				$posted["txt_state_title"]			= trim($info["state"]);
				$posted["txt_state_description"]	= trim($info["abbreviation"]);
				
				$posted["dt_created_on"]			= trim($info["dt_created_on"]);
				$posted["i_state_is_active"]		= trim($info["i_is_active"]);
				$posted["h_id"]= $i_id;
        ...
        try
        {
            if(trim($i_id)!="")
            {
                ...
                if(!empty($info))
                {
					...
					$temp["s_state"]= trim($info["state"]);
					$temp["s_abbreviation"]= trim($info["abbreviation"]);
        ...
        try
        {
            ...
            if($posted["duplicate_value"]!="")
            {
                $qry=" Where ".(intval($posted["id"])>0 ? " n.id!=".intval($posted["id"])." And " : "" )
                    ." n.state='".$posted["duplicate_value"]."'";
