...
class Manage_buyers extends My_Controller implements InfController
{
    ...
    {            
        ...
        {
          parent::__construct();
          $this->data['title']="Clients User Management";////Browser Title

          ////////Define Errors Here//////
          $this->cls_msg = array();
          $this->cls_msg["no_result"]="No information found about client.";
          $this->cls_msg["save_err"]="Information about client failed to save.";
          $this->cls_msg["save_succ"]="Information about client saved successfully.";
          $this->cls_msg["delete_err"]="Information about client failed to remove.";
          $this->cls_msg["delete_succ"]="Information about client removed successfully.";
		  $this->cls_msg["send_err"]="Message not delivered.";
          $this->cls_msg["send_succ"]="Message delivered successfully.";
		  $this->cls_msg["status_err"]="Client status change failed.";
          $this->cls_msg["status_succ"]="Client status changed successfully.";
    ...
    */
    public function show_list($order_name='',$order_by='asc',$start=NULL,$limit=NULL)
    {
        try
        {
            $this->data['heading']="Clients";////Package Name[@package] Panel Heading
          ...
          //  $table_view["headers"][0]["width"]	="15%";
            ...
            for($i=0; $i<$table_view["total_rows"]; $i++)
            {
				...
				{
					$action .='<a  href="'.admin_base_url().'manage_buyers/change_status/'.encrypt($info[$i]["id"]).'/0">
					<img width="12" height="12" title="Inactive" alt="Inactive" src="images/admin/reject.png"></a>';
				}
				else
				{
					$action .=' <a  href="'.admin_base_url().'manage_buyers/change_status/'.encrypt($info[$i]["id"]).'/1">
        ...
        try
        {
            ...
            if($_POST)
            {
                ...
                else///validated, now save into DB
                {
                    ...
                    $info["s_verified"]		=	$posted["i_verified"];
					$info["i_is_active"]	=	$posted["i_is_active"];
                    $info["i_edited_date"]	=	strtotime(date("Y-m-d H:i:s"));
    ...
    */
    public function show_detail($i_id=0)
    {
        try
        {
            ...
            $this->render("manage_buyers/show_detail",TRUE);
            unset($i_id);
        }
        catch(Exception $err_obj)
        {
		...
		try
		{
            ...
            if($_POST)
            {
                ...
                else///validated, now save into DB
                {
                    ...
                    $info["s_content"]	=nl2br($posted["s_message"]);
