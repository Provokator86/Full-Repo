...
*/            }
    ...
    * 
    */
    public function show_report_param_list($type=NULL,$order_name='i_created_date',$order_by='asc',$start=NULL,$limit=NULL)
    {
        try
        {
			if(empty($type))
          ...
          echo  $s_where=" WHERE n.i_status=".decrypt($type)." ";
            if($s_search=="basic")
            {
/*                $s_where=" WHERE n.s_title LIKE '%".get_formatted_string($s_news_title)."%' ";
                /////Storing search values into session///
...
*/            }
	...
	public function show_closed_list($order_name='',$order_by='asc',$start=NULL,$limit=NULL)
    {
        try
        {
			if(empty($type))
	...
	public function show_won_list($order_name='',$order_by='asc',$start=NULL,$limit=NULL)
    {
        try
        {
			if(empty($type))
           ...
           $s_title=($this->input->post("h_search")?$this->input->post("s_title"):$this->session->userdata("s_title")); 
            ...
            {                
                if(trim($dt_created_on)!="")
                {
                    $dt_start=date("Y-m-d",strtotime(trim($dt_created_on." "))) ; 
                    //$s_where.=" WHERE FROM_UNIXTIME( n.i_created_date , '%Y-%m-%d' ) >='".$dt_start."' ";
					$arr_search[]="  FROM_UNIXTIME( n.i_payment_date , '%Y-%m-%d' ) >='".$dt_start."' ";
                    unset($dt_start);
                }
				if(trim($dt_to)!="")
				{
					$dt_end = date("Y-m-d",strtotime(trim($dt_to." "))) ; 
					//$s_where.=" And FROM_UNIXTIME( n.i_created_date , '%Y-%m-%d' ) <='".$dt_end."' ";
					$arr_search[]="  FROM_UNIXTIME( n.i_payment_date , '%Y-%m-%d' ) <='".$dt_end."' ";
                    unset($dt_end);
				}
				$s_where .= (count($arr_search) !=0)?' AND '.implode('AND',$arr_search):'';	
                //echo $s_where; exit;
            ...
            {
                $s_where=" WHERE  j.i_is_deleted=0 AND FROM_UNIXTIME( i_payment_date , '%Y-%m-%d' ) 
           ...
           $i_uri_seg = 7;
            ...
            ///////////end generating search query///////
             // List of fields for sorting
			$arr_sort = array(0=>'n.i_payment_date',1=>'j.s_title');
			// echo $order_name.'---';
			//  echo decrypt($order_name);
            $s_order_name = !empty($order_name)?in_array(decrypt($order_name),$arr_sort)?decrypt($order_name):$arr_sort[0]:$arr_sort[0];
			$order_name = empty($order_name)?encrypt($arr_sort[0]):$order_name;
            ...
            $info	= $this->comm_payment_model->fetch_multi_sorted_list($s_where,$s_order_name,$order_by,intval($start),$limit);
            ...
            $table_view["caption"]="Job ".$job_type;
            $table_view["total_rows"]=count($info);
			$table_view["total_db_records"]=$this->comm_payment_model->gettotal_sort_info($s_where);
			$table_view["order_name"]=$order_name;
			$table_view["order_by"]  =$order_by;
            $table_view["src_action"]= $this->pathtoclass.$this->router->fetch_method() ;  
			$table_view["detail_view"] = FALSE;
        ...
        //    $table_view["headers"][++$j_col]["val"]	="Status"; 
            ...
            for($i=0; $i<$table_view["total_rows"]; $i++)
            {
				...
				$table_view["tablerows"][$i][$i_col++]	=$info[$i]["d_quote_amount"];				
				$table_view["tablerows"][$i][$i_col++]	=$info[$i]["d_pay_amount"];
    ...
    * 
    */
    public function show_qoute_list($order_name='',$order_by='asc',$start=NULL,$limit=NULL)
    {
        try
        {
