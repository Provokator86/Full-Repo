...
class Job_overview extends My_Controller implements InfController
{
    ...
    public function show_detail($i_id = 0)
    {
        try
        {
			//echo $this->session->userdata("i_buyer_id");
				...
				$s_where = " WHERE (n.i_status = 1 OR n.i_status = 2) AND n.i_job_id = $i_id";
   ...
   public function show_terminate_detail($s_id)
    {
        try
        {
			...
			//echo $this->session->userdata("i_buyer_id");
                ...
                if(!empty($info))
                {
					...
					$temp["s_terminate_reason"]		=	trim($info["s_terminate_reason"]);
            ...
            $this->render("job_overview/show_terminate_detail");
            unset($i_id);
        }
        catch(Exception $err_obj)
        {
