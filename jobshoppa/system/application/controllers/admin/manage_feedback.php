    ...
    public function show_list($order_name='',$order_by='desc',$start=NULL,$limit=NULL)
    {
        try
        {
           ...
           // $s_cat=($this->input->post("h_search")?$this->input->post("s_cat"):$this->session->userdata("s_cat")); 
            ...
            $s_where=" WHERE j.i_is_deleted=0 AND n.i_status!=0";
            if($s_search=="basic")
            {
			...
			$arr_sort = array(0=>'s_title',2=>'i_buyer_user_id',4=>'i_created_date',6=>'i_rating');   
			// echo $order_name.'---';
            ...
            for($i=0; $i<$table_view["total_rows"]; $i++)
            {
				...
				if($info[$i]["s_sender_type"]==1)
				{
					$link = "<a alt='Click here to view profile' href='".admin_base_url().'buyers_profile/index/'.encrypt($info[$i]["i_sender_user_id"])."' target='_blank'>".$info[$i]["s_sender_user"].'</a>';			
				}
				else if($info[$i]["s_sender_type"]==2)
				{
					$link = "<a alt='Click here to view profile' href='".admin_base_url().'tradesman_profile_view/index/'.encrypt($info[$i]["i_sender_user_id"])."' target='_blank'>".$info[$i]["s_sender_user"].'</a>';
				}
				if($info[$i]["s_receiver_type"]==1)
				{
					$link2 = "<a alt='Click here to view profile' href='".admin_base_url().'buyers_profile/index/'.encrypt($info[$i]["i_receiver_user_id"])."' target='_blank'>".$info[$i]["s_receiver_user"].'</a>';
				}
				else if($info[$i]["s_receiver_type"]==2)
				{
					$link2 = "<a alt='Click here to view profile' href='".admin_base_url().'tradesman_profile_view/index/'.encrypt($info[$i]["i_receiver_user_id"])."' target='_blank'>".$info[$i]["s_receiver_user"].'</a>';
				}
    ...
    public function modify_information($i_id=0)
    {
        ...
        try
        {
            ...
            if($_POST)
            {
				...
				$posted["s_terminate_reason"]= trim($this->input->post("s_terminate_reason"));
                ...
                else///validated, now save into DB
                {
					...
					$info["s_terminate_reason"]	=	$posted["s_terminate_reason"];
                    ...
                    if($i_aff)////saved successfully
                    {
						...
						//var_dump($user_type);
						$s_where = " WHERE i_receiver_user_id ={$feed_back_deatils['i_receiver_user_id']} AND n.i_status=1 AND n.i_feedback_complete_status=1"; 
						$tot_accepted_feedback = $this->mod_rect->gettotal_info($s_where);
						
						$s_where = " WHERE i_receiver_user_id ={$feed_back_deatils['i_receiver_user_id']} AND n.i_status!=0 AND n.i_feedback_complete_status=1"; 				
						$tot_feedback = $this->mod_rect->gettotal_info($s_where);
						
						$s_where = " WHERE i_receiver_user_id ={$feed_back_deatils['i_receiver_user_id']} AND n.i_status !=0 AND n.i_feedback_complete_status=1"; 				
						$feedback_details = $this->mod_rect->fetch_feedback_rating($s_where);						
						
						$s_where = " WHERE i_receiver_user_id ={$feed_back_deatils['i_receiver_user_id']} AND n.i_status !=0 AND n.i_positive=1 AND n.i_feedback_complete_status=1" ;
						$i_positive = $this->mod_rect->fetch_feedback_positive($s_where);
						//pr($feed_back_deatils);
						//echo $user_type['i_role'].'======'.$feed_back_deatils['i_feedback_complete_status'];
						if($user_type['i_role'] == 2 && $feed_back_deatils['i_feedback_complete_status']==1)
						{
							$arr1 = array();
						...
						else if($user_type['i_role'] == 1 && $feed_back_deatils['i_feedback_complete_status']==1)
						{
							$arr1 = array();
							$arr1['i_feedback_rating'] = round($feedback_details['i_rating']);
							$arr1['f_positive_feedback_percentage'] = ($tot_feedback!=0)?round(($i_positive['i_positive']/$tot_feedback)*100):0;
            ...
            else
            {
				...
				$posted["s_terminate_reason"]= trim($info["s_terminate_reason"]);
            ...
            ////////////end Submitted Form///////////
			$this->data['arr_rating'] = ($this->data["posted"]['i_positive']==1) ? $this->db->POSITIVE_FEEDBACK_RATING : $this->db->NEGATIVE_FEEDBACK_RATING;
