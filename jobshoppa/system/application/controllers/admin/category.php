    ...
    */
    public function show_list($start=NULL,$limit=NULL)
    {
        try
        {
            ...
            if($s_search=="basic")
            {
                $s_where=" WHERE c.s_category_name LIKE '%".get_formatted_string($s_category_name)."%' ";
            ...
            elseif($s_search=="advanced")
            {
                ...
                {
                	 $arr_search[] =" c.s_category_name LIKE '%".get_formatted_string($s_category_name)."%' ";
				}	
				
				if(trim($s_category_type)!="")
                {
                    
                    $arr_search[] =" c.s_category_type ='".decrypt($s_category_type)."' ";
                    unset($dt_start);
                }
				...
				$this->data["opd_category_type"]=$s_state_short;            
                /////end Storing search values into session///                
			...
			$table_view["headers"][1]["val"]	="Category Type";
    ...
    */
    public function add_information()           
    {
		...
		try
        {
            ...
            if($_POST)
            {
               ...
               //pr($arr_lang_val,1);
                ...
                else///validated, now save into DB
                {
                    ...
                    $info["s_category_type"]	=	decrypt($posted["opd_category_type"]);
					$info["i_parent_id"]		=	decrypt($posted["opd_parent_id"]);
			...
			$this->data['parent_detail_option']=makeOption($this->mod_rect->get_cat_selectlist((isset($item_type))?$item_type:'',0,0,1));
            ////////////end Submitted Form///////////
        ...
        try
        {
            ...
            if($_POST)
            {
                ...
                else///validated, now save into DB
                {
					...
					//$info["s_category_name"]=$posted["txt_category_name"];
                    $info["s_category_type"]=decrypt($posted["opd_category_type"]);
					$info["i_parent_id"]=decrypt($posted["opt_parent_id"]);
					$info["dt_created_on"]=time();
        ...
        try
        {
            ...
            if($posted["duplicate_value"]!="")
            {
                $qry=" Where ".(intval($posted["id"])>0 ? " c.id!=".intval($posted["id"])." And " : "" )
                   ." c.s_category_name='".$posted["duplicate_value"]."' AND c.s_category_type = '".$posted["i_type"]."'";
					
					
                //$info=$this->mod_rect->fetch_multi($qry,$start,$limit); /*don't change*/
				$info=$this->mod_rect->fetch_duplicate_category($qry,$start,$limit); /*don't change*/
