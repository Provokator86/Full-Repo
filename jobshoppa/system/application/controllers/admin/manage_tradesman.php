...
class Manage_tradesman extends My_Controller implements InfController
{
    ...
    public function __construct()
    {
        ...
        try
        {
          ...
          $this->cls_msg["send_succ"]="Message delivered successfully.";
		   $this->cls_msg["status_err"]="Professional status change failed.";
          $this->cls_msg["status_succ"]="Professional status changed successfully.";
    ...
    */
    public function show_list($order_name='',$order_by='asc',$start=NULL,$limit=NULL)
    {
        try
        {
            $this->data['heading']="Professional";////Package Name[@package] Panel Heading
    ...
    */
	...
	public function show_verification_list($order_name='',$order_by='desc',$start=NULL,$limit=NULL)
    {
        try
        {
            ...
            // List of fields for sorting
			$arr_sort = array(0=>'s_username',1=>'s_name',3=>'s_email',4=>'i_created_on');   
			// echo $order_name.'---';
			//  echo decrypt($order_name);
            $s_order_name = !empty($order_name)?in_array(decrypt($order_name),$arr_sort)?decrypt($order_name):$arr_sort[4]:$arr_sort[4];
            $limit	= $this->i_admin_page_limit;
           // $info	= $this->mod_rect->fetch_multi($s_where,intval($start),$limit);
            ...
            {	
				...
				$action = '<a href="'.admin_base_url().'tradesman_profile_view/index/'.encrypt($info[$i]["id"]).'">'.'<img src="images/admin/view.png" title="View" alt="View" />'.'</a>';
				if($info[$i]["i_verifcation_status"] == 1)
				{
					$action .='<a  href="'.admin_base_url().'manage_tradesman/change_verification_status/'.encrypt($info[$i]["id"]).'/2">
					<img width="12" height="12" title="Reject" alt="Reject" src="images/admin/reject.png"></a>';
				
					$action .=' <a  href="'.admin_base_url().'manage_tradesman/change_verification_status/'.encrypt($info[$i]["id"]).'/3">
