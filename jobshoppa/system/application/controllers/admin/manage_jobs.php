...
* @link model/manage_jobs_model.php
        ...
        try
        {
            ...
            elseif($s_search=="advanced")
            {
				...
				if($s_cat!="")
				{
					...
					$s_where.=" And cat.id=".decrypt($s_cat)." ";
				}
           ...
           // $table_view["headers"][$j_col]["width"]	="20%";
			...
			$table_view["headers"][$j_col]["width"]	="20%";
	//		$table_view["headers"][5]["val"]	="Budget";
...
/* 			//$table_view["headers"][7]["align"]	="middle";
			$table_view["headers"][7]["sort"]	= array('field_name'=>encrypt($arr_sort[7]));
			
*/			$table_view["headers"][++$j_col]["val"]	="Action";
			$table_view["headers"][$j_col]["width"]	="9%";
		//	$table_view["headers"][6]["align"]	="right";
            ...
            for($i=0; $i<$table_view["total_rows"]; $i++)
            {
				...
				$address .= 		'<td>'.$info[$i]["i_quoting_period_days"].' Day(s)</td>';
				$address .= '</tr>';	
				$address .='</table>';
				$detail = $address;
				$address = '';
				...
				$address .= '</tr>';
				$address .= '<tr>';	
				$address .= 	'<td width="30%">Posta lcode: </td>';
             ...
             //   $table_view["tablerows"][$i][$i_col++]	=$info[$i]["dt_expired_on"];
				$table_view["tablerows"][$i][$i_col++]	='<a value="'.encrypt($info[$i]["id"]).'" id="btn_edit_'.$info[$i]["id"].'" href="'.$this->pathtoclass.'modify_information'.'/'.encrypt(0).'/'.encrypt($info[$i]["id"]).'"><img width="12" height="12" src="images/admin/edit_inline.gif" alt="Edit"  title="Edit" ></a>'.' <a onClick="javascript:approve('.$i.');"  href="javascript:void(0);"><img width="12" height="12" alt="Approve" title="Approve" src="images/admin/tick.png"></a>'.' <a  onClick="javascript:reject('.$i.');"  href="javascript:void(0);"><img width="12" height="12" alt="Reject" title="Reject" src="images/admin/reject.png"></a>'.' <a  href="'.admin_base_url().'job_overview/index/'.encrypt($info[$i]["id"]).'">'.'<img src="images/admin/view.png" title="View" alt="View" />'.'</a>';
			//	$table_view["tablerows"][$i]['style'] = "style='border-right:none;'";
    ...
    * 
    */
    public function show_active($order_name='',$order_by='asc',$start=NULL,$limit=NULL)
    {
        try
        {
    ...
    * 
    */
    public function show_complete($order_name='',$order_by='asc',$start=NULL,$limit=NULL)
    {
        try
        {
...
*/			$table_view["headers"][++$j_col]["val"]	="Action";
    ...
    * 
    */
    public function show_in_progress($order_name='',$order_by='asc',$start=NULL,$limit=NULL)
    {
        try
        {
    ...
    * 
    */
    public function show_all($order_name='',$order_by='asc',$start=NULL,$limit=NULL)
    {
        try
        {
...
*/			$table_view["headers"][++$j_col]["val"]	="Action";
            ...
            for($i=0; $i<$table_view["total_rows"]; $i++)
            {
             ...
             //   $table_view["tablerows"][$i][$i_col++]	=$info[$i]["dt_expired_on"];
			 $pmb_link = admin_base_url().'manage_private_message/job_message_board/'.encrypt($info[$i]["id"]);
				$table_view["tablerows"][$i][$i_col++]	=' <a title="View" href="'.admin_base_url().'job_overview/index/'.encrypt($info[$i]["id"]).'">'.'<img src="images/admin/view.png" alt="View" />'.'</a>'.' <a title="Private message board" href="'.$pmb_link.'"><img alt="Private message board" src="images/admin/pmb.png"></a>';
			//	$table_view["tablerows"][$i]['style'] = "style='border-right:none;'";
    ...
    public function modify_information($type=NULL,$i_id=0)
    {
        ...
        try
        {
            ...
            if($_POST)
            {
				...
				$this->form_validation->set_rules('i_quoting_period_days', 'quoting period', 'required');
                ...
                else///validated, now save into DB
                {
					...
					$info["i_is_active"]			=	decrypt($posted["i_is_active"]);
					
					$days	=	$info["i_quoting_period_days"];
					if($info["i_is_active"]==1)
					{
                    	$info["i_admin_approval_date"]	=	strtotime(date("Y-m-d H:i:s"));
						
						$info["i_expire_date"]	=	strtotime(date("Y-m-d H:i:s",strtotime(date("Y-m-d H:i:s")))."+{$days} days");

					}
					if($info["i_is_active"]==7)
					{
						$info["i_expire_date"] = strtotime(date("Y-m-d H:i:s"));
					}
                    ...
                    {
						$mail_send = $this->mod_rect->job_accept_reject(decrypt($posted['h_id']),$info['i_is_active']);
                        set_success_msg($this->cls_msg["save_succ"]);
                        redirect($this->pathtoclass."show_list");
                    }
                    else///Not saved, show the form again
        ...
        try
        {
            ...
            switch($s_del_these)
			{
				...
				default: 		///Deleting selected,page ///
							...
							{
								//echo decrypt($id[$tot]);
								//$s_where = " WHERE n.id=".decrypt($id[$tot]);
								$job_details = $this->job_model->fetch_this(decrypt($id[$tot]));
								
								$i_ret_=$this->mod_rect->update_status(decrypt($id[$tot]),$i_status);
								$mail_send = $this->mod_rect->job_accept_reject(decrypt($id[$tot]),$i_status);
								/* insert data to job history and stattus change*/
									if($i_status==1) // AActive
									{
										$s_message = 'job_approved';
										$s_status = 'Active';
										//$job_details['i_buyer_user_id']=1;
										$this->mod_rect->update_buyer_posted_job($job_details['i_buyer_user_id']);
									}
									else
									{
									...
									$arr1['i_created_date'] =  time();			
									$this->job_model->set_data_insert($table,$arr1);			
							/** End Job history And Status Insert**/
							...
							elseif($id>0)///Deleting single Records
							{
								...
								$mail_send = $this->mod_rect->job_accept_reject(decrypt($id),$i_status);
								/* insert data to job history and stattus change*/
									if($i_status==1) // AActive
									{
										$s_message = 'job_approved';
        ...
        try
        {
            if(trim($i_id)!="")
            {
                ...
                if(!empty($info))
                {
					...
					$temp["i_supply_material"]		=	trim($info["i_supply_material"]);
            ...
            $this->render("manage_jobs/show_detail",TRUE);
            unset($i_id);
        }
        catch(Exception $err_obj)
        {
    ...
    */
    public function show_param_list($type=NULL,$start=NULL,$limit=NULL)
    {
        try
        {
            ...
            $this->data['heading']= ucfirst(decrypt($type))." Jobs";////Package Name[@package] Panel Heading
            ...
            //$s_where=" WHERE n.s_type='".decrypt($type)."' And n.i_status!=0";
            ...
            //////Table Headers, with width,alignment///////
            $table_view["caption"]=ucfirst(decrypt($type))." Jobs";
            $table_view["total_rows"]=count($info);
			...
			$table_view["headers"][1]["val"]	="Description";
			$table_view["headers"][2]["val"]	="Client";
			$table_view["headers"][3]["width"]	="10%";
            $table_view["headers"][3]["val"]	="Post Date"; 
            $table_view["headers"][4]["val"]	="Status"; 
			$table_view["headers"][5]["val"]	="Edit";
