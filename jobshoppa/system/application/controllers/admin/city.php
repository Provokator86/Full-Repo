    ...
    */
    public function show_list($order_name='',$order_by='asc',$start=NULL,$limit=NULL)
    {
        try
        {
            ...
            $s_state=($this->input->post("h_search")?$this->input->post("opt_type_id"):$this->session->userdata("opt_type_id"));
            ...
            elseif($s_search=="advanced")
            {
                ...
                {                    
                    $s_where.=" AND c.state_id ='".decrypt($s_state)."' ";
                }
				
                
                /////Storing search values into session///
                $this->session->set_userdata("txt_city_title",$s_city_title);
				$this->session->set_userdata("opt_type_id",$s_state);
                $this->session->set_userdata("h_search",$s_search);
                
                $this->data["h_search"]=$s_search;
                $this->data["txt_city_title"]=$s_city_title;                
                //$this->data["txt_created_on"]=$dt_created_on;  
				$this->data["opt_type_id"]=$s_state;            
                /////end Storing search values into session///                
			...
			$arr_sort = array(0=>'city',1=>'state',2=>'abbreviation'); 
			...
			$table_view["headers"][1]["val"]	="County Name";
			//$table_view["headers"][2]["sort"]	= array('field_name'=>encrypt($arr_sort[2]));
			//$table_view["headers"][2]["val"]	="State Abbreviation";
    ...
    public function add_information()           
    {
		...
		try
        {
            ...
            if($_POST)
            {
				...
				$this->form_validation->set_rules('opt_type_id', 'select county', 'required');
                ...
                else///validated, now save into DB
                {
                    ...
                    $info["state_id"]=$posted["opt_type_id"];
    ...
    public function modify_information($i_id=0)
    {
        ...
        try
        {
            ...
            else
            {
				...
				$posted["opt_type_id"]	   = trim($info["state_id"]);				
        ...
        try
        {
            ...
            if($posted["duplicate_value"]!="")
            {
                $qry=" Where ".(intval($posted["id"])>0 ? " c.id!=".intval($posted["id"])." And " : "" )
                    ." c.city='".$posted["duplicate_value"]."' AND c.state_id = '".$posted["state_id"]."'";
					
					
                //$info=$this->mod_rect->fetch_multi($qry,$start,$limit); /*don't change*/
				$info=$this->mod_rect->fetch_duplicate_city($qry,$start,$limit); /*don't change*/
