...
class Tradesman extends My_Controller
{
    ...
    public function __construct()
    {
        ...
        { 
		  ...
		  $this->cls_msg["save_profile"] = " profile details saved succesfully";
    ...
    public function dashboard()
    {
        ...
        {		
			...
			$this->data['today_msg'] = $this->private_message_model->gettotal_pmb_info($s_wh_pmb);
   ...
   /* Ajax pagination for all jobs */
	...
	function complete_job_update()
	{
		...
		if($_POST)
		{
			...
			/* end job quote mail to the user */					
				...
				//$this->session->set_userdata(array('message'=>$this->cls_msg["job_del"],'message_type'=>'succ'));
				$msg = '1|'.t('Feedback asked sucessfully');
			}
			else
			{
				//$this->session->set_userdata(array('message'=>$this->cls_msg["job_del_err"],'message_type'=>'err'));
				$msg = '2|'.t('Feedback not asked sucessfully');
			}
		}
		else
		{
    ...
    function pay_job($i_job_id=0)
	{
		...
		if($tot_recomm>=$waiver_comm)
		{
			...
			{		
			   ...
			   $mail_html = @fread($handle, filesize($filename));				
				...
				$this->session->set_userdata(array('message'=>'Congratulations! You are waived for the job payment.','message_type'=>'succ'));
		...
		$comm = $this->commission_slab_model->fetch_multi($s_where);
		//pr($comm); exit;
		$site_settings = $this->mod_site_setting->fetch_this(1);
		...
		{}*/
			$tot_amt = number_format(($job_details[0]['d_quote']*$comm[0]['s_commission_slab_100'])/100, 2, '.', '');
			//$tot_amt = doubleval(($job_details[0]['d_quote']*$comm[0]['s_commission_slab_100'])/100);
		//echo 	$job_details[0]['d_quote'].'===='.$comm[0]['s_commission_slab_100'];
		//pr($tot_amt); exit;
		//echo $tot_amt ;exit;
		...
		$data['currency'] = 'GBP';//$this->config->item('default_currency');
						  ...
						  <tr>
								<td><table width="100%" border="0" cellspacing="0" cellpadding="0">
											<tr>
												  ...
												  <td width="40%" align="right" style="padding:5px 12px;color:#616161;">49 Featherstone Street<br />
					LONDON<br />
					EC1Y 8SY</td>
											</tr>
									  </table></td>
											...
											<tr>
												  <td style="padding:0px 30px;"><table width="100%" border="0" cellspacing="0" cellpadding="0">
														...
														<table width="100%" border="0" cellspacing="0" cellpadding="3" style="margin:10px 8px 0px;">
															  <tr>
																	<td width="30%" valign="top">Amount In Words : </td>
																	<td width="70%" valign="top" style="color:#616161;"> '.$paid_amount_word.' only</td>
															  </tr>
															  <tr>
...
*/				$this->data['feedback_contents'] = $contents;
	...
	function pagination_ajax($start=0,$param=0) {	
		...
		$sessArrTmp['src_job_category_id']  = $this->get_session_data('i_category_id');
		$sessArrTmp['src_job_radius'] 		= $this->get_session_data('opt_radius');
		$sessArrTmp['src_job_postal_code']  = $this->get_session_data('txt_postal_code');
                              ...
                              <table width="100%" border="0" cellspacing="0" cellpadding="0">
                                    ...
                                    <table width="100%" border="0" cellspacing="0" cellpadding="3">
                                          ...
                                          <tr>
                                                ...
                                                <td width="80%" valign="top" style="color:#616161;">  '.$paid_amount_word.' </td>
                                          </tr>
                                            <tr>
                                                  <td valign="top"> Payment Method  :</td>
                                                  <td style="color:#616161;" valign="top"> PAYPAL  </td>
