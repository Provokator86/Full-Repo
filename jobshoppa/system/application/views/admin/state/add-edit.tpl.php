...
$("#frm_add_edit").submit(function(){
    ...
    $("#div_err").hide("slow"); 
	 ...
	 if($.trim($("#txt_state_title").val())=="") 
    {
        s_err +='Please provide county name.<br />';
        b_valid=false;
    }
	else if(!pattern.test($("#txt_state_title").val()))
	{
		s_err +='Please provide proper county name.<br />';
        b_valid=false;
	}
	
	/*if($.trim($("#txt_state_description").val())=="") 
    {
        s_err +='Please provide state abbreviation.<br />';
        b_valid=false;
    }
	else if(!pattern.test($("#txt_state_description").val()))
	{
		s_err +='Please provide proper state abbreviation.<br />';
        b_valid=false;
	}*/
...
<input type="hidden" id="h_id" name="h_id" value="<?php echo $posted["h_id"];?>"> 
    ...
    <div>
      <table width="100%" border="0" cellspacing="0" cellpadding="0">
		...
		<tr>
          <td>Title *:</td>
          <td><input id="txt_state_title" name="txt_state_title" value="<?php echo $posted["txt_state_title"];?>" type="text" size="50" /></td>
          <td>&nbsp;</td>
        </tr>
		
		<?php /*?><tr>
          <td>Abbreviation *:</td>
          <td><input id="txt_state_description" name="txt_state_description" value="<?php echo $posted["txt_state_description"];?>" type="text" size="50" /></td>
          <td>&nbsp;</td>
        </tr><?php */?>
        ...
        <tr style="display:none;">
          <td>Active:</td>
          <td><input id="i_state_is_active" name="i_state_is_active" value="1" <?php if($posted["i_state_is_active"]==2) echo ''; else echo 'checked="checked"';?>  type="checkbox" /></td>
          <td>&nbsp;</td>
        </tr>
      </table>
      </div>
